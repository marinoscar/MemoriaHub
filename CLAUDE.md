...
- `POST /api/media/location-suggestions/bulk-accept` body `{ circleId, minConfidence }` - Enqueues a `location_bulk_accept` enrichment job that accepts every pending suggestion at/above a confidence floor, unmodified (always `coordSource='inferred'`); returns immediately `{ data: { jobId, status } }` — the accept loop runs on the enrichment queue rather than inside the request, decoupling it from the HTTP request lifetime; idempotent: a re-click while a job is already pending/running for the circle returns the existing job instead of enqueueing a duplicate (media:write + collaborator)

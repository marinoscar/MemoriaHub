...
- `GET /api/media/dashboard?circleId=` - On This Day + recent/favorites + review-queue counts; also returns `pendingBurstGroups` count when burst detection is enabled for the circle and `pendingSimilarityGroups` count when visual deduplication is enabled for the circle
...
Similar Photos groups near-duplicate photos across the **entire circle library** using dHash perceptual-hash Hamming distance only — no temporal window, no same-device requirement. This is the key difference from Burst Detection. Per-circle opt-in (default off), non-destructive review queue. Groups are surfaced only once they reach `similarity.minGroupSize`. `GET /api/media/dashboard` returns a `pendingSimilarityGroups` count. The default Hamming threshold (6 bits) is stricter than burst detection (10 bits) because there is no temporal/device corroboration.
- `GET /api/media/similar?circleId=&status=&page=&pageSize=` - List similarity groups (review queue); items `{ id, circleId, status, mediaCount, createdAt, suggestedBestItemId, suggestedBestThumbnailUrl, coverThumbnailUrls[] }`; response `{ items, meta:{total,page,pageSize} }` (media:read + viewer)
- `GET /api/media/similar/:id` - Group detail; ordered members `{ id, similarityScore, sharpnessScore, thumbnailUrl, width, height, capturedAt, importedAt, isSuggestedBest }` (media:read + viewer)
- `POST /api/media/similar/:id/resolve` body `{ keepIds[] }` - Keep selected members, soft-delete the rest, mark resolved (media:delete + collaborator)
- `POST /api/media/similar/:id/dismiss` - Mark "not duplicates": clear `similarityGroupId`/`similarityScore` on all members, status=dismissed (media:write + collaborator)
- `POST /api/media/similar/backfill` body `{ circleId, from?, to?, force? }` - Bulk-enqueue similarity_detection over the circle, optionally limited to a `capturedAt` range (`from`/`to`, ISO-8601); the job computes the perceptual hash on demand for photos that lack one (retroactive fingerprinting of legacy uploads); requires visualDedupEnabled; returns `{ enqueued }` (media:write + collaborator)

### Circle Visual Dedup Settings (circles:read / circles:write + per-circle viewer/circle_admin role)
- `GET /api/circles/:id/dedup-settings` - Get visual deduplication opt-in flag for a circle (circles:read + viewer)
- `PUT /api/circles/:id/dedup-settings` body `{enabled}` - Enable/disable visual deduplication for a circle; writes audit event (circles:write + circle_admin)
...
- `circles` - Family circles; `is_personal=true` circles cannot be deleted; `face_recognition_enabled` column (default false) controls face recognition per-circle opt-in; `auto_tagging_enabled` column (default false) controls auto-tagging per-circle opt-in; `burst_detection_enabled` column (default false) controls burst detection per-circle opt-in; `visual_dedup_enabled` column (default false) controls Similar Photos per-circle opt-in
...
- `similarity_groups` - Circle-scoped visual near-duplicate review groups; one row per detected similarity cluster; status `pending` | `resolved` | `dismissed`; `suggestedBestItemId` FK → `media_items` (SetNull on delete); `mediaCount` denormalized member count; `resolvedById` / `resolvedAt` track who actioned the group; ordered by `createdAt` (no `capturedAt` — members may span many years); members linked via `media_items.similarity_group_id`
...
**Similar Photos (Visual Deduplication):**
- `VISUAL_DEDUP_ENABLED` - Global kill-switch for auto-enqueue on upload; set to `false` to disable `SimilarityEnqueueListener` for all circles (per-circle opt-in still applies when `true`; default: `true`)

The following similar-photos parameters are controlled via system settings (not environment variables), editable in the Admin UI under `similarity.*`:
- `similarity.hashDistance` — integer, 0–32, default 6; maximum Hamming distance (bits, out of 64) for two photos to be considered near-duplicates; stricter than burst detection's default of 10 because there is no temporal or device corroboration in library-wide matching
- `similarity.minGroupSize` — integer, 2–20, default 2; minimum number of items required for a group to be surfaced in the review queue
- `similarity.maxGroupSize` — integer, default 50; maximum members per group; excess members (by `importedAt DESC`) are left ungrouped to prevent a single highly-duplicated photo from creating an unbounded group
...
- [Similar Photos / Visual Deduplication](docs/specs/visual-dedup.md) — library-wide dHash Hamming grouping (no temporal/device gating), `dhash_bits bit(64)` HNSW index, stricter default threshold (6 bits vs. burst's 10), non-destructive review queue, per-circle opt-in, honest accuracy trade-off documented

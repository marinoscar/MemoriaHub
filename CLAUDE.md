...
- `POST /api/admin/jobs/reset-stuck` (jobs:write) - Reset jobs stuck in `running` past a threshold (including `startedAt IS NULL` zombie rows, aged by `createdAt`) back to pending; optional `{olderThanMinutes}` body — when omitted, defaults to the `jobs.stuckThresholdMinutes` system setting (no hard-coded default)
...
- `ENRICHMENT_STUCK_MINUTES` - **Legacy fallback only.** The stuck-job threshold is now the runtime `jobs.stuckThresholdMinutes` system setting (see Job History Retention below); this env var seeds that setting's default (clamped to 120) only until an explicit value is saved via the admin UI, after which it has no effect. The automatic stuck-job reset cron (`EnrichmentStuckResetTask`, runs every 10 min) resets `running` jobs beyond the resolved threshold back to `pending` (including zombie rows with `startedAt IS NULL`, aged by `createdAt`) so the worker can re-claim them after an OOM kill or process restart; the threshold must exceed the longest expected single-job runtime

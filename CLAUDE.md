...
A third, distinct gap is a `StorageObject` permanently stuck at `status='failed'` — most commonly a HEIC/HEIF photo that failed to decode before the ffmpeg-transcode fallback below existed (issue #106), but also any image that exhausted `STORAGE_PROCESSING_MAX_RETRIES` for another reason. Unlike the cron above (which targets `status='processing'`, objects orphaned mid-pipeline by a crash), nothing automatically retries a `failed` object — it needs an explicit admin trigger.
- `POST /api/admin/media/reprocess-failed` body `{ limit? }` - Finds all image StorageObjects at `status='failed'` (excluding `thumbnails/`-prefixed objects) and re-runs the full processing pipeline for each via `StorageProcessingRecoveryService.recoverFailedImageObjects` — regenerates the thumbnail and, via `OBJECT_PROCESSED_EVENT`, re-fires face/tag/duplicate enrichment automatically. Optional `limit` bounds the batch size. Distinct from `reprocess-stuck` (targets `status='processing'`) — `reprocess-failed` is the one to use for a format-decode backlog like HEIC. Returns `{ claimed, reprocessed, exhausted, errors }` (Admin + storage:delete_any)
...
On-demand configuration health sweep across ~25 checks in 8 sections (core, auth, storage, AI, face, geo, jobs, nodes); reuses each domain's existing "test connection" service for live provider connectivity and surfaces feature-flag/provider inconsistencies (e.g. a feature enabled with no provider configured). The AI & Enrichment section includes an `ai.socialMedia` check that reports whether social-media video detection is off (skipped), misconfigured (env override / out-of-range `socialMedia.*` values), running degraded (OCR unavailable, Tier-1-only), or fully operational (two-tier). The AI & Enrichment section also includes an `ai.pictureEnhancer` check (`skipped` when `features.pictureEnhancement` is off; `warning` when the `PICTURE_ENHANCEMENT_ENABLED` env override disables it or no `ai.features.enhance` model is selected; `error` when no enabled credential is configured for the resolved provider; `ok` when a credential and model are both present) — see the [AI Picture Enhancer spec](docs/specs/picture-enhancer.md). The new `nodes` (Worker Nodes) section adds four checks: registered nodes (any nodes registered at all), heartbeat freshness (nodes not seen within the expected interval), expired leases (claimed jobs whose `lease_expires_at` has passed without renewal), and per-node capability health (each node's last-reported `capabilities`/`node doctor` summary). The Face section adds a `face.pgvector` check that verifies the `faces.embedding_vec` column and its three HNSW indexes exist when `FACE_VECTOR_BACKEND` resolves to `pgvector` (skipped when the backend is `app`); it warns separately if the column/main index is missing (actionable: run migrations or roll back to `app`), if only the partial archive index is missing (face-auto-archive KNN degrades to the main index, slower but functional), or if only the partial assigned-set index is missing (person-match KNN falls back to the main index and can be starved by unassigned faces after bulk imports). No new permission, nothing persisted, no cron — a fresh report is computed on every call. See [Doctor Diagnostics spec](docs/specs/doctor.md).
...
An admin dashboard at `/admin/settings/jobs` provides monitoring and control over the generic `enrichment_jobs` queue (used by face detection, storage insights computation, and all future enrichment handlers). As of issue #258 (epic #238) this page is the **pilot migration onto the shared DataTable** — see [DataTable spec §18](docs/specs/datatable.md#18-the-pilot-migration--job-queue-258). The migration is presentational: no API endpoint, parameter, or permission changed, and the `status` / `type` / `processedWithin` / `scheduled` filter semantics are preserved exactly (including `scheduled=true` forcing `status=pending`, which the UI now models as a filter mutually exclusive with `status`). Three user-visible additions come from the component rather than the page: the 13 columns are responsive (card list on a phone, row expansion on a tablet) with a per-user column picker and density persisted under `user_settings.dataTables['admin-jobs']`, a CSV export of the current page or all matching rows, and row selection with a "Re-run selected" bulk action (a sequential loop over the existing `POST /api/admin/jobs/:id/retry`, not a new endpoint). The queue-wide operations — Retry all failed, Reset stuck, Repair missing thumbnails — remain page-level buttons, since they act on the whole queue rather than on a selection.
- `GET /api/admin/jobs/stats` (jobs:read) - Queue stats: total, byStatus, byType breakdown, `stuckRunning` count, `stuckThresholdMinutes` (the effective threshold, minutes, used to compute `stuckRunning` — resolved from the `jobs.stuckThresholdMinutes` system setting), and `scheduled` (count of pending jobs currently in backoff, i.e. `scheduledFor > now`); `stuckRunning` also counts zombie rows stuck in `running` with `startedAt IS NULL` (aged by `createdAt` instead), which earlier versions of this count silently missed; result is cached in-process for ~2s (unwindowed, all-time counts unchanged) to bound per-poll query cost on the dashboard's 5s poll — see [Job Queue Insights spec §2](docs/specs/job-insights.md#2-on-demand-aggregate-model)
...
CLI-driven machines can register as worker nodes, claim `enrichment_jobs` rows, run the compute locally, and submit results back to the API — a complete, end-to-end feature (control plane, result ingestion, compute/persist split, shared parity package, worker daemon). Media bytes stream directly between the node and the storage provider via short-lived presigned URLs — they are never proxied through the API, and no storage credential is ever placed on the node. `auto_tagging`/`geocode` are the one pair of job types where a node does see a provider secret: a **transient, per-job credential** fetched from the API and held in memory only for the duration of one compute call, never persisted to disk/config/logs (the originally-proposed "AI-proxy" design, where the server made the keyed call on the node's behalf, was rejected in favor of this). See the [Distributed Nodes spec](docs/specs/distributed-nodes.md) for full detail. `video_face_detection` compute is now fully implemented and node-eligible (matches the server's video face detection pipeline: frame extraction, per-frame detection, cross-frame dedup, and thumbnail upload). The one remaining gap is `thumbnail_repair`, which was removed from the CLI's node-claimable `NODE_JOB_TYPES` — it stays server-only since it's a global sweep job with no `inputUrl` for a node to act on, not a per-item unit of work; the API-side `nodeResultSchema`/`persistNodeResult` pair is kept only for interface parity with `thumbnail_regen`.

**Node control plane** (PAT-auth via `jobs:write`, owner-scoped to the registering user's node):
- `POST /api/nodes/register` - Register a new worker node (name, hostname, platform, cliVersion, eligibleTypes, concurrency); idempotent per (owner, name) — re-registering under a name the caller already owns re-attaches to the existing record (refreshes hostname/platform/cliVersion/eligibleTypes/concurrency, status→online) instead of creating a duplicate row; returns the node record plus `reattached: true|false`
...
- `GET /api/nodes/models/manifest` (jobs:read) - List enrichment model files/versions a node should have locally to serve its `eligibleTypes`, with real sha256/byte-size values for the CLIP ONNX file (face detection runs against the CompreFace sidecar, not a local model file, so it has no manifest entry)
...
- `GET /api/admin/nodes/credentials` (jobs:read) - List all node credentials across the deployment
- `DELETE /api/admin/nodes/credentials/:id` (jobs:write) - Revoke any node credential

**Node Credentials** (durable, `/api/nodes/*`-only auth; owner-scoped, requires the owner's `jobs:write`, no new permission):
- `POST /api/node-credentials` - Mint a new `nod_`-prefixed bearer credential; accepted ONLY on `/api/nodes/*` routes — it can never reach media/settings/admin endpoints, even for an Admin owner; raw token shown once and never retrievable again
- `GET /api/node-credentials` - List the caller's node credentials (masked)
- `DELETE /api/node-credentials/:id` - Revoke a node credential

A `nod_` credential's `expiresAt` is nullable (may never expire) and it is individually revocable at any time. PATs still authenticate on `/api/nodes/*` routes for back-compat; `nod_` is the new preferred least-privilege option for a worker that runs unattended for a long time (a systemd service or a fleet container), since it cannot be used against any other part of the API even if leaked. Managed in the web UI at `/admin/settings/nodes` (Node Credentials section: list, create-and-copy-once, revoke).

> **CLI:** `apps/cli` gains `memoriahub node install-deps|register|start [--daemon]|stop|status|logs|set-concurrency|service install|uninstall|status|list|doctor|enroll` for running a fat worker that performs enrichment compute locally, streaming media bytes to/from storage via presigned URLs. `node install-deps` (Linux-only) is a one-command dependency installer that sets up ffmpeg/ffprobe, the npm native compute libraries, tesseract OCR language data, model files, and (optionally) Docker + the local `compreface-core` sidecar before you register a node — see [Worker Node Setup & Troubleshooting](docs/worker-node-setup.md). `node enroll` is the interactive onboarding path: device-flow login, then mints and stores a `nod_` node credential automatically as the CLI's token. `node start --daemon` detaches into the background (pidfile + a Unix-domain-socket NDJSON IPC channel at `~/.memoriahub/node.sock`); `node service install` installs a systemd **user** unit so the node runs as an always-on service (with WSL/Windows guidance when systemd isn't available); `node stop`/`status`/`logs`/`set-concurrency` all talk to a running daemon over IPC when one exists. The Tools > Worker Node TUI dashboard **attaches** to an already-running daemon over the same IPC socket, so an operator can inspect a headless service without stopping it; when no daemon is running, its `[s]` key spawns the memory-tuned detached daemon and attaches (the interactive TUI process cannot re-exec to raise its own ~2 GB heap ceiling without breaking raw-mode input — issue #156/PR #159 — so sustained compute never runs in-process by default), with `[e]` as an explicit in-process fallback that warns and tightens the memory watchdog when the heap is untuned. Heavy model libraries (face/vision/CLIP/OCR) are `optionalDependencies` loaded at runtime, so a lean CLI install doesn't force-download them; the shared `packages/enrichment-compute` package (dual CJS/ESM build, exact-pinned native deps) guarantees a node's compute is numerically identical to the server's — see the spec's parity section and its golden-vector regression test. For a production fleet, the recommended deployment is the containerized worker image (`ghcr.io/marinoscar/memoriahub-worker`, multi-arch, version-pinned) via the root `docker-compose.worker.yml` bundle (scale with `--scale memoriahub-worker=N`) rather than a native per-machine install — see [Worker Node Setup & Troubleshooting](docs/worker-node-setup.md) — paired with `ENRICHMENT_WORKER_MODE=system` on the API so its in-process worker stays limited to server-only jobs while the fleet handles all per-item media compute.

### Workflows (Media Workflow Automation — media:read / media:write / media:delete + per-circle roles)
Rule-based automation on a **Subject · Trigger · If · Then** model (v1 Subject: Media Item): a circle collaborator defines a workflow (conditions over the Media Item field catalog + an ordered action list), previews exactly what it matches, and runs it manually, on a cron schedule, or automatically whenever a new item finishes enrichment. Every action reuses an existing bulk-mutation service call (`MediaService.bulkArchive`/`bulkTags`/`bulkUpdateMedia`, `BurstService`/`DuplicateService`/`LocationSuggestionService` resolve/dismiss/accept/reject) — no parallel mutation logic. Runs execute through the `enrichment_jobs` queue (background priority 100, never starving upload enrichment), are fleet-compatible (the batch-execution job type is node-claimable), and are gated behind `features.workflows` (default off) + the `WORKFLOWS_ENABLED` env kill-switch. No new RBAC permission — reuses `media:read`/`media:write`/`media:delete` + per-circle `viewer`/`collaborator` roles. See the [Workflow Automation spec](docs/specs/workflows.md) for the full model, condition/action catalog, run-lifecycle state machine, scale strategy, and node-execution split.

- `POST /api/workflows` body `{ circleId, name, description?, enabled?, trigger?, cronExpression?, definition }` - Create a workflow; validates the definition (registry-aware: subject/fields/ops/actions must be registered); enforces `workflows.maxWorkflowsPerCircle`; rejects `hard_delete` on a non-manual trigger (media:write + collaborator)
- `GET /api/workflows?circleId=&page=&pageSize=` - List workflows in a circle (paginated); each item includes a derived `dependencies[]` array (media:read + viewer)
- `GET /api/workflows/subjects` - Return the per-Subject registry: available Subjects and, per Subject, its field catalog (operators/valueType/enumValues/dependency) + action catalog (type/label/destructive) + valid triggers; drives the builder's dynamic form; v1 returns a single Subject, `media_item` (media:read)
- `POST /api/workflows/preview` body `{ circleId, definition }` - **Stateless** builder feedback: `{ matchedCount, capped, sample: [≤12 items w/ signed thumbnailUrl] }`; count is bounded by `LIMIT (cap+1)`, never a full `COUNT(*)` (media:read + viewer)
- `GET /api/workflows/:id` / `PATCH /api/workflows/:id` / `DELETE /api/workflows/:id` - Get / update (re-validates trigger/action compatibility against the resulting post-patch definition) / delete (cascades runs + items) a workflow (media:read + viewer for GET; media:write + collaborator for PATCH/DELETE)
- `POST /api/workflows/:id/run` body `{ maxItems? }` - Start a run; `{ runId, status }`; 409 if `workflows.maxConcurrentRuns` exceeded; enforces gated-action authorization (feature flags, both-circle perms for `move_to_circle`) at create time (media:write + collaborator)
- `GET /api/workflows/:id/runs?page=&pageSize=` - Run history for one workflow (media:read + viewer)
- `GET /api/workflow-runs/:id` - Run detail: counts, per-status item tally, and a bounded (5,000-row-scanned) best-effort per-action-type summary (media:read + viewer)
- `GET /api/workflow-runs/:id/items?status=&page=&pageSize=` - Paginated run items with batched signed thumbnails (media:read + viewer)
- `POST /api/workflow-runs/:id/approve` body `{ excludedItemIds?≤500, confirmation? }` - Approve an `awaiting_approval` run; excluded items → `excluded`; when the definition contains `hard_delete`, `confirmation` must equal the literal string `DELETE {matchedCount}`; re-checks gated-action authorization (media:write + collaborator)
- `POST /api/workflow-runs/:id/cancel` - Cancel a non-terminal run (media:write + collaborator)

**Admin oversight** (Admin role, no circle-membership check):
- `GET /api/admin/workflows/stats` (system_settings:read) - KPI strip: `{ windowDays: 7, runsLast7Days, itemsActioned, failures, currentlyRunning }`
- `GET /api/admin/workflows?page=&pageSize=&circleId=&trigger=&enabled=` (system_settings:read) - Every workflow across every circle, with circle/creator, last-run summary, and lifetime `totals: { runs, matched, actioned }`
- `GET /api/admin/workflow-runs?status=&page=&pageSize=&circleId=&workflowId=` (jobs:read) - Every run across every circle
- `POST /api/admin/workflows/:id/disable` (system_settings:write) - Force-disable a workflow regardless of circle membership (admin override)
- `POST /api/admin/workflow-runs/:id/cancel` (jobs:write) - Stop a runaway run app-wide

### Media — List / Gallery (media:read + per-circle viewer role)
`GET /api/media` is DUAL-MODE, selected by whether `page` is present (issue #104):
- **Keyset mode (default — omit `page`)**: used by the web gallery. Ordered by `(sortBy, id)` DESC with an `id` tiebreak for determinism when multiple items share the same `sortBy` value; runs **no `COUNT(*)`**. Response `meta: { pageSize, nextCursor: string|null, hasMore: boolean }`; pass `cursor=<nextCursor>` to fetch the next page. Served by the `media_items_gallery_idx` partial composite index (see Database Tables below) as a pure index scan with no separate sort node.
- **Legacy offset mode (`page` provided)**: unchanged behavior for legacy clients (Android app, CLI). Response `meta: { page, pageSize, totalItems, totalPages }`, backed by a `COUNT(*)`.
- `GET /api/media?circleId=&pageSize=&cursor=&sortBy=&sortOrder=&...filters` (keyset) or `&page=&pageSize=&...` (offset) - List circle media with filters (type, date range, album, favorite, tag, geo, camera, person, etc. — see the various `GET /api/media?...` filter bullets throughout this doc); each item includes a signed `thumbnailUrl` (media:read + viewer)

Thumbnail signing for every list surface (listMedia, dashboard, album, archived, trash, explore, search, bursts, duplicates, people, location-suggestions, shares) is batched through `MediaThumbnailService.signThumbsBatched` — one `storageObject.findMany` per page instead of a per-item lookup. Signed thumbnail URLs use a 24h TTL with an in-memory per-key cache so the same URL string (and thus the browser's cached bytes, since thumbnail objects are uploaded with `Cache-Control: public, max-age=31536000, immutable`) is reused across gallery visits.
...
- `POST /api/media/trash/empty` body `{ circleId }` → `{ runId, status, matchedCount }` — **asynchronous** as of issue #165 (previously a synchronous purge that timed out past ~2,000 items): starts a background `TrashEmptyRun` that hard-deletes ALL trashed items in a circle via the enrichment queue; 409 if a run is already in progress for the circle; poll `GET /api/trash-empty-runs/:id` for progress (media:delete + circle_admin)
- `GET /api/trash-empty-runs/:id` — Get a trash-empty run's counters (`matchedCount`, `processedCount`, `succeededCount`, `failedCount`, `skippedCount`) plus `itemStatusCounts` (live tally by item status) (media:read + viewer)
- `GET /api/trash-empty-runs/:id/items?status=&page=&pageSize=` — Paginated run items with batched signed thumbnails; `status` filters `matched|deleted|failed|skipped` (media:read + viewer)
- `POST /api/trash-empty-runs/:id/cancel` — Cancel a non-terminal run (cooperative — in-flight batch jobs stop before their next unclaimed chunk, already-deleted items stay deleted); 400 if the run already finished (media:delete + circle_admin)

Automatic purge: an hourly cron (`TrashPurgeTask`) enqueues a global `trash_purge` enrichment job that hard-deletes trashed items whose `deletedAt` is older than `storage.trash.retentionDays` days. The job runs on the shared enrichment worker and is visible in `/admin/jobs`. See `storage.trash.retentionDays` under System Settings below. `trash_purge`, `POST /api/media/trash/delete-forever`, and the async empty-trash run's `trash_empty_execute_batch` job (see the `enrichment_jobs` table below and [Archive & Trash spec §10](docs/specs/archive-trash.md#10-empty-trash-at-scale-async-run-model)) all share the same batched hard-delete path, `MediaService.purgeMediaItemsBatched`, which uses the storage provider's native batched `deleteMany` (S3 `DeleteObjectsCommand`, ≤1,000 keys/call) instead of one delete call per item.
...
- `GET /api/media/dashboard?circleId=` - On This Day + recent/favorites + review-queue counts; also returns `pendingBurstGroups` (when `features.burstDetection` is enabled globally), `pendingDuplicateGroups`, `pendingLocationSuggestions`, and `pendingEnhancements` (counts only `ready`-status enhancements — queued/processing rows aren't yet actionable) counts; all four are nested under the response's `counts` object, not top-level (issue #201 fixed a drift where the web client read them from `counts.*` while the API returned them at the top level, leaving all three pre-existing review-queue banners on Home permanently dead)
- `GET /api/media/review-counts?circleId=` - Lightweight companion to the dashboard endpoint above, returning ONLY the same four review-queue integers — no On This Day / recent / favorites payload, no thumbnail signing: `{ data: { pendingBurstGroups, pendingDuplicateGroups, pendingLocationSuggestions, pendingEnhancements } }` (issue #204). Added so the web sidebar's "AI Enhancements" badge — which needs exactly one integer and, being a React hook, must fire unconditionally on every circle switch regardless of whether `features.pictureEnhancement` is on — doesn't have to pay for three signed-thumbnail media collections it never renders. Both endpoints share one implementation, `MediaService.computeReviewCounts`, so the counts can't drift between them (media:read + viewer)

### Media — Burst Detection (media:read / media:write / media:delete + per-circle roles)
Burst detection is enabled globally via `features.burstDetection` system setting (default off) and non-destructive — no photo is deleted until a human confirms. Groups are surfaced in a review queue only once they reach `burst.minGroupSize`. `GET /api/media/dashboard` returns a `pendingBurstGroups` count that feeds the review-queue section of the dashboard UI. Archive and Trash surfaces show a burst-origin badge/link (to `/bursts/:id`) when an archived/trashed item's `burstGroupId` survives group resolution — resolve does not clear it, only dismiss does — letting a user confirm a kept copy exists before permanent deletion.
- `GET /api/media/bursts?circleId=&status=&page=&pageSize=&sortBy=&sortOrder=` - List burst groups (review queue); items `{ id, status, mediaCount, suggestedBestItemId, capturedAt, confidence, suggestedBestThumbnailUrl, coverThumbnailUrls[] }`; `confidence` is a 0–1 visual-cohesion score persisted at detection time (may be `null` for legacy groups); `sortBy` `capturedAt`(default)|`confidence`|`mediaCount`, `sortOrder` `asc`(default)|`desc`; a null `confidence`/`capturedAt` (legacy group) sorts last regardless of direction; every ordering has a trailing `id` tiebreaker; response `{ items, meta:{total,page,pageSize} }` (media:read + viewer)
...
- `POST /api/media/bursts/bulk/resolve-by-threshold` body `{ circleId, threshold (int 0–100), action: 'archive'|'trash' }` - **Async** (issue #190, rebuilt on the shared review-run engine — see the [Review Runs spec](docs/specs/review-runs.md)): starts a run that resolves every PENDING burst group whose persisted `confidence >= threshold/100` (null-confidence legacy groups excluded — never auto-resolved by this path); manual trigger only, no cron; same keep-`suggestedBestItemId`-archive/trash-the-rest semantics as the bulk resolve above; returns immediately `{ data: { runId, status, matchedCount } }` (`matchedCount` is 0 at creation, the real total once evaluated); 409 if a `burst_group` review run is already in progress for the circle; no group-count cap (the old `MAX_THRESHOLD_RESOLVE`/`remaining` are gone) (media:write + collaborator; trash requires media:delete)
- `POST /api/media/bursts/bulk/dismiss-by-threshold` body `{ circleId, threshold (int 0–100) }` - **Async**, same run engine as above: starts a run that dismisses every PENDING burst group whose persisted `confidence < threshold/100` (null-confidence legacy groups excluded); manual trigger only, no cron; mark "not a burst": ungroups members, status=dismissed (nothing archived or trashed); returns `{ data: { runId, status, matchedCount } }`; 409 if a run is already active (media:write + collaborator); together with `bulk/resolve-by-threshold` above, one threshold partitions the pending queue: resolve `>= N%`, dismiss `< N%`

### Media — Duplicates (media:read / media:write / media:delete + per-circle roles)
Near-duplicate detection is enabled globally via `features.duplicateDetection` system setting (default off) and non-destructive — no photo is archived or trashed until a human confirms. Catches visually-identical re-uploads (e.g. WhatsApp re-shares: recompressed/resized/filtered copies with a different `contentHash` and stripped EXIF) that exact-hash dedup and burst detection cannot — see the [Duplicate Detection spec](docs/specs/duplicate-detection.md). Two-tier matching, OR-combined: CLIP ViT-B/32 visual-embedding cosine similarity (pgvector KNN, `dedup.similarityThreshold`) OR dHash Hamming distance (`dedup.hashMaxDistance`); union-find grouping mirrors burst detection, with explicit exclusion rules so the two review queues never fight over the same items. **Burst wins:** since `burst_detection` and `duplicate_detection` are enqueued together on upload, either can run first — if dedup wins the race and groups an item before burst detection does, `BurstDetectionService.processMediaItem` evicts that item (and the rest of its new burst group) from any duplicate group via `DuplicateDetectionService.evictFromDuplicateGroups` once it forms the burst group, so an item never sits in both review queues at once (see spec §3.2). This eviction is now backed by a write-time guarantee: `DuplicateDetectionService` re-checks `burstGroupId` under a `SELECT ... FOR UPDATE` row lock immediately before writing `duplicateGroupId`, closing the residual race at write time rather than relying solely on reactive eviction; additionally, `burst_detection` is now enqueued at upload-time priority 5 vs. `duplicate_detection`'s priority 10, so burst is claimed first in the common case. Best-copy scoring and `kind` classification (`exact_variant`|`edited`|`similar`) are computed at read time, never persisted. `GET /api/media/dashboard` returns a `pendingDuplicateGroups` count (no minimum-size gate, unlike bursts — a group only ever exists with `mediaCount >= 2`). Archive and Trash surfaces show a duplicate-origin badge/link (to `/duplicates/:id`) when an archived/trashed item's `duplicateGroupId` survives group resolution — resolve does not clear it, only dismiss does.
- `GET /api/media/duplicates?circleId=&status=&kind=&page=&pageSize=&sortBy=&sortOrder=` - List duplicate groups (review queue), ordered chronologically by `capturedAt` by default; items `{ id, status, kind, mediaCount, suggestedBestItemId, capturedAt, confidence, coverThumbnailUrls[] }`; `confidence` is the tightest-pair CLIP cosine similarity (0–1) and is **persisted** as of issue #190 (`duplicate_groups.confidence`, single writer `DuplicateDetectionService.recomputeGroupMeta`, read-time self-heal backstop for pre-existing rows — see the [Review Runs spec §4](docs/specs/review-runs.md#4-duplicate-group-confidence-becomes-persisted)); `sortBy` `capturedAt`(default)|`confidence`|`mediaCount`, `sortOrder` `asc`(default)|`desc`. As of issue #194, all three `sortBy` values — including `confidence` — are a real DB `orderBy` over the persisted, indexed column, nulls-last in both directions, mirroring `burst_groups.confidence`'s existing ordering; each carries a trailing `id` tiebreaker. (Issue #189 originally added the `confidence` sort as an in-memory `Array#sort` because the column didn't exist yet; issue #190 persisted the column but deliberately left the sort in memory to keep that PR's read-path byte-identical; #194 finished the conversion.) `kind` filtering is still applied in application code after read-time classification, since `kind` is not a persisted column — `computeGroupKind` still runs its per-group similarity computation over the full matched set on every call regardless of `sortBy`, so this sort change removes one cost, not the dominant one; see [duplicate-detection.md §3.5.1](docs/specs/duplicate-detection.md#351-sortbyconfidence-is-a-db-orderby-issue-194). Response `{ items, meta:{total,page,pageSize} }` (media:read + viewer)
...
- `POST /api/media/duplicates/bulk/resolve-by-threshold` body `{ circleId, threshold (int 0–100), action: 'archive'|'trash' }` - **Async** (issue #190, shared review-run engine — [Review Runs spec](docs/specs/review-runs.md)): now that `confidence` is persisted (above), this filters directly in SQL exactly like the burst threshold endpoint — no more read-time `computeGroupKind` scan, no more 500-group cap; groups whose `confidence IS NULL` are excluded in both directions; returns immediately `{ data: { runId, status, matchedCount } }`; 409 if a `duplicate_group` review run is already active for the circle; the old `MAX_THRESHOLD_RESOLVE`/`hasMore` fields are gone (media:write + collaborator; trash requires media:delete)
- `POST /api/media/duplicates/bulk/dismiss-by-threshold` body `{ circleId, threshold (int 0–100) }` - **Async**, same run engine as above: dismisses every PENDING duplicate group whose persisted `confidence < threshold/100`; groups whose `confidence IS NULL` are excluded; mark "not duplicates": ungroups members, status=dismissed; returns `{ data: { runId, status, matchedCount } }` (media:write + collaborator); together with `bulk/resolve-by-threshold` above, one threshold partitions the pending queue: resolve `>= N%`, dismiss `< N%`
...
- `POST /api/admin/duplicates/confidence/backfill` body `{ limit? }` - One-pass backfill of `duplicate_groups.confidence` for pre-existing `NULL` rows via the global `duplicate_confidence_backfill` job; not gated on `features.duplicateDetection`; returns `{ data: { jobId, status } }` (system_settings:write, Admin)
...
- `GET /api/media/location-suggestions?circleId=&status=&page=&pageSize=&mediaItemId=&sortBy=&sortOrder=` - List location suggestions (review queue); `status` defaults to `pending`; optional `mediaItemId` filters to one item's suggestion; items include thumbnail, confidence, method (`interpolated`|`nearest`), anchor IDs/gaps/distance, and `impliedSpeedKmh`; `sortBy` `createdAt`(default)|`confidence`, `sortOrder` `desc`(default)|`asc` — both columns are non-null, so (unlike bursts/duplicates) there's no nulls-last rule here; trailing `id` tiebreaker (media:read + viewer)
...
- `POST /api/media/location-suggestions/bulk-accept` body `{ circleId, threshold }` (threshold 0–100 int) - **Async** (issue #125; runs on the shared review-run engine as of issue #190 — see the [Review Runs spec](docs/specs/review-runs.md)): starts a run that accepts every pending suggestion in the circle at/above `threshold/100` confidence (always unmodified, `coordSource='inferred'`); returns immediately `{ data: { runId, status, matchedCount } }` (`matchedCount` is 0 at creation, the real total once evaluated); 409 if a `location_suggestion` review run is already in progress for the circle (media:write + collaborator)
- `POST /api/media/location-suggestions/bulk-reject` body `{ circleId, threshold }` - Async run that rejects every pending suggestion BELOW `threshold/100` confidence (the low-confidence noise); same response/409 shape as bulk-accept above (media:write + collaborator)
- `GET /api/review-runs/:id` - Run detail (shared across bursts/duplicates/location suggestions, see the [Review Runs spec](docs/specs/review-runs.md)): counters (`matchedCount`/`processedCount`/`succeededCount`/`failedCount`/`skippedCount`), `subjectType`, `action`, `threshold`, status, timestamps, `lastError`, plus `itemStatusCounts` (live tally by item status); deprecated alias `GET /api/location-suggestion-runs/:id` still works, delegating to the same service (media:read + viewer)
- `GET /api/review-runs/:id/items?status=&page=&pageSize=` - Paginated run items with batched signed thumbnails; `status` filters `matched|processing|applied|failed|skipped`; deprecated alias `GET /api/location-suggestion-runs/:id/items` still works (media:read + viewer)
- `POST /api/review-runs/:id/cancel` - Cancel a non-terminal run (cooperative — an in-flight batch finishes before the next unclaimed chunk stops; already-applied/rejected suggestions stay resolved); deprecated alias `POST /api/location-suggestion-runs/:id/cancel` still works (media:write + collaborator)
...
### Media — AI Picture Enhancer (media:read / media:write + per-circle roles)
Photo-only, single-item, human-reviewed AI enhancement via OpenAI `images.edit` (`gpt-image-1`) — see [AI Picture Enhancer spec](docs/specs/picture-enhancer.md). The AI output regenerates rather than retouches the image (potential fidelity risk on faces/text/detail) and is capped at `1024×1024`/`1024×1536`/`1536×1024`, so it is frequently lower-resolution than the original; nothing is ever auto-applied. The enhanced bytes are written to a staging object first — the user must explicitly review a side-by-side compare and choose **keep both** (creates a new `MediaItem`, inherits the source item's own `metadata` JSON, "AI Enhanced" system tag + `metadata._aiEnhanced` breadcrumb, original untouched) or **replace** (destructively overwrites the original storage key, mirroring orientation edit: nulls `contentHash` to force recompute, `reprocessObjectNow`, best-effort re-enqueues `face_detection`), or **discard**. Only one live (`pending`/`processing`/`ready`) enhancement per item — a new request supersedes and discards any prior unresolved staging. Enabled globally via `features.pictureEnhancement` (default off, and the only one of the app's eight feature flags with a dedicated admin toggle page at `/admin/settings/enhancer`); requires `ai.features.enhance` (provider+model) to be configured. The enhanced file itself carries a real EXIF marker: `ExifCarryoverService` (`apps/api/src/enhancement/exif-carryover.service.ts`, backed by `exiftool-vendored`) copies the original's EXIF/GPS/IPTC/XMP/ICC onto the enhanced JPEG and stamps `Software`/`XMP-xmp:CreatorTool` = `"MemoriaHub AI Enhance (<model>)"` plus `XMP-iptcExt:DigitalSourceType` = the IPTC `compositeWithTrainedAlgorithmicMedia` NewsCode, gated on `pictureEnhancement.stampExif` (now defaults `true`); strictly best-effort (a missing `exiftool-vendored`/`perl` install degrades to unstamped bytes, never fails the job) — see the spec's §5.1 for the full tag list and the `Orientation#=1` PrintConv-bypass gotcha.
- `POST /api/media/:id/enhance` body (all optional — §4.1 of the spec: `intent`, `preset` (`restore_old_photo`\|`low_light`\|`colorize_bw`\|`portrait_polish`, orthogonal to `intent`), `adjustments.*`, `strength`, `quality` (per-run override of `pictureEnhancement.defaultQuality`), `preserveFaces`, `instructions`, `model`) - Start an enhancement; photo-only guard (400 for video/non-image); creates a `media_enhancements` row (`pending`) and enqueues a `picture_enhancement` job at priority 0; supersedes any existing live enhancement for the item; returns `202` `{ data: { enhancementId, jobId, status: 'pending' } }`; 400 if feature disabled / not a photo / no `ai.features.enhance` configured (media:write + collaborator)
- `GET /api/media/:id/enhance/:enhancementId` - Poll status + compare payload: `{ data: { id, status, model, original: {url,width,height,size}, enhanced: {url,width,height,size}, downscaled, params } }` (byte fields as strings; `enhanced.width/height` reflect the actual returned bytes, not the requested canvas); `lastError` present on `failed` (media:read + viewer)
- `GET /api/media/:id/enhance` - Get the latest enhancement for the item (resume review after reload); same payload shape as above (media:read + viewer)
- `POST /api/media/:id/enhance/:enhancementId/apply` body `{ decision: 'keep_both' | 'replace' }` - Commit the result; `keep_both` → `201` with the new item id; `replace` → `200` `{ data: { status: 'ready'|'failed', width, height } }`; 400 if not ready / `pictureEnhancement.allowReplace=false` / blocked by `pictureEnhancement.blockReplaceOnDownscale`; writes an `audit_events` row (`media_enhancement:applied`) (media:write + collaborator)
- `POST /api/media/:id/enhance/:enhancementId/discard` - Delete staging bytes, row → `discarded`; 204 No Content (media:write + collaborator)
- `GET /api/media/enhancements?circleId=&status=&page=&pageSize=&sortBy=&sortOrder=` - Cross-item paginated listing of a circle's AI enhancements, backing the `/enhancements` hub (issue #201); `status` accepts a concrete `MediaEnhancementStatus` or one of three aliases — `in_progress` (pending+processing), `awaiting_decision` (ready), `terminal` (applied+discarded+expired); byte-size fields (`original.size`/`enhanced.size`) are strings; `expiresAt` is computed server-side from `updatedAt` + `pictureEnhancement.retentionHours` and is non-null only for `ready`/`failed` rows; `enhanced.*` (a signed URL to the staged full-resolution bytes, not a thumbnail derivative) is populated only while `status='ready'` (media:read + viewer)

Enrichment: `picture_enhancement` (per-item, priority 0, reason `rerun`) downloads the original, sharp auto-orients it (deterministic pre-pass), calls the configured OpenAI model via `enhanceImage()` with `input_fidelity`, reads the actual output dims via sharp, applies the EXIF carryover above (once, on the staged bytes — both `keep_both` and `replace` inherit the stamp), uploads the result to a staging object, and records dims/size → `ready`/`failed`. `OpenAiProvider.enhanceImage` preserves the provider's HTTP `status`/`code`/`retry-after` on the rethrown error, so a genuine OpenAI 429/529 IS classified as a rate limit and routed through the rate-limit-deferral path like other enrichment handlers; the handler's catch is a state machine that rolls the `media_enhancements` row back to `pending` (not `failed`) on a retryable or rate-limited failure with budget remaining, matching whatever `EnrichmentTerminalService` is about to do to the job, and only tombstones the row `failed` on the terminal attempt. Server-only in v1 — no `nodeResultSchema`/`persistNodeResult`, absent from the CLI `NODE_JOB_TYPES`, mirroring the `location_inference`/`face_auto_archive_sweep` precedent. An hourly cron (`PictureEnhancementPurgeTask`) enqueues a global `picture_enhancement_purge` job that deletes staging objects and marks rows `expired` for `ready`/`failed` enhancements older than `pictureEnhancement.retentionHours`, mirroring `TrashPurgeTask`.

> **UI:** An "AI Enhance" (`AutoFixHigh`) trigger appears in the gallery selection top bar (`BulkActionToolbar`, single-photo selection only) and in the full-screen viewer (`MediaLightbox`, next to Edit orientation), both gated on `useFeatureFlags()` (reading `GET /api/features` below, not the Admin-only `GET /api/system-settings`, so non-admin circle members see the button too). `MediaEnhancementDrawer` walks a preset picker (Auto / Restore old photo / Low-light rescue / Colorize B&W ["interpretive"] / Portrait polish / Custom, pre-filling the existing toggles) → progress (distinguished queued-vs-enhancing states with elapsed time, 2s poll) → a draggable before/after compare slider (`BeforeAfterSlider`, with a side-by-side toggle and full-screen zoom, downscale warning) → keep both / replace / discard confirm dialogs, with replace hidden/disabled per the admin's replace policy instead of failing with a 400 after confirmation. `MediaDetailDrawer` shows an "AI Enhanced" badge.

### Settings — Client-Visible Feature Flags
- `GET /api/features` - Return client-visible feature flags for the calling user: `{ features: { <all features.* booleans> }, pictureEnhancement: { enabled, allowReplace, blockReplaceOnDownscale, model } }`; `pictureEnhancement.enabled` already folds in the `PICTURE_ENHANCEMENT_ENABLED` env kill-switch (via the shared `isPictureEnhancementEnabled()` helper, also used by the two server-side enhance gates, so client and server can't drift) and `model` surfaces only the configured model name, never a credential. Unlike `GET /api/system-settings` (Admin-only `system_settings:read`), this endpoint requires **authentication only** — any role, no permission — because it exists specifically so non-admin circle members can resolve gated affordances (currently just the AI Picture Enhancer) without a 403. No other settings namespace is exposed. (authenticated, any role)

### Notifications (authenticated, any role)
Notification Center (epic #240, issues #245 + #246 + #247) — the in-app bell. **No new RBAC permission**, deliberately: every route is already scoped to the JWT's `userId` and a notification is inherently personal, so there is nothing an Admin-gated permission would protect — the same least-privilege, authentication-only rationale as `GET /api/features`. Every row is written through the SINGLE service write path `NotificationsService` (`apps/api/src/notifications/`), which exposes exactly THREE producer primitives:
- `emit()` — one row per occurrence, never deduplicated. Used by the two EVENT types where each occurrence is genuinely its own thing: `workflow_run_completed` and `share_expiring`.
- `upsertCountedEvent()` — EVENT types that must NOT fan out (`upload_completed`, `enrichment_failed`): repeated occurrences fold into ONE row whose `data.count` grows, optionally inside a rolling time window. Event types have **no** partial unique index behind them (#244's index predicate lists only the four `review_queue_*` values), so the find-or-create is serialized with a transaction-scoped Postgres **advisory lock** — `pg_advisory_xact_lock(hashtext(<userId|type|circleId|matchData>))` — which, unlike a row lock, works when no row exists yet, exactly the case that has to be protected. Optional `matchData` adds a JSONB containment predicate (`data @> …`) that further partitions live rows beyond `(userId, circleId, type)`.
- `upsertState()` — the four `review_queue_*` STATE types: at most ONE LIVE row per `(userId, circleId, type)`, refreshed in place, so a circle with 929 pending duplicate groups produces exactly ONE row per user rather than 929 (enforced in the DB by the partial unique index `notifications_review_queue_live_uniq_idx`).

All three are **best-effort / fire-and-forget**: a failed notification write logs a warning and never throws into or blocks the triggering action — the same contract as the transactional email sends. `NotificationsModule` exports `NotificationsService` plus the two injectable #247 producers (`UploadNotificationService`, `WorkflowRunNotificationService`) and `NotificationPreferencesService` (#251).

**Per-type user preferences (epic #240, issue #251) — enforced in the write path, not the producers.** Preferences live in the `user_settings.notifications` namespace (see the `user_settings` table entry above for the schema, the absent-means-ENABLED rule, and the PATCH merge semantics) — **no new table, no new endpoint, no migration, no new RBAC permission**. The gate is applied in **all THREE `NotificationsService` write paths** (`emit()`, `upsertCountedEvent()`, `upsertState()`), so a suppressed type writes **no row at all** and no producer — present or future — has to remember to check: if it goes through the service, it is gated. On `upsertCountedEvent()` the check runs *before* the transaction opens, so a suppressed type never contends for the advisory lock. The lookup is `NotificationPreferencesService`, a **per-user TTL cache** (`Map<userId, …>`, 5 s, mirroring `SystemSettingsService.getSettings()` and diverging from its single un-keyed slot for the same reason the unread-count cache does — preferences are per user) — necessary because these methods are called **per recipient in hot loops** (a bulk upload fans `upsertCountedEvent` out to every circle member for every file). Its `prime(userIds)` batched form is used by #246's reconcile, which knows a circle's whole eligible membership up front, turning N `findUnique`s into one `findMany`. The gate is **fail-open**: a preferences read that fails resolves to all-enabled with a logged warning, so a broken lookup over-notifies rather than silently swallowing everything.
- **Disabling a type also DISMISSES its existing live rows** for that user (sets `dismissed_at`), because the intent behind flipping a toggle off is "stop showing me this", not "stop showing me this except for the eleven already there". For a STATE type it is the only way out at all: the reconcile that would otherwise drain those rows to zero is now gated on the same preference, so they would nag forever. The dismissal runs in the **same transaction** as the settings write (`UserSettingsService.writeSettings` → `NotificationsService.dismissTypesForUser(userId, types, tx)`), so the two commit or roll back together; a transaction is opened **only** when something is actually suppressed, leaving the common save (theme, profile, a table layout) the single statement it was. The suppressed set is "types disabled **after** this write", not "types this write newly disabled" — dismissing an already-off type is a no-op (the gate means it has no live rows), it needs no before/after diff, and it makes every settings save self-healing. The preference cache is invalidated **after** the write returns (never inside the transaction, which may still roll back), so a re-enabled type resumes on the very next notification instead of up to one TTL later.
- **`workflowMicroRuns`** (absent ⇒ **false**) is a second, independent gate specific to `on_media_enriched` workflow micro-runs, honored in `WorkflowRunNotificationService` *after* recipient resolution (the preference belongs to whoever would be notified). It is orthogonal to the `workflow_run_completed` type toggle — **both** must pass for a micro-run to notify.
- **Module wiring:** `SettingsModule → NotificationsModule` (for the dismiss-on-disable write). Safe and cycle-free precisely because `NotificationsModule` imports nothing; correspondingly `NotificationPreferencesService` reads `user_settings` with `PrismaService` **directly** rather than through `UserSettingsService`, since importing `SettingsModule` back into `NotificationsModule` is exactly the cycle the no-imports rule exists to prevent.
- `GET /api/notifications?status=unread|read|all|dismissed&circleId=&page=&pageSize=` - Paginated list, newest first (`createdAt DESC, id DESC` tiebreak), `pageSize` max 100; `status` defaults to `all`. **Dismissed rows are EXCLUDED from `unread`/`read`/`all` and returned ONLY by `status=dismissed`** — dismissing means "stop showing me this", so the only way back to a dismissed row is to ask for it explicitly. Returns `{ items, meta: { page, pageSize, totalItems, totalPages } }`
- `GET /api/notifications/unread-count` - `{ count }` for the bell badge: live (non-dismissed), never-read rows across all circles. Result is cached in-process for ~2s, **keyed per user** in a `Map<userId, …>` — this deliberately diverges from the single un-keyed slot in `GET /api/admin/jobs/stats` (`STATS_CACHE_TTL_MS`) that it otherwise mirrors, because job stats are global while unread counts are not; copying that pattern literally would serve one user's badge to another. The user's entry is invalidated on every mutation of theirs (read, dismiss, read-all, dismiss-all, delete, emit, upsertState) so the badge never lags behind their own action; the Map is bounded by an expired-entry sweep past a soft cap
- `POST /api/notifications/:id/read` - Set `read_at` (idempotent — preserved via `COALESCE`, so a repeat call never moves the original timestamp) — 204 No Content
- `POST /api/notifications/:id/dismiss` - Set `dismissed_at` (idempotent; **implies read** — also backfills `read_at` when null, so a dismissed row never lingers in the unread count) — 204 No Content
- `POST /api/notifications/read-all` - Optional `{ circleId? }` scope → `{ updated: number }`
- `POST /api/notifications/dismiss-all` - Optional `{ circleId? }` scope → `{ updated: number }`
- `DELETE /api/notifications/:id` - Hard-delete a single row — 204 No Content

**Review-queue producer — `NotificationReconcileTask` (epic #240, issue #246).** The four `review_queue_*` STATE rows are produced by an hourly `@Cron(CronExpression.EVERY_HOUR)` reconcile (`apps/api/src/notifications/notification-reconcile.task.ts` → `ReviewQueueReconcileService`, wired in `NotificationsReconcileModule`), NOT by per-request or per-upload events — this replaces the old Home dashboard banners, which only existed while the user happened to be looking at the dashboard. It is a **reconcile, not an event stream**: each tick re-states the truth, so a missed write, a crash mid-run, or a hand-resolved row all converge on the next tick. Env kill-switch `NOTIFICATIONS_RECONCILE_ENABLED=false` (mirrors `THUMBNAIL_REPAIR_ENABLED`); overlapping runs are skipped by an in-process `sweepInFlight` boolean (single-process only, and sufficient — cross-replica the writes are idempotent, so the worst case is duplicated effort, never a duplicated row). Unlike `TrashPurgeTask`/`ThumbnailRepairTask` it does its work **in process** rather than enqueuing an `enrichment_jobs` row: the sweep is a bounded set of small reads and upserts, and a new job type would need a schema change for no benefit. Circles are keyset-paged 100 at a time; a per-circle failure is logged and counted, never fatal to the rest of the sweep.
- **Audience — `collaborator` and `circle_admin` members ONLY.** `viewer` members are excluded: resolving or dismissing a review group needs `media:write` + the per-circle `collaborator` role, so a viewer could not act on the notification and it would be pure noise.
- **Counts come from `MediaService.computeReviewCounts`** — the same method behind `GET /api/media/dashboard` and `GET /api/media/review-counts` (made public for this caller; it is access-check-free by design, and the two HTTP callers run their own membership check). Called **once per circle** and fanned out to every eligible member, so there is no parallel counting path that could drift from what the user sees on those surfaces.
- **Feature-gated per queue:** `features.burstDetection` → `review_queue_bursts` (`/bursts`), `features.duplicateDetection` → `review_queue_duplicates` (`/duplicates`), `features.locationInference` → `review_queue_location_suggestions` (`/location-suggestions`), and the shared `isPictureEnhancementEnabled()` helper (folds in the `PICTURE_ENHANCEMENT_ENABLED` env kill-switch, so this sweep can never advertise a queue the enhance endpoints would reject) → `review_queue_enhancements` (`/enhancements`). Flags are read through the cached `SystemSettingsService.getSettings()` **once per sweep**, not once per circle. A disabled queue produces nothing — and any rows produced before it was disabled are resolved in one bounded statement, since the reconcile that would otherwise drain them to zero no longer runs for that type. All four off ⇒ circle paging is skipped entirely. Links are circle-agnostic by design (#249/#250 switch the active circle from the row's `circleId` before navigating).
- **Reconcile semantics per (circle × eligible member × type):** `count > 0` with no live row → insert via `upsertState()` (never a direct `create` — a concurrent run must fold into the existing row rather than race the partial unique index into a P2002); `count > 0` with a live row → refresh `title`/`data.count`/`updated_at` in place; `count` drops to 0 → set `dismissed_at` so a drained queue stops nagging. Titles are singular/plural aware (`1 burst group ready to review in {circle}` / `2 burst groups …`; enhancements read `awaiting review`), `data` is `{ count, countAtRead? }`, `circle_id` is populated.
- **Re-unread rule:** a count change NEVER touches `read_at`, with exactly ONE exception — a **read** row whose `newCount > data.countAtRead` (the snapshot below) is re-marked unread. Never on a decrease, never on an equal count, and **never on a read row with no `countAtRead` key** (an absent snapshot is treated conservatively as "already seen"; re-unreading on it would re-nag every legacy row on every tick). `countAtRead` is carried forward across refreshes rather than synthesized, because `upsertState()` replaces `data` wholesale and dropping it would silently disable the rule until the row's next read.
- **Known gap (deliberate):** the live-row lookup is scoped by `userId` so the `(user_id, dismissed_at)` index serves it — the `notifications` table has no `circle_id` index. A member DEMOTED to `viewer` or removed from the circle therefore falls outside that set, and their now-unactionable row is not auto-resolved; they can dismiss it, and #248's retention sweep reaps it.

**Event producers (epic #240, issue #247).** Four producers, in `apps/api/src/notifications/producers/`. Each is best-effort and fire-and-forget: a notification write never fails, blocks, or rolls back the action that triggered it. **Every producer's real design problem is volume**, and each solves it differently — the guards below are the load-bearing part of this feature, not an optimization.

- **`upload_completed` — `UploadNotificationService`, 15-minute ROLLING WINDOW.** Hooked in `MediaService.createMedia` immediately after `enqueueUploadEnrichment`, on the **non-deduplicated** path only (the two dedup returns exit earlier; a dedup hit is not an upload — nothing new landed in the circle). A 4 000-file import must be ONE row that grows to `count: 4000`, not 4 000 rows, with **no batch id and no client change**: each registration folds into the live `(user, circle, upload_completed)` row whose `updated_at` is inside the window, or starts a new one if there is none. The window is anchored on `updated_at`, not `created_at`, so a steady import keeps extending the SAME row instead of rolling a fresh one every 15 minutes, while an import next week is correctly its own row. **Race:** two concurrent uploads into one circle could both observe "no live row in window" and both INSERT — prevented by `upsertCountedEvent()`'s advisory lock on the aggregation key, which is why that primitive exists rather than reusing `emit()`. **Audience: the uploader PLUS every other circle member, regardless of per-circle role** — deliberately unlike #246's collaborator-only review-queue audience, since browsing new photos needs no write access, so a `viewer` genuinely can act on it. Titles are pluralized and role-specific: uploader gets `"{count} item(s) uploaded to {circle}"`, everyone else `"{uploaderName} added {count} photo(s) to {circle}"` with `data: { count, uploaderId }`; `link: '/'` for both. Circle name + member ids and the uploader's display name are cached in-process for 30 s so a bulk import does not re-read them per file.
- **`enrichment_failed` — `EnrichmentFailureNotificationListener`, roll-up per job type, NO window.** Hooked on `ENRICHMENT_JOB_SETTLED_EVENT` with `outcome === 'failed'`. That event is emitted by `EnrichmentTerminalService` from **exactly the two genuinely terminal branches** — the rate-limit path only when `giveUp` (`rateLimitHits >= ENRICHMENT_RATELIMIT_MAX_HITS`) and the normal-failure path only when `!shouldRetry` (`attempts >= ENRICHMENT_MAX_ATTEMPTS`) — and never on an intermediate retry or a rate-limit deferral, both of which requeue to `pending` and skip `emitSettled`. Subscribing to it therefore inherits terminal-only semantics from the single chokepoint shared by the in-process worker AND node-reported failures, with **no edit to `EnrichmentTerminalService`** and no `NotificationsModule → EnrichmentModule` import (the event name is a plain constant, not a provider); hooking the service directly would have meant re-deriving `giveUp`/`shouldRetry` at a second site that could drift. `lastError` is not on the event, so the job row is re-read — only ever on a terminal failure, and after the terminal write, so the value is final. **Recipients: users holding the Admin system role only** (`userRoles.some(role.name = 'admin')`, the same predicate `DoctorService` uses; cached 60 s), because the action link is Admin-gated and a non-admin could not act on it. **Roll-up: one live row per `(admin, jobType)` with `circle_id = null` and NO time window** — a poison-pill batch failing 400 jobs is ONE row with `count: 400`. The absence of a window is deliberate: an unnoticed failure backlog should keep accumulating into the same row until an admin dismisses it, not silently split every 15 minutes. Partitioning is `matchData: { jobType }` (JSONB containment), so a `geocode` backlog and an `auto_tagging` backlog stay separate rows. Title `"{count} {jobType} job(s) failed permanently"`, `data: { jobType, lastError }` (`lastError` truncated to 500 chars — the job row holds the full text), link `/admin/settings/jobs?status=failed`.
- **`workflow_run_completed` — `WorkflowRunNotificationService`, per-run, no aggregation.** Called fire-and-forget from the three terminal `WorkflowRun` transitions: `WorkflowEvaluateHandler`'s 0-matches `completed` and its retries-exhausted `failed`, and `WorkflowExecuteBatchHandler.maybeFinalizeRun` — inside its `fin.count > 0` guard, so only the batch that WON the conditional finalize notifies. The producer is handed a `runId` and re-reads the run rather than trusting caller-supplied counters, so a caller that lost the finalize race produces nothing. **Volume guard: `on_media_enriched` micro-runs are SKIPPED by default** — they fire continuously during an import (one rolling micro-run per workflow every ~5 minutes, each terminating through that same `maybeFinalizeRun` path) and would drown the bell in exactly the fan-out this feature exists to prevent. #251 keeps that as the default and makes it **per-user opt-in** via `user_settings.notifications.workflowMicroRuns` (absent ⇒ false), checked *after* recipient resolution since the preference belongs to whoever would be notified. The skip lives in the producer, not at the call sites, so no terminal path can forget it. Recipient: the run's `started_by_id`; for unattended (`scheduled`) runs the workflow's `created_by_id` — the user whose permissions authorize the unattended run — with each falling back to the other when null (both are SetNull FKs) and a run with neither silently skipped. Title `"Workflow '{name}' finished: {succeededCount} applied, {failedCount} failed"`, link `/workflows/runs/{runId}`, `data: { runId, status, succeededCount, failedCount }`.
- **`share_expiring` — `ShareExpiringService` + `ShareExpiringTask`, daily sweep with a `notifiedFor` stamp.** `@Cron(CronExpression.EVERY_DAY_AT_9AM)`, same task shape as `NotificationReconcileTask`/`TrashPurgeTask` (env kill-switch `NOTIFICATIONS_SHARE_EXPIRING_ENABLED=false`, in-process `sweepInFlight` overlap guard, never throws), doing its work **in process** rather than enqueuing an `enrichment_jobs` row for the same reason #246 does. A sweep rather than an event because expiry is a passage of time with nothing to hook. Selects `media_shares` with `revoked_at IS NULL` and `expires_at` in `(now, now + 7 days]` — the `gt: now` half excludes already-dark links, which are noise rather than an action item — keyset-paged 200 at a time; a per-share failure is logged and counted, never fatal to the rest. Recipient: the share's `created_by`; `MediaShare.createdBy` is a REQUIRED FK with `onDelete: Cascade`, so selecting the relation makes "creator still exists" a structural INNER JOIN rather than a hopeful post-check. Title `"A public share of '{targetName}' expires {relativeDate}"` (`today`/`tomorrow`/`in N days`, bucketed on elapsed hours so an expiry 3 hours out reads `today`, not the `tomorrow` a naive day-ceiling would give), link `/admin/settings/sharing`.
  - **Idempotency rule (the whole point):** the sweep runs daily and the lead window is 7 days wide, so the same share is a candidate on seven consecutive days. Each emitted row stamps `data.notifiedFor = <expiresAt ISO>`, and the sweep skips any share already carrying a row with the same `(shareId, notifiedFor)` pair — **LIVE *or DISMISSED***. Checking dismissed rows too is the load-bearing half: an owner who dismisses today's warning must not be re-warned tomorrow. Keying on the expiry VALUE rather than a boolean "notified" flag is what makes a rescheduled share work: if the owner pushes `expires_at` out, the stamp no longer matches and a fresh notification IS allowed — correctly, since the new date is genuinely new information.
- **Re-unread on increment (both counted types):** incrementing an EXISTING `upload_completed`/`enrichment_failed` row clears `read_at`, so continued activity re-badges the bell. This is the event-type analog of #246's re-unread rule, minus the snapshot comparison: a counted row only ever grows when something new happened, so the increment IS the "new activity" signal that STATE rows have to infer from `data.count > data.countAtRead`.
- **Module wiring — the cycle that had to be avoided:** `NotificationsModule` imports NOTHING, and that is load-bearing. `MediaModule → NotificationsModule` (for `UploadNotificationService`) and `WorkflowsModule → NotificationsModule` (for `WorkflowRunNotificationService`) both point INTO it; adding any import to `NotificationsModule` that transitively reaches `MediaModule`/`WorkflowsModule` would close a cycle. This is exactly why #246 put its review-queue reconcile — which needs `MediaService.computeReviewCounts` — in the separate `NotificationsReconcileModule`, and that module now also hosts #247's two SELF-DRIVEN producers (`ShareExpiringService`/`ShareExpiringTask` and `EnrichmentFailureNotificationListener`), which nobody injects and which add no import edge of their own: the share sweep needs only `PrismaService` + `NotificationsService`, and the enrichment listener subscribes to an event-name constant rather than injecting anything from `EnrichmentModule`. Net edges: `NotificationsReconcileModule → {NotificationsModule, MediaModule, SettingsModule}`, `MediaModule → NotificationsModule`, `WorkflowsModule → NotificationsModule` — no forwardRef anywhere.

**`countAtRead` snapshot (contract with #246):** both read paths also set `data.countAtRead = data.count` for rows whose `data.count` exists (the `review_queue_*` STATE types). #246 uses that snapshot to re-mark a row unread only when the queue has since grown past what the user last saw. Because `updateMany` cannot perform a per-row JSONB merge, `read-all` is TWO statements in one transaction: a raw `UPDATE … SET read_at = now(), updated_at = now(), data = jsonb_set(data, '{countAtRead}', data->'count') WHERE … AND jsonb_exists(data, 'count')`, followed by a plain `updateMany` for the rest — ordering matters, since step 1 has already set `read_at` on the rows it touched, they no longer satisfy step 2's `readAt: null` filter, which sidesteps needing a negated JSON predicate Prisma's typed `where` cannot express. `updated` is the SUM of both. `jsonb_exists(data, 'count')` is used rather than the equivalent `data ? 'count'` operator (no `?`-operator precedent in this repo, and it avoids any question about Prisma's tagged-template parameter parsing). Dismiss paths write no snapshot — a dismissed row is invisible to every live listing and to #246's re-mark logic, so nothing would read it.

**Two invariants worth remembering:** (1) every raw `UPDATE` here sets `updated_at = now()` EXPLICITLY, because the column has no database trigger — it is Prisma-side `@updatedAt` only, which raw SQL bypasses, and #248's retention purge keys off `(dismissed_at, updated_at)`. (2) Every `:id` route scopes its WHERE by BOTH `id` AND the JWT `userId`, so another user's valid id returns **404, not 403** (enumeration-resistant, matching the public-share 404 policy).
...
- `GET /api/media/locations?type=&capturedAtFrom=&capturedAtTo=&country=&region=&locality=&place=&location=&albumId=&bbox=&limit=` - List the caller's geotagged (non-deleted) media as a flat array of lightweight points for map display (no pagination by default): `{ id, takenLat, takenLng, capturedAt, geoLocality }`; per-row thumbnail signing was removed (it was an N+1 pattern causing multi-second loads and 502s on large circles) — use `GET /api/media/thumbnails` to fetch thumbnails for the items actually in view; new `bbox` param scopes results to a viewport cell; optional `albumId` scopes results to a single album's members, used by the album Map view; optional `limit` (1–500) caps the result to the most recent N (existing `capturedAt desc` ordering) — deliberately optional since the album Map view still needs every point plotted (media:read + viewer)
- `GET /api/media/locations/aggregate?circleId=&precision=<0-5>&zoom=<0-22>&bbox=<minLng,minLat,maxLng,maxLat>&capturedAtFrom=&capturedAtTo=&type=` - Server-side spatial clustering for the map view; buckets points into a grid via a single GROUP BY (no thumbnails, no metadata reads); returns `Array<{ lat, lng, count, sampleId, minLat, maxLat, minLng, maxLng }>` — the min/max fields are the cell's true bounding box (same GROUP BY, no extra scan), since a centroid alone can't locate a cluster's photos for the client's "refetch by bbox" flow (media:read + viewer)
...
- `GET /api/ai/models?provider=&capability=` - List available models for a provider; optional `capability` param: `chat` (default), `embedding`, or `image` — when `embedding`, returns embedding model IDs (OpenAI only: `text-embedding-3-small`, `text-embedding-3-large`; other providers return empty); when `image`, returns image-edit (AI Picture Enhancer) model IDs (OpenAI only, curated list: `['gpt-image-1']`; other providers return empty) (ai_settings:read)
...
- `PUT /api/ai/features/enhance` body `{ provider, model }` - Set active provider and model for the AI Picture Enhancer (ai_settings:write)
- `GET /api/admin/ai/enhance/status` - Get AI Picture Enhancer feature/provider status: `{ featureEnabled, provider, model, credentialConfigured, ready }`; backs the Doctor `ai.pictureEnhancer` check and the `/admin/settings/enhancer` readiness panel (system_settings:read, Admin)
...
CompreFace (keyless `compreface-core` sidecar, 128-d mobilenet, `requiresCredentials:false`) is the sole face-recognition provider (issue #113 removed the Human WASM and AWS Rekognition providers). The Face Settings UI has a "Test connection" button since even a keyless provider needs sidecar connectivity verified. Face recognition is enabled globally via `features.faceRecognition` system setting (default off); the per-circle `face_recognition_enabled` column was dropped in migration `20260621050000_drop_circle_feature_flags`.
...
- `/admin/settings/enhancer` — global AI Picture Enhancer toggle (`features.pictureEnhancement`), a readiness panel consuming `GET /api/admin/ai/enhance/status`, and all seven `pictureEnhancement.*` defaults/policy knobs; linked from `/admin/settings/ai`
...
- `/admin/settings/workflows` — global Media Workflow Automation toggle (`features.workflows`), per-trigger master switches, all `workflows.*` limits with scale-rationale help text, a danger card for the hard-delete unlock (`workflows.allowHardDelete`), a 7-day KPI strip, and a cross-circle oversight table (list/disable any workflow, cancel a runaway run) — see the [Workflow Automation spec](docs/specs/workflows.md)
...
- `jobs:write` - Retry, reset, and delete enrichment jobs (Admin only); also gates registering/managing a caller's own worker nodes and minting/revoking that owner's durable node credentials (`nod_` tokens, accepted only on `/api/nodes/*` routes) — no separate node permission exists
...
- `user_settings` - Per-user settings (JSONB); includes `activeCircleId` (UX convenience, never trusted for authz) and `dataTables` — per-user, per-table DataTable layout (issue #255, epic #238) keyed by stable `tableId` (`/^[a-z0-9][a-z0-9_-]*$/`, ≤64 chars, ≤40 ids per user), each entry `{ visibleColumns?: string[] (≤60, each ≤64 chars), density?: 'compact'|'standard'|'comfortable', sort?: { field, direction }, pageSize?: int 1–500 }` with unknown keys rejected (`.strict()`). Read/written ONLY through the existing `GET`/`PATCH`/`PUT /api/user-settings` — no new endpoint, no new table, no new permission, no migration (the namespace is optional and absent is the normal state). **Every key is optional and is never populated server-side**: an absent namespace/entry/field means "use the column contract's priority-derived defaults", so a newly-added column appears instead of being silently hidden for anyone who ever saved a layout — adding a `.default()` anywhere in this namespace would invert that. PATCH merges **per `tableId`, one level deep**: an unlisted table id is untouched, a listed one is replaced wholesale (so `{}` resets it to defaults), and `null` deletes it (JSON Merge Patch); the 40-id cap is re-checked post-merge in `UserSettingsService` so an over-cap merge is a 400, not a raw ZodError 500. Canonical schema: `dataTablesSchema` in `apps/api/src/common/schemas/settings.schema.ts`; see [DataTable spec §14](docs/specs/datatable.md#14-persistence--user_settingsdatatables-255). Also includes `notifications` — per-user notification preferences (issue #251, epic #240): `{ enabled?: boolean, types?: { [NotificationType]: boolean }, workflowMicroRuns?: boolean }`, unknown keys AND unknown type names both rejected (`.strict()` + a zod enum over the live `NotificationType` values). Read/written ONLY through the existing `GET`/`PATCH`/`PUT /api/user-settings` — no new endpoint, no new table, no new permission, no migration, and nothing is ever populated server-side. **Absent means ENABLED** (an absent namespace, an absent `enabled`, or an absent type key), which is exactly what makes it migration-free — every existing row has no namespace, so shipping it mutes nobody — and makes a newly-added `NotificationType` **opt-OUT** rather than silently opt-in for anyone who ever saved a preference; adding a `.default()` anywhere in this namespace would invert that, same warning as `dataTables`. The ONE inverted default is `workflowMicroRuns` (absent ⇒ **false**), which re-enables the `on_media_enriched` workflow micro-run rows #247 suppresses unconditionally; that inversion lives in `resolveNotificationPreferences()` (`apps/api/src/notifications/notification-preferences.service.ts`), NOT in the zod schema, so the schema keeps one uniform "optional, no default" rule. Note `z.partialRecord` (not `z.record`) is used for `types`: in zod v4 `z.record(z.enum([…]), …)` is **exhaustive** and would reject `{ upload_completed: false }` for omitting the other seven. PATCH merges **field-wise, and per type key inside `types`** (`UserSettingsService.mergeNotifications`): an unlisted field/type is untouched, a listed one is replaced, a type mapped to `null` is deleted (resetting it to its default rather than pinning an explicit `true`), and `notifications: null` clears the whole namespace back to defaults. Canonical schemas: `notificationPreferencesSchema` / `notificationPreferencesPatchSchema` in `apps/api/src/common/schemas/settings.schema.ts`
...
- `face_provider_credentials` - Face provider API keys/config (AES-256-GCM encrypted via same key as AI); one row per provider; `last4` exposed; plaintext never stored or returned. CompreFace is keyless — the credential row (if present) stores only a `baseUrl` override — no API key is set or required.
...
- `faces` - Individual detected face records with bounding box, confidence, and a 128-d embedding (`Float[]` fallback or pgvector column; CompreFace mobilenet is the only provider, so this dimension is fixed — the `externalFaceId` column that formerly supported AWS Rekognition's delegated no-embedding matching was dropped in the issue #113 migration, along with the ~125 legacy `providerKey='human'` 1024-d rows it made permanently incompatible); keyed to `mediaItemId` + `circleId`; `manuallyAssigned` flag protects user-labeled faces from re-clustering; video-specific columns added in migration `20260627000000_face_video_columns`: `videoTimestampMs` (Int?, representative appearance time in ms from video start; null for photos), `videoTimestamps` (Int[], all frame timestamps where this identity was observed; empty for photos), `frameThumbnailKey` (Text?, storage key of the saved representative-frame JPEG; null for photos); `hiddenAt` (DateTime?, nullable; migration `20260704000000_add_face_hidden_at`) — archive flag for individual **unassigned** (`personId=null`) faces, mirroring `people.hiddenAt`; set/cleared via `PATCH /api/people/faces/bulk/hide|unhide`; archived faces are excluded from the unassigned-faces list and from clustering; permanent removal (`POST /api/people/faces/bulk/purge`) is a hard row delete (not a soft-delete) that reclaims the `embedding` column; new index `faces(circle_id, hidden_at)` added alongside a matching `people(circle_id, hidden_at)` index; `hiddenReason` (String?, `@map("hidden_reason")`; migration `20260712000000_add_face_hidden_reason`) — provenance of a hide: `'auto_archive_match'` when the auto-archive feature (`features.faceAutoArchive`) hid the face because it matched a previously-archived face, `null` for a manual `PATCH /api/people/faces/bulk/hide` call. `embedding_vec` (`vector(128)`, pgvector; migration `20260715000000_add_face_embedding_vector`) is a derived column automatically maintained by the `faces_sync_embedding_vec` trigger from `embedding` — application code never writes it directly — indexed with three HNSW cosine indexes (`faces_embedding_vec_hnsw_idx` main, `faces_embedding_vec_archive_hnsw_idx` partial on `person_id IS NULL AND hidden_at IS NOT NULL`, `faces_embedding_vec_assigned_hnsw_idx` partial on `person_id IS NOT NULL AND embedding_vec IS NOT NULL` added by migration `20260718000000_add_face_assigned_embedding_index` to close a bulk-import person-match starvation gap where the full-table main index's KNN walk was dominated by unassigned faces, issue #133) enabling indexed KNN face matching (`FACE_VECTOR_BACKEND=pgvector`, now the default); NULL only for empty embeddings (e.g. manual `providerKey='manual'` associations).
- `enrichment_jobs` - Generic background job queue for all enrichment handlers (face detection, video face detection, storage insights, etc.); statuses: `pending`, `running`, `succeeded`, `failed`; reasons: `upload`, `rerun`, `backfill`; `media_item_id` and `circle_id` are **NULLABLE** — null values indicate a global/system job that is not scoped to a single media item or circle (e.g. the `storage_insights` handler); idempotency for global jobs deduplicates on `(type, media_item_id IS NULL)`; three backoff columns: `scheduled_for` (DateTime?, when the job becomes eligible again — null = eligible now; the worker claim query skips jobs where `scheduled_for > now`), `rate_limited_at` (DateTime?, timestamp of the most recent rate-limit hit), `rate_limit_hits` (Int default 0, count of rate-limit deferrals tracked separately from `attempts`); `attempts` (Int default 0) is charged at CLAIM time — it means "attempts STARTED", not "attempts failed", so a job that takes the whole process down (OOM SIGKILL) and never reaches the in-process failure path still consumes its attempt; this is what lets the stuck-reset path (cron + `POST /api/admin/jobs/reset-stuck`) mark a stuck `running` job **failed** once `attempts >= ENRICHMENT_MAX_ATTEMPTS` instead of requeueing it forever, bounding a poison-pill/OOM job to `ENRICHMENT_MAX_ATTEMPTS` crashes; rate-limit deferrals un-charge the claim-time increment so they never consume an attempt; `storage_migration` copies a single object from source to target provider (copy→verify→repoint→leave source; one job per object; `skipDedup` option prevents the `(type, mediaItemId IS NULL)` dedup from collapsing per-object jobs); `video_face_detection` samples frames from a video via ffmpeg, runs the active face provider on each frame, deduplicates faces across frames by embedding similarity, and writes one `Face` row per identity cluster per video (skipped without downloading — marked `no_faces` — when the item is flagged social-media (`social_media_source` non-null), when `face.video.enabled=false`, or when the object exceeds `VIDEO_ENRICHMENT_MAX_BYTES`); `job_history_purge` is a global nightly job (`mediaItemId: null`, `circleId: null`) that batch-deletes terminal rows (`status IN (succeeded, failed)` with `finishedAt` older than `jobs.history.retentionDays`) in 5 000-row batches — pending and running rows are never deleted; before deleting each batch it folds the rows into the `job_stats_rollup` lifetime aggregate (in the same transaction) so all-time analytics survive the purge; enqueued by `JobHistoryPurgeTask` at midnight; `notification_purge` is a global nightly job (`mediaItemId: null`, `circleId: null`, priority 100, dedup'd against any pending/running instance) enqueued by `NotificationPurgeTask` at midnight that batch-deletes `notifications` rows in 5 000-row chunks, in two passes — `dismissed_at IS NOT NULL AND dismissed_at < now() - notifications.retentionDays`, then `read_at IS NOT NULL AND read_at < ` the same cutoff (each a clean single-column range scan on the index #244 added for it). **UNREAD ROWS ARE NEVER DELETED BY AGE ALONE** — a deliberate asymmetry with `job_history_purge` above, which deletes purely by `finishedAt` age: it is read/dismissed STATE, not age, that makes a notification disposable, because an unread one is by definition something the user has not dealt with. Gated by `notifications.purgeEnabled` (re-checked in the handler, not only the cron, so a manual `/admin/jobs` retry honors it too); **server-only, not node-claimable** (no `nodeResultSchema`/`persistNodeResult`, absent from the CLI's `NODE_JOB_TYPES`) and auto-eligible for `ENRICHMENT_WORKER_MODE=system` by the same no-node-pair inference as `trash_purge`/`review_run_history_purge`; `location_inference` has TWO shapes sharing one handler dispatched on `mediaItemId` presence — per-item mode (`mediaItemId` set: infer a single item's coordinates from timeline anchors) and sweep mode (`mediaItemId: null`, `circleId` set, `payload: {mode:'sweep', from?, to?, force?}`: one job per circle computes suggestions for every eligible GPS-less item in that circle via a single in-memory pass, used by both the admin backfill and — unlike every other global job type in this table — never chunked into multiple jobs, since the work is pure DB read + in-memory compute rather than compute-bound; see [Location Inference spec](docs/specs/location-inference.md)); `social_media_detection` is a video-only, per-item gate: when `features.socialMediaDetection` is on, video uploads enqueue `social_media_detection` INSTEAD OF `video_face_detection` (priority 10); the handler runs pre-flight size/duration caps (`VIDEO_ENRICHMENT_MAX_BYTES`, `socialMedia.maxDurationSeconds`, `socialMedia.maxSizeBytes` → treat as clean, `matchedRule: skip-*-cap`, no download) then a landscape-orientation gate (strictly-landscape videos are never downloaded — Tier-1-only, no OCR), then classifies via container-metadata/filename rules (Tier 1) falling back to on-server OCR (Tier 2) when inconclusive-but-suspicious — a DETECTED result applies "Social Media" + platform tags and stops (no further video enrichment is enqueued), a CLEAN result fans out the withheld `video_face_detection` job via `MediaEnrichmentService.enqueueVideoPostDetectionEnrichment`; see [Social Media Detection spec](docs/specs/social-media-detection.md)); `thumbnail_regen` is the async worker-side counterpart to the synchronous single-item `POST /api/media/:id/thumbnail/rerun` — its handler resolves the item's `StorageObject` and calls `StorageProcessingRecoveryService.reprocessObjectNow` on the enrichment worker so a bulk rerun across many items doesn't block the request; used by `POST /api/media/bulk/thumbnail/rerun`); `thumbnail_repair` is a global hourly job (`mediaItemId: null`, `circleId: null`, priority 100, dedup'd on pending/running) that self-heals media items whose thumbnail sync was missed even though the underlying `StorageObject` reached `status IN ('ready','failed')` — see the `thumbnail_repair` paragraph under Admin: Stuck StorageObject Recovery above for the two repair paths and attempts-cap detail. `face_auto_archive_sweep` is a global per-circle backfill job (`mediaItemId: null`, `circleId` set, priority 100, `skipDedup: true` since the type is shared across many circles — per-circle pending/running dedup is enforced by `FaceBackfillService` itself) that hides LIVE unassigned faces matching a circle's already-archived unassigned-face pool; **server-only, not node-claimable** (no `nodeResultSchema`/`persistNodeResult`; deliberately absent from the CLI's `NODE_JOB_TYPES`), mirroring the `location_inference` sweep precedent — see the auto-archive feature detail under `faces` table and Feature Toggles below. `picture_enhancement` is a per-item job (`media_item_id` set, priority 0, reason `rerun`) that downloads the original, sharp auto-orients it (deterministic pre-pass), calls the configured OpenAI model via `enhanceImage()` (`images.edit` with `input_fidelity`), reads the actual output dims via sharp, runs the `ExifCarryoverService` EXIF-carryover + AI-marker stamp on the staged bytes (gated on `pictureEnhancement.stampExif`, default `true`; strictly best-effort), uploads the result to a staging object, and records dims/size → `ready`/`failed` on the corresponding `media_enhancements` row; **server-only, not node-claimable** (no `nodeResultSchema`/`persistNodeResult`, absent from the CLI's `NODE_JOB_TYPES`), same precedent as `location_inference`/`face_auto_archive_sweep`; `enhanceImage()` preserves the provider's HTTP `status`/`code`/`retry-after` on the rethrown error, so an actual OpenAI 429/529 during enhancement IS classified as a rate limit and routed through rate-limit deferral rather than normal-failure retry/backoff — and the handler's catch is a state machine (roll the row back to `pending` and rethrow while the queue still has retry/deferral budget, only mark it `failed` on the terminal attempt) so a queue-driven retry is never a silent no-op against an already-tombstoned row. `picture_enhancement_purge` is a global hourly job (`mediaItemId: null`, `circleId: null`) enqueued by `PictureEnhancementPurgeTask` that deletes staging objects and marks `ready`/`failed` `media_enhancements` rows `expired` once they exceed `pictureEnhancement.retentionHours`, mirroring `trash_purge`/`TrashPurgeTask` — see the [AI Picture Enhancer spec](docs/specs/picture-enhancer.md). Four job types back Media Workflow Automation (epic #138): `workflow_evaluate` is a server-only, one-per-run job (`circleId` set, `mediaItemId: null`, priority 20, payload `{runId, maxItems?}`) that compiles the run's frozen `definition_snapshot` and streams matching item ids into `workflow_run_items` via constant-memory keyset pagination (1,000-row pages, `(capturedAt DESC NULLS LAST, id DESC)` ordering), capped by `workflows.maxItemsPerRun`, then transitions the run to `completed` (0 matches), straight to `running` (approval-bypass eligible), or `awaiting_approval`; `workflow_evaluate_item` is a server-only, per-item job (priority 50, payload `{workflowId, mediaItemId}`, `skipDedup: true`) enqueued only once an item's enrichment dependencies the workflow's conditions actually read have all settled (dependency-aware, not a fixed delay) — a match is appended to (or opens) a rolling 5-minute "micro-run" per workflow so a bulk import produces a handful of `on_media_enriched` runs, not one run per photo; `workflow_execute_batch` (**node-claimable**, epic #138 Phase 6/#144) chunks a run's matched items into `workflows.batchSize`-sized jobs (priority 100, `skipDedup: true`, payload `{runId, itemIds}`) that apply the run's ordered action list per item with an idempotent claim + drift re-validation (an item may no longer satisfy the conditions by the time its batch runs) before acting — a node's compute is a pure declared-intent pass with no DB access; `persistNodeResult` re-derives and re-applies everything authoritatively server-side from the trusted `job.payload`, ignoring the node's submitted result as a source of truth; `workflow_history_purge` is a global nightly job (`mediaItemId: null`, `circleId: null`, priority 100) that expires stale `awaiting_approval` runs past `workflows.previewTtlHours` and batch-deletes (5,000-row batches) terminal runs past `workflows.runHistoryRetentionDays`, mirroring `job_history_purge`. Only `workflow_evaluate`, `workflow_evaluate_item`, and `workflow_history_purge` are server-only (pure SQL sweeps with no per-item unit of work for a node, same precedent as the `location_inference` sweep); `workflow_execute_batch` is genuinely node-eligible but is ALSO kept in the `ENRICHMENT_WORKER_MODE=system` claim set (alongside `thumbnail_repair`) so a fleet-only or `system`-mode deployment still executes workflows without requiring a registered node — see the [Workflow Automation spec](docs/specs/workflows.md). Two job types back Empty Trash at Scale (issue #165), rebuilt on the same evaluate/execute-batch pattern as the workflow job types above but stripped down to a single fixed action (hard-delete), no conditions, no approval gate: `trash_empty_evaluate` is a per-run job (`circleId` set, `mediaItemId: null`, priority 20, payload `{runId}`) that keyset-paginates every trashed item in the circle (`deletedAt IS NOT NULL`, same `(capturedAt DESC NULLS LAST, id DESC)` ordering, 1,000-row pages) into `trash_empty_run_items` at status `matched`, sets `matchedCount`, then transitions the run straight to `completed` (0 matches) or to `running` plus fans out `trash_empty_execute_batch` jobs; `trash_empty_execute_batch` (priority 100, payload `{runId, itemIds[]}`, `skipDedup: true`) claims a 200-item chunk via one atomic `updateMany` (still-`matched` → `deleted`, retry-safe), hard-deletes the claimed items via the new batched `MediaService.purgeMediaItemsBatched`, increments the run's atomic counters, and race-safely finalizes the run (`completed` or `completed_with_errors`) via a conditional `updateMany` on `status='running'` so only the last batch to drain wins. Both types are **server-only, not node-claimable** (no `nodeResultSchema`/`persistNodeResult` — purging needs storage credentials a node never holds), same precedent as `location_inference`/`face_auto_archive_sweep`/`trash_purge`, and are auto-eligible for `ENRICHMENT_WORKER_MODE=system` by the same no-node-pair inference as those types — see [Archive & Trash spec §10](docs/specs/archive-trash.md#10-empty-trash-at-scale-async-run-model). Issue #125 originally added a dedicated `location_suggestion_run_evaluate`/`location_suggestion_run_execute_batch` pair for Location Suggestion Bulk Accept/Reject at Scale, built on this same evaluate/execute-batch pattern; **issue #190 generalized that exact pair into a shared engine used by all three review queues** — bursts, duplicates, AND location suggestions — see the [Review Runs spec](docs/specs/review-runs.md) for the full architecture. `review_run_evaluate` is a per-run job (`circleId` set, `mediaItemId: null`, priority 20, payload `{runId}`) that keyset-pages (1,000 rows/page) a per-subject strategy's eligible-subject query — burst/duplicate groups filtered `status='pending' AND confidence [gte|lt] threshold/100` (null-confidence excluded both directions), location suggestions filtered `status='pending' AND confidence [gte|lt] threshold/100` (confidence non-nullable there, so no null exclusion) — into `review_run_items` at status `matched` via `createMany({skipDuplicates:true})` against the table's per-subject partial unique indexes, sets `matchedCount`, then transitions the run straight to `completed` (0 matches) or to `running` plus fans out `review_run_execute_batch` jobs. `review_run_execute_batch` (priority 100, payload `{runId, subjectIds[]}`, `skipDedup: true`) claims a 200-item chunk via one atomic `updateMany` (still-`matched` → `processing`, a transient claim marker since applying most of these actions does not delete the run-item's underlying row), then per subject delegates to the strategy (`BurstService`/`DuplicateService`'s now-public `resolveOne*`/`dismissOne*` methods for groups; the accept/reject transaction, unchanged, for location suggestions — writing `takenLat`/`takenLng`+`coordSource='inferred'` and enqueueing a deferred, post-commit `geocode` job on accept, no coordinate write on reject), transitioning each run-item `matched`→`processing`→a terminal `applied`/`skipped`/`failed`; atomic counter increments and a race-safe finalize (`completed`/`completed_with_errors`) mirror the trash-empty handler. Because a matched subject can be cascade-deleted mid-run (its `review_run_items` row cascades away with it via the exclusive-arc FKs), `processedCount` may legitimately finish below `matchedCount` — finalize keys off "no rows remain matched/processing", never counter equality. `review_run_history_purge` is a global nightly job (`mediaItemId: null`, `circleId: null`, priority 100) that, in one pass, marks a non-terminal run stuck for >24h `failed` (unblocking the per-`(circle,subjectType)` 409 concurrency guard) and batch-deletes (5,000-row batches) terminal runs older than `reviewRuns.runHistoryRetentionDays` — across **both** `review_runs` AND `trash_empty_runs` (the latter had no purge at all before this). `duplicate_confidence_backfill` is a global job (`mediaItemId: null`, `circleId: null`, priority 100, payload `{limit?}`) that keyset-backfills the newly-persisted `duplicate_groups.confidence` column (see the `duplicate_groups` table entry below) for pre-existing `NULL` rows. All four types are **server-only, not node-claimable** (no `nodeResultSchema`/`persistNodeResult`), auto-eligible for `ENRICHMENT_WORKER_MODE=system`, and none is in the CLI's `NODE_JOB_TYPES`. The old `location_suggestion_run_evaluate`/`location_suggestion_run_execute_batch` types still exist in the codebase as thin deprecated shim handlers that delegate straight into the generic pair above — kept only so a job already enqueued under the old type at deploy time still executes — but nothing enqueues new jobs under them. Three columns support distributed worker nodes (see the [Distributed Nodes spec](docs/specs/distributed-nodes.md)): `claimed_by_node_id` (FK → `worker_nodes`, SetNull on delete), `lease_expires_at` (DateTime?, set at claim time), and `executor` (`'server'`|`'node'`, which side ran the job). The in-process worker now claims through a shared, DB-atomic `EnrichmentClaimService` using `FOR UPDATE SKIP LOCKED`, making claiming safe across multiple API/worker processes as well as node claims via `POST /api/nodes/:id/claim`; a lease-expiry reaper requeues jobs whose `lease_expires_at` has passed without a renewal or completion
...
- `duplicate_groups` - Circle-scoped near-duplicate review groups; one row per detected visual-duplicate cluster; status `pending` | `resolved` | `dismissed`; `suggestedBestItemId` FK → `media_items` (SetNull on delete), opportunistically refreshed at read time by the best-copy scoring formula; `mediaCount` denormalized member count (always ≥ 2 — no minimum-size gate the way `burst_groups` has); `capturedAt` of the earliest active member for chronological queue sorting (added in migration `20260702010000_duplicate_groups_captured_at`); `resolvedById` / `resolvedAt` track who resolved or dismissed the group; `resolutionAction` (TEXT?, `'archive'` | `'trash'`), `keptCount` (Int?), `removedCount` (Int?) record the outcome of a resolve; `confidence` (Float?, migration `20260726000000_duplicate_group_confidence`, issue #190) — the tightest-pair CLIP cosine similarity, mirroring `burst_groups.confidence`; **now persisted** (previously computed only at read time), single writer `DuplicateDetectionService.recomputeGroupMeta` (invoked on every membership mutation), read-time self-heal backstop for pre-existing rows, one-pass `duplicate_confidence_backfill` job for historical `NULL` rows nobody reads; `null` when fewer than two members carry a `media_visual_embedding` row; new index `(circle_id, status, confidence)` serves the review-run threshold filter — see [Review Runs spec §4](docs/specs/review-runs.md#4-duplicate-group-confidence-becomes-persisted); indexes `(circle_id, status, resolution_action)` and `(circle_id, status, confidence)` both added in their respective migrations; added in migration `20260702000000_add_visual_embeddings_and_duplicate_groups`. **Burst membership wins over duplicate membership:** an item assigned to a `burst_group` is evicted from any `duplicate_group` it was already placed in (`DuplicateDetectionService.evictFromDuplicateGroups`, called from `BurstDetectionService.processMediaItem`), closing the upload-time race where dedup processes an item before burst detection does; see [duplicate-detection.md §3.2](docs/specs/duplicate-detection.md#32-burst-overlap-exclusion-rules)
...
- `review_runs` - Shared async run engine backing the bulk review actions for **three** review queues — bursts, duplicates, and location suggestions (issue #190; migration `20260726010000_review_runs`); one row per bulk action started against one queue in one circle. `subjectType` (`ReviewRunSubject` enum: `burst_group` | `duplicate_group` | `location_suggestion`), `action` (`ReviewRunAction` enum: `resolve_archive` | `resolve_trash` | `dismiss` | `accept` | `reject`), `threshold` (Int, 0–100 snapshot of the confidence floor used at evaluation time), `status` (`ReviewRunStatus` enum: `evaluating` | `running` | `completed` | `completed_with_errors` | `failed` | `cancelled`), the five counters (`matched_count`/`processed_count`/`succeeded_count`/`failed_count`/`skipped_count`), `started_by_id` (FK → `users`, SetNull), `last_error`, timestamps; `@@unique([id, subjectType])` is the FK target for `review_run_items`' composite type-agreement constraint below; indexes `(circle_id, subject_type, status)` (per-`(circle, queue)` 409 concurrency guard — a burst run and a location-suggestion run for the same circle may run concurrently, but two burst runs may not) and `(status, updated_at)` (retention + stale-run sweep). This table **replaces** the earlier feature-specific `location_suggestion_runs` (issue #125) — the migration copies that table's data forward with run UUIDs preserved, then drops it; see the [Review Runs spec](docs/specs/review-runs.md) for the full architecture, the exclusive-arc constraint rationale, and the data-model alternatives considered
- `review_run_items` - One row per subject (burst group / duplicate group / location suggestion) matched by a `ReviewRun`; `subject_type` denormalized from the parent run; **exclusive arc** — exactly one of `burst_group_id` / `duplicate_group_id` / `location_suggestion_id` is populated, matching `subject_type` (same XOR pattern as `media_shares.targetType` + `mediaItemId`/`albumId`); each FK cascades on delete from its subject table, so a subject deleted mid-run takes its item row with it (`processedCount` may legitimately finish below `matchedCount` as a result — finalize keys off "no rows remain matched/processing", not counter equality); a composite FK `(run_id, subject_type) → review_runs(id, subject_type)` prevents an item from ever belonging to a run of the wrong subject type; `status` (`ReviewRunItemStatus` enum: `matched` | `processing` | `applied` | `failed` | `skipped`); `error`; three partial unique indexes `(run_id, burst_group_id) WHERE burst_group_id IS NOT NULL` (and the equivalent for the other two columns) serve as the per-subject idempotency anchor for batch retries; index `(run_id, status)`. The CHECK constraints and partial unique indexes are raw-SQL-only schema drift (same precedent as `media_items_gallery_idx`), documented in the [Review Runs spec §3](docs/specs/review-runs.md#3-data-model)
- `worker_nodes` - Registers CLI-driven machines that claim and process `enrichment_jobs` rows; columns: `name`, `hostname`, `platform`, `cliVersion`, `eligibleTypes` (text[]), `concurrency`, `status` (`NodeStatus` enum: `online` | `draining` | `offline` | `disabled`), `capabilities` (JSONB, latest `node doctor` summary), `registeredAt`, `lastHeartbeatAt`, `createdById` (FK → `users`); `@@unique([createdById, name])` backs the register-or-reattach contract on `POST /api/nodes/register` (re-registering under an owned name updates the existing row instead of creating a duplicate); a daily `NodeOfflinePruneTask` deletes rows offline past `NODE_OFFLINE_RETENTION_DAYS` with no jobs currently claimed and running; see the [Distributed Nodes spec](docs/specs/distributed-nodes.md)
- `node_credentials` - Durable, `nod_`-prefixed bearer credentials that authenticate like a PAT but are accepted ONLY on `/api/nodes/*` routes (least-privilege — can never reach media/settings/admin endpoints, even for an Admin owner); stores a sha256 hash of the token (plaintext shown once at creation, never stored or returned again); `expiresAt` nullable (may never expire); individually revocable; managed via `POST`/`GET /api/node-credentials`, `DELETE /api/node-credentials/:id` (owner) and `GET`/`DELETE /api/admin/nodes/credentials[/:id]` (Admin); gated by the owner's existing `jobs:write` — no new permission
- `media_enhancements` - One row per AI Picture Enhancer run requested against a source `MediaItem`; enum `MediaEnhancementStatus` (`pending` | `processing` | `ready` | `failed` | `applied` | `discarded` | `expired`), enum `MediaEnhancementDecision` (`keep_both` | `replace`, null until applied); dual named relations to `MediaItem` — `MediaEnhancementSource` (`media_item_id`, cascade delete) and `MediaEnhancementResult` (`result_media_item_id`, SetNull) for the new item created on `keep_both`; `circle_id` cascade delete; `params` (JSONB, the request params), `provider`/`model`/`prompt` (audit of what was sent); staging pointer `staging_storage_key`/`staging_provider`/`staging_bucket` (null after apply/discard) holding the enhanced preview bytes until the user decides; `original_width`/`original_height`/`enhanced_width`/`enhanced_height` (drives the downscale-warning UI) and `enhanced_size` (BigInt, serialized as a STRING in API responses per the BigInt-serialization gotcha below); `last_error`; `created_by_id` (FK → `users`, SetNull); indexes `(media_item_id, status)`, `(circle_id, status)`, `(status, updated_at)` (for the retention sweep). Migration `20260716010000_add_media_enhancements`; see the [AI Picture Enhancer spec](docs/specs/picture-enhancer.md)
- `workflows` - Circle-scoped Media Workflow Automation definitions (epic #138); `subject_type` (`WorkflowSubject` enum, v1: `media_item` only — the extension point for future Subjects), `enabled` (disabled workflows never trigger), `trigger` (`WorkflowTrigger` enum: `manual` | `on_media_enriched` | `scheduled`), `cron_expression` (required iff `trigger='scheduled'`, validated 5-field cron with a minimum-interval floor), `next_run_at` (maintained by the scheduler cron), `definition` (JSONB, versioned + Subject-tagged condition/action document), `created_by_id` (FK → `users`, SetNull — authorizes unattended runs); indexes `(circle_id, enabled)` and `(trigger, enabled, next_run_at)`; see the [Workflow Automation spec](docs/specs/workflows.md)
- `workflow_runs` - One row per execution attempt of a `Workflow`; `circle_id` denormalized from the parent workflow; `status` (`WorkflowRunStatus` enum: `evaluating` | `awaiting_approval` | `running` | `completed` | `completed_with_errors` | `failed` | `cancelled` | `expired`); `trigger_type` (how THIS run started, may differ from the workflow's current trigger); `definition_snapshot` (JSONB, frozen copy of `Workflow.definition` at run-start — later edits never change an in-flight or historical run); `matched_count`/`truncated`/`processed_count`/`succeeded_count`/`failed_count`/`skipped_count`; `started_by_id`/`approved_by_id` (FK → `users`, SetNull) — `approved_at` is repurposed as the dispatch marker for `on_media_enriched` rolling micro-runs (null = still collecting); indexes `(workflow_id, created_at)`, `(circle_id, status)`, `(status, updated_at)` (the app-wide concurrency-gate count + retention/stuck-run sweeps)
- `workflow_run_items` - Per-media-item outcome row within a `WorkflowRun`; `status` (`WorkflowRunItemStatus` enum: `matched` | `excluded` | `applied` | `partially_applied` | `failed` | `skipped`); `action_results` (JSONB?, per-action outcome array); `error`; `@@unique([runId, mediaItemId])` is the idempotency anchor for batch retries (mirrors `storage_migration_items`'s `@@unique([runId, objectId])`); index `(run_id, status)`. The column is `media_item_id` because v1's only Subject is the media item — a future multi-Subject version would generalize this to a subject-id/type pair, documented but not built (see the spec's forward-compat contract)
- `trash_empty_runs` - One row per "Empty Trash" run for a circle (issue #165); a strict simplification of `WorkflowRun` — no conditions, no action list, no approval gate, since every matched (trashed) item is simply hard-deleted. `status` (`TrashEmptyRunStatus` enum: `evaluating` | `running` | `completed` | `completed_with_errors` | `failed` | `cancelled`); `matchedCount` (Int, trashed items discovered when the run was evaluated), `processedCount`/`succeededCount`/`failedCount`/`skippedCount` (Int, updated atomically as `trash_empty_execute_batch` jobs work through the matched set); `startedById` (FK → `users`, SetNull); `lastError`; `startedAt`/`finishedAt`; indexes `(circle_id, status)` (serves the per-circle concurrency guard — only one `evaluating`/`running` run per circle at a time, 409 otherwise) and `(status, updated_at)`; migration `20260724000000_trash_empty_runs`; see [Archive & Trash spec §10](docs/specs/archive-trash.md#10-empty-trash-at-scale-async-run-model)
- `trash_empty_run_items` - Per-media-item outcome row within a `TrashEmptyRun`; `status` (`TrashEmptyRunItemStatus` enum: `matched` | `deleted` | `failed` | `skipped`); `error`; `@@unique([runId, mediaItemId])` is the idempotency anchor for batch retries (same pattern as `workflow_run_items`/`storage_migration_items`); `mediaItemId` FK is `onDelete: Cascade` — a successful purge deletes the `MediaItem` row and cascades this row away with it; index `(run_id, status)`; migration `20260724000000_trash_empty_runs`
- `notifications` - Backing table for the in-app notification center (epic #240; schema in #244, service + API in #245, review-queue producer in #246, event producers in #247). The retention purge (#248 — the `notification_purge` nightly job, which is what two of the indexes below exist to serve) has since landed; the UI (#249/#250) lands later. `userId` (FK → `users`, cascade delete); `circleId` (FK → `circles`, nullable, cascade delete — null means a system/global notification); `type` (`NotificationType` enum: `review_queue_bursts` | `review_queue_duplicates` | `review_queue_location_suggestions` | `review_queue_enhancements` | `upload_completed` | `enrichment_failed` | `workflow_run_completed` | `share_expiring`); `title` (text); `body` (text, nullable); `link` (text, nullable, e.g. `/duplicates` — an in-app route to act on); `data` (JSONB, nullable, type-specific payload such as counts/ids); `readAt` (nullable, null = unread); `dismissedAt` (nullable, null = not dismissed); `createdAt`/`updatedAt`. Indexes: `(user_id, read_at)` (unread-count query and unread filter), `(user_id, created_at DESC)` (paginated list), `(user_id, dismissed_at)` (dismissed filtering), `(dismissed_at, updated_at)` and `(read_at)` (both serving the #248 retention purge). **Dedup is the load-bearing constraint of this table:** the four `review_queue_*` types are STATE notifications — at most one LIVE row per `(user_id, circle_id, type)`, refreshed rather than appended, so a circle with 929 pending duplicate groups produces exactly ONE row, never 929; the other four types (`upload_completed`, `enrichment_failed`, `workflow_run_completed`, `share_expiring`) are per-occurrence EVENT notifications appended freely. Enforced by a raw-SQL partial unique index, `notifications_review_queue_live_uniq_idx`, on `(user_id, circle_id, type)` WHERE `dismissed_at IS NULL AND circle_id IS NOT NULL AND type IN (the four review_queue_* values)` — the `circle_id IS NOT NULL` clause exists because Postgres unique indexes treat every NULL as distinct from every other NULL, which would otherwise silently defeat uniqueness across two same-type global notifications for one user (moot in practice, since a `review_queue_*` row's `circleId` is always non-null, but the predicate makes that invariant explicit). This index is raw-SQL-only, not representable in Prisma's schema DSL — the same precedent as `media_items_gallery_idx`, `media_items_map_locations_idx`, `people_circle_id_hidden_at_idx`, and the `review_run_items` partial uniques; documented as intentional schema drift in a comment above the `Notification` model in `schema.prisma`. Migration `20260805000000_add_notifications`; see `docs/specs/notifications.md` (not yet written) for the full notification-center feature this schema is the foundation for.

**Note:** `media_items` has `description` (nullable, max 8 192 chars) written by the auto-tagging handler on each successful vision call. There is no `title` column. `media_items`, `albums`, and `tags` use `added_by_id` (not `owner_id`) to track the uploading user. Dedup uniqueness for `media_items` is `(circle_id, content_hash)`. Tag names are unique per `(circle_id, name)`. The `media_tags` join table has a `source` column (`MediaTagSource` enum: `manual` | `ai` | `system`, default `manual`) that tracks whether a tag was applied by a user manually, by the AI auto-tagging service, or by an automated system feature (currently only social-media video detection's "Social Media"/platform tags); AI re-runs are authoritative over `source='ai'` rows only and never modify `source='manual'` rows; `system` tags follow a similar one-directional protection rule — an existing `source='ai'` row for the same tag/item is promoted to `system` (never the reverse), and `source='manual'` rows are never overwritten or deleted by the system writer. The `system` value was added in its own migration (`20260705000000_add_media_tag_source_system`) because Postgres cannot add an enum value in the same transaction as statements referencing it. `media_items` also carries burst detection columns: `perceptual_hash` (**TEXT?**, unsigned 64-bit dHash stored as a decimal string — see storage rationale below), `sharpness_score` (Float?, variance-of-Laplacian sharpness measure), `burst_uuid` (String?, Apple BurstUUID from EXIF MakerNote — null for non-Apple cameras), `burst_score` (Float?, composite quality score within the group — null when not in a group), and `burst_group_id` (FK → `burst_groups`, SetNull on delete). `media_items` also carries `duplicate_group_id` (FK → `duplicate_groups`, SetNull on delete) for near-duplicate detection, plus a `(circle_id, perceptual_hash)` index added specifically to serve the duplicate-detection hash-candidate scan. The `perceptual_hash` column is omitted from default API responses via a Prisma global `omit` because it is an internal computation value; the burst matcher (whose Hamming-distance helper is shared with duplicate detection) parses it with `BigInt(string)` only when computing Hamming distance. **Why TEXT and not `bigint`:** Postgres `bigint` is a signed 64-bit integer (max 2^63-1); a dHash is an unsigned 64-bit value and hashes with the high bit set overflow, producing a "value out of range for type bigint" error. Additionally, Prisma maps `BigInt` to JavaScript's `BigInt` primitive, which throws "Do not know how to serialize a BigInt" on `JSON.stringify`, crashing any endpoint that returns the column. Storing the value as a decimal string avoids both problems. `media_items` also carries the archive column: `archived_at` (DateTime?, nullable; null = active/not-archived; non-null = archived). Trash reuses the existing `deleted_at` column (soft-delete). The two states are independent: `deleted_at` for Trash, `archived_at` for Archive. `media_items` also carries `coord_source` (TEXT?, `'exif'` | `'manual'` | `'inferred'`; null = no coordinates) — provenance of `taken_lat`/`taken_lng`, distinct from `geo_source` (which tracks the reverse-geocode *provider/trigger* and is overwritten on every geocode run; a `geocode` job overwriting `geo_source` after a location-inference auto-apply is expected and does not touch `coord_source`). Three writers: EXIF metadata sync (`MediaMetadataSyncService`, present-only — sets `'exif'` only when EXIF actually supplied latitude in that run), `bulkUpdateMedia`'s manual location edit (`'manual'`), and location inference (`'inferred'` for auto-apply/unmodified-accept, `'manual'` for an adjusted accept) — see the [Location Inference spec](docs/specs/location-inference.md). `GEO_CLEAR_COLUMNS` (used whenever a location is cleared) also nulls `coord_source`. `media_items` also carries `social_media_source` (TEXT?, `'tiktok'` | `'instagram'` | `'facebook'` | `'other'`; null = clean/unknown) — written only by the `social_media_detection` enrichment handler and used to gate video enrichment: `VideoFaceDetectionHandler` skips face detection entirely (marking `no_faces`) when this column is non-null, and `FaceBackfillService` excludes flagged videos from its eligibility query. Detaching the "Social Media" tag manually does NOT clear this column — only a detection rerun does; see the [Social Media Detection spec](docs/specs/social-media-detection.md). `media_items` also has a hand-authored, raw-SQL-only partial composite index `media_items_map_locations_idx` (migration `20260713130000_media_locations_index`, not representable in Prisma's schema DSL) serving the `GET /api/media/locations/aggregate` map clustering query. A further hand-authored, raw-SQL-only partial composite index `media_items_gallery_idx` on `media_items (circle_id, captured_at DESC, id DESC) WHERE deleted_at IS NULL AND archived_at IS NULL` (migration `20260716000000_media_gallery_index`, issue #104) serves the default keyset-paginated `GET /api/media` gallery query as a pure index scan with no separate sort node — same schema-drift precedent as the other two hand-authored indexes above.
...
- `ENRICHMENT_WORKER_MODE` - Controls what the API's own in-process worker claims: `all` (default; runs every job type — the single-machine/dev posture), `system` (runs ONLY the server-only job types — `burst_detection`, `duplicate_detection_batch`, `job_history_purge`, `face_auto_archive_sweep`, `storage_insights`, `location_inference`, `trash_purge`, `storage_migration`, `thumbnail_repair`, `review_run_evaluate`, `review_run_execute_batch`, `review_run_history_purge`, `duplicate_confidence_backfill` — while all per-item media compute — `face_detection`, `video_face_detection`, `auto_tagging`, `geocode`, `duplicate_detection`, `metadata_extraction`, `social_media_detection`, `thumbnail_regen` — goes to the worker-container fleet; the recommended posture once a fleet is running), or `off` (no in-process worker at all — every claimable job type must be served by nodes). Legacy `ENRICHMENT_WORKER_ENABLED` is still honored as a fallback (`false` → `off`)
- `ENRICHMENT_REAPER_ENABLED` - The lease-expiry reaper (`EnrichmentStuckResetTask`) is decoupled from `ENRICHMENT_WORKER_MODE` and always runs, including under `off` — an external node fleet is exactly where claimed jobs die mid-flight and need requeuing regardless of whether the API also runs its own worker. Set `false` to opt out (default: `true`)
- `NODE_OFFLINE_RETENTION_DAYS` - A daily `NodeOfflinePruneTask` deletes `worker_nodes` rows that are both offline (stale `lastHeartbeatAt`) past this many days AND have no jobs currently claimed and running (default: `14`)

**Worker node CLI (headless/container config, `apps/cli`):** `loadConfig()` overlays these env vars over `~/.memoriahub/config.json` (env wins), and synthesizes a config from env alone when no file exists — what lets a worker container run with zero interactive setup:
- `MEMORIAHUB_URL` / `MEMORIAHUB_TOKEN` - API base URL and bearer credential (a `nod_` node credential or a PAT — see the Node Data-Plane section above)
- `MEMORIAHUB_NODE_ID` / `MEMORIAHUB_NODE_NAME` - Existing node ID to resume, or a display name for a new/reattaching registration (must be unique per host/replica in a fleet — see the [Distributed Nodes spec](docs/specs/distributed-nodes.md))
- `MEMORIAHUB_CONCURRENCY` / `MEMORIAHUB_ELIGIBLE_TYPES` (csv) / `MEMORIAHUB_POLL_INTERVAL_MS` - Worker slot count, advertised job types, and claim-loop poll interval; `MEMORIAHUB_CONCURRENCY`'s default is now core/RAM-aware (2–4 on a capable host) instead of a flat 1, when unset
- `MEMORIAHUB_FACE_PROVIDER` (`compreface`, the only accepted value; default `compreface`) / `MEMORIAHUB_COMPREFACE_URL` - Face-detection provider selection and sidecar URL; any other value is a startup error, not a silent fallback
- `MEMORIAHUB_STATE_DIR` - Relocates the whole state directory (config, pidfile, socket, logs, models)
- `MEMORIAHUB_HEADLESS` - `1` implies `--headless` (drains in-flight jobs and exits WITHOUT deregistering on SIGTERM/SIGINT, auto-registers inline when no node ID is stored); the worker container image always runs headless regardless
- `MEMORIAHUB_MAX_OLD_SPACE_MB` / `MEMORIAHUB_HEAP_SNAPSHOT` / `MEMORIAHUB_SHARP_CONCURRENCY` - Memory/OOM hardening overrides (`apps/cli/src/node/runtime-tuning.ts`): computed `--max-old-space-size` heap ceiling in MB (`0` disables re-tuning), heap-snapshot-on-near-OOM diagnostic toggle (`0` suppresses), and per-pipeline libvips thread cap — see [Worker Container Configuration Reference §3.5](docs/worker-container-config.md#35-memory--oom-hardening)
- `MEMORIAHUB_MEMWATCH` - `0` disables the periodic memory-watchdog log sample (rss/heapUsed/heapTotal/external/arrayBuffers/heapLimit); default enabled — see §3.5
- `MEMORIAHUB_HEAP_RESTART_FRACTION` - heapUsed/heapLimit fraction (default `0.90`) that triggers the drain-and-exit-75 pre-OOM safety valve; `0` disables — see §3.5
- `MEMORIAHUB_CLIP_RECYCLE_AFTER` - embeds before the CLIP `duplicate_detection` onnxruntime `InferenceSession` is recycled (release + recreate) to bound native memory growth (default `1000`; `0` disables); the API's equivalent is `CLIP_SESSION_RECYCLE_AFTER`. `duplicate_detection` is the only onnxruntime path, so excluding it (or `features.duplicateDetection=off`) avoids it entirely — see issue #156
...
- **Temp-file janitor** (`TempFileJanitorTask`): on startup and every hour, sweeps `os.tmpdir()` for `memoriaHub-*` files older than 6h (mtime) and deletes them, cleaning orphans left behind when a job is SIGKILLed (e.g. OOM) mid-download before its `finally`-block cleanup runs. Best-effort per-file; only active on worker instances — it respects the same worker-disable state (`ENRICHMENT_WORKER_MODE=off`, or legacy `ENRICHMENT_WORKER_ENABLED=false`) as the worker, since only worker instances create these files.
...
- `MEMORIAHUB_MAX_OLD_SPACE_MB` / `MEMORIAHUB_HEAP_SNAPSHOT` / `MEMORIAHUB_SHARP_CONCURRENCY` — worker nodes now auto-harden memory for sustained multi-hour imports (raised V8 heap ceiling, capped libvips concurrency, core/RAM-aware `MEMORIAHUB_CONCURRENCY` default), assuming a 16 GB RAM / 8-core baseline host; override these when the auto-computed values don't fit your host, especially inside a memory-limited container (see [Worker Container Configuration Reference §3.5](docs/worker-container-config.md#35-memory--oom-hardening)).
- The worker now watches its own memory (`MEMORIAHUB_MEMWATCH`) and, before hitting OOM, drains and exits (`MEMORIAHUB_HEAP_RESTART_FRACTION`) so a bulk import self-recovers instead of crash-looping — requires a supervisor (container/systemd) to actually restart the process (see §3.5).
...
- `locationInference.bulkAcceptThreshold` — integer, 0–100, default 80; confidence floor (as a percent) for the Location Suggestions review page's bulk actions — "Accept all ≥ N%" starts an async run accepting pending suggestions with `confidence >= N/100`, "Reject all < N%" starts an async run rejecting pending suggestions with `confidence < N/100`; one threshold partitions the pending queue between the two bulk actions (issue #126)
...
- `PICTURE_ENHANCEMENT_ENABLED` - Environment kill-switch for the feature; set to `false` to disable regardless of system settings. The runtime toggle is `features.pictureEnhancement` in system settings; this env var is a hard override for CI/test environments (default: `true`)

The following picture-enhancer parameters are controlled via system settings (not environment variables), editable in the Admin UI under `pictureEnhancement.*`; see the [AI Picture Enhancer spec](docs/specs/picture-enhancer.md) for the full pipeline this configures:
- `pictureEnhancement.defaultQuality` — enum `low`|`medium`|`high`, default `high`; `gpt-image-1` output quality
- `pictureEnhancement.defaultStrength` — enum `subtle`|`balanced`|`strong`, default `balanced`; default correction aggressiveness, maps to prompt wording and `input_fidelity`
- `pictureEnhancement.stampExif` — boolean, default `true`; whether `ExifCarryoverService` copies the original's EXIF/GPS/IPTC/XMP/ICC onto the enhanced file and stamps a file-level "AI enhanced" marker (`Software`/`XMP-xmp:CreatorTool`/`XMP-iptcExt:DigitalSourceType`), in addition to the in-app marker (`metadata._aiEnhanced` + "AI Enhanced" system tag) which is always written regardless of this setting. Backed by `exiftool-vendored` (with `perl` in the API Dockerfile base stage); strictly best-effort — a missing install or unwritable tmpdir silently keeps the enhanced bytes unstamped rather than failing the job
- `pictureEnhancement.allowReplace` — boolean, default `true`; if `false`, only "keep both" is offered on apply — the original can never be overwritten
- `pictureEnhancement.blockReplaceOnDownscale` — boolean, default `false`; if `true`, disables "replace" when the enhanced image's dimensions are smaller than the original's (the UI still shows the downscale warning either way)
- `pictureEnhancement.maxInputMegapixels` — number, 1–100, default `50`; guards against absurdly large inputs
- `pictureEnhancement.retentionHours` — integer, 1–720, default `168` (7 days; raised from 72h in issue #201 — the Enhancements hub is an inbox users work through over time, not a use-immediately dialog, and 72h was reaping unreviewed, already-paid `gpt-image-1` calls); how long unapplied staging previews live before `PictureEnhancementPurgeTask`'s hourly `picture_enhancement_purge` job reaps them

**Media Workflow Automation:**

- `WORKFLOWS_ENABLED` - Environment kill-switch for the feature; set to `false` to disable regardless of system settings. The runtime toggle is `features.workflows` in system settings; this env var is a hard override for CI/test environments (default: `true`)

The following workflow parameters are controlled via system settings (not environment variables; the entire `workflows.*` schema ships with defaults so every layer of the feature reads it through the one cached `SystemSettingsService.getSettings()` call), editable in the Admin UI under `/admin/settings/workflows`; see the [Workflow Automation spec](docs/specs/workflows.md) for the full engine this configures:
- `workflows.maxItemsPerRun` — integer, 100–500,000, default `10000`; hard cap on media items materialized/evaluated per run; a run matching more sets `truncated=true`
- `workflows.batchSize` — integer, 50–1,000, default `200`; items per `workflow_execute_batch` job
- `workflows.maxConcurrentRuns` — integer, 1–10, default `2`; simultaneous non-terminal runs allowed APP-WIDE (not per-circle); excess manual `POST /api/workflows/:id/run` calls get a 409, scheduled-trigger starts are skipped and rolled forward
- `workflows.requirePreview` — boolean, default `true`; when true, every MANUAL run must pass approval (`awaiting_approval`) unless a per-workflow `options.requirePreview=false` override is also present; never applies to unattended (`scheduled`/`on_media_enriched`) runs, which always skip straight to execution
- `workflows.allowHardDelete` — boolean, default `false`; unlocks the `hard_delete` action app-wide; `media:delete` + manual-trigger-only + a typed `"DELETE {matchedCount}"` approval confirmation are still all separately required
- `workflows.maxWorkflowsPerCircle` — integer, 1–100, default `20`
- `workflows.previewTtlHours` — integer, 1–168, default `24`; an `awaiting_approval` run untouched past this long is expired by the nightly purge
- `workflows.runHistoryRetentionDays` — integer, 1–365, default `30`; retention for terminal runs/items before `workflow_history_purge` deletes them
- `workflows.triggers.onEnrichment` / `workflows.triggers.scheduled` — booleans, default `true`; master switches that let an admin turn off each unattended trigger type entirely without disabling individual workflows
- `workflows.scheduleMinIntervalMinutes` — integer, 60–10,080, default `60`; tightest cron cadence a `scheduled` workflow may use (validated by sampling ~20 upcoming fire times, catching dense comma-lists and step schedules, not just an obviously-tight `* * * * *`)
...
- `features.pictureEnhancement` — boolean, default false; global on/off for the AI Picture Enhancer; env `PICTURE_ENHANCEMENT_ENABLED=false` overrides this; see `ai.features.enhance` and `pictureEnhancement.*` below and the [AI Picture Enhancer spec](docs/specs/picture-enhancer.md)
- `features.workflows` — boolean, default false; global on/off for Media Workflow Automation; env `WORKFLOWS_ENABLED=false` overrides this; see `workflows.*` above and the [Workflow Automation spec](docs/specs/workflows.md)

Related, under the `ai.features.*` namespace (not `features.*`): `ai.features.enhance` — object `{ provider, model } | null`, default `null`; active provider/model for the AI Picture Enhancer (set via `PUT /api/ai/features/enhance`); currently OpenAI-only (`gpt-image-1`); required for `POST /api/media/:id/enhance` to succeed even when `features.pictureEnhancement` is on.
...
- `notifications.retentionDays` — integer, 1–365, default 30; editable in the System Settings admin page (Storage tab, Notifications section, alongside `jobs.history.retentionDays` and `storage.trash.retentionDays`). Controls how many days a **resolved** notification is kept before automatic deletion, measured from `dismissed_at` / `read_at` — **not** from `created_at`. **An UNREAD notification is never deleted by age**, which is the deliberate asymmetry with `jobs.history.retentionDays` (that one deletes purely by `finishedAt` age): an enrichment-job row is a machine artifact nobody is waiting on, whereas an unread notification is by definition something the user has not dealt with, and silently deleting it would defeat the feature. Unread rows growing without bound is prevented at the WRITE path instead — `upsertState()` keeps at most one live row per `(user, circle, review_queue_*)` and `upsertCountedEvent()` folds repeated occurrences into one counted row — so the producers cannot fan out.
- `notifications.purgeEnabled` — boolean, default true; editable in the same panel. Mirrors `jobs.history.purgeEnabled` as the forensic-investigation escape hatch: when false the nightly `NotificationPurgeTask` cron does not enqueue, and the handler also re-checks it so a manual retry from `/admin/jobs` is a no-op too. The nightly cron runs `@Cron(CronExpression.EVERY_DAY_AT_MIDNIGHT)`; when enabled and no `notification_purge` job is already pending or running, it enqueues a global `notification_purge` enrichment job at priority 100. The handler deletes in 5 000-row batches (lock-safe, two passes — dismissed then read — each a single-column range scan on the index added for it) and is visible in the `/admin/jobs` dashboard under `type='notification_purge'`.

**Review Run History Retention:**

- `reviewRuns.runHistoryRetentionDays` — integer, 1–365, default 30; validated by the same Zod schema as every other system-settings write path (round-tripped through `PATCH`/`PUT /api/system-settings`); as of this writing has no dedicated admin settings-page panel of its own (unlike `workflows.runHistoryRetentionDays`), so it is editable only via the generic system-settings JSON. Controls how many days of terminal `review_runs` rows (and, in the same nightly sweep, terminal `trash_empty_runs` rows — which had no retention purge before this setting existed) are retained before the nightly `review_run_history_purge` job hard-deletes them in 5 000-row batches. The same job also marks a non-terminal run stuck for more than a fixed 24 hours `failed`, unblocking the per-`(circle, subjectType)` 409 concurrency guard a permanently-dead run would otherwise leave locked. See the [Review Runs spec §10](docs/specs/review-runs.md#10-retention-and-stale-run-sweep).
...
- **HEIC/HEIF native decode (issue #106, #128):** sharp's *bundled* libvips ships without the HEVC/HEIC decoder (patent/licensing reasons), so a plain `sharp(buffer)` throws on iPhone HEIC photos. Both the API image (`apps/api/Dockerfile`) and the worker image (`apps/cli/Dockerfile`) now install a HEIF-enabled system libvips via apt (Debian trixie: `libvips42t64 libheif1 libde265-0 libheif-plugin-libde265`) and set `SHARP_FORCE_GLOBAL_LIBVIPS=1`, forcing sharp to build its native addon from source against that system libvips — so sharp decodes HEIC/HEIF **natively** on both executors. `transcodeToDecodableJpeg` (`packages/enrichment-compute/src/image/index.ts`, re-exported from `apps/api/src/storage/processing/image-orientation.util.ts`) stays wired as a safety-net FALLBACK — triggered only inside the sharp `catch` — in `prepareImageForProcessing`, `getOrientedDimensions`, and `ThumbnailProcessor.processImage`, now rarely hit; it ffmpeg-transcodes to a JPEG sharp can process, bounded by `FFMPEG_TIMEOUT_MS` (default 60000ms) and SIGKILL-guarded, writes `memoriaHub-heic-*` temp files (swept by `TempFileJanitorTask` on leak), and THROWS on genuinely-corrupt input so the caller's existing catch still fails that one item cleanly rather than crashing the worker. **Cross-executor parity:** since both images now build sharp against the identical apt libvips/libheif package set, HEIC decode is consistent across the server and worker node — this is why #128 switched both images together in one release rather than one at a time, which would have diverged HEIC-derived CLIP/dedup embeddings between executors. The golden-vector parity gate was extended with a committed HEIC fixture (`packages/enrichment-compute/test/fixtures/golden-fixture.heic`) and a `REQUIRE_HEIC_DECODE=1` marker enforced in-image for both the worker and API images in `.github/workflows/deploy.yml`.
...
- **Prisma's typed `where` cannot express a column-to-column comparison or column arithmetic.** A filter like "width > height" (orientation) or "width × height > threshold" (megapixels) has no representation in `Prisma.WhereInput` — there's no operator that references another column. The Workflow condition compiler's fix (`apps/api/src/workflows/registry/media-item-fields.ts`, `readTimeRefinement`) is the general pattern: `buildWhere` returns only the tightest *bounding* predicate it can express as real SQL (e.g. "both columns are non-null"), and an exact in-process `refinementPredicate` re-checks the real condition against the already-fetched row. This is only sound when applied post-filter on a pure-AND path (an OR-nested bounding predicate would silently under-match if narrowed further) — see the compiler's `pureAndPath` threading for how that soundness constraint is enforced. The same technique generalizes to any future feature needing a computed/derived-column filter Prisma can't compile directly.
...
- [Archive & Trash Bin](docs/specs/archive-trash.md) — two independent soft-state columns (`archivedAt` / `deletedAt`), search-inclusion asymmetry, automatic purge via `trash_purge` enrichment job, dedup-on-restore conflict handling, and the async run-based "Empty Trash at Scale" rebuild (issue #165): `trash_empty_runs`/`trash_empty_run_items`, the `trash_empty_evaluate`/`trash_empty_execute_batch` server-only job pair, the shared batched `purgeMediaItemsBatched` purge path, per-circle concurrency guard, and progress-polling run API/UI
...
- [AI Picture Enhancer](docs/specs/picture-enhancer.md) — photo-only, single-item, human-reviewed OpenAI `gpt-image-1` enhancement via a staged preview + keep-both/replace/discard review gate; enhancement presets and per-run quality override; `media_enhancements` data model, async `picture_enhancement`/`picture_enhancement_purge` enrichment job types (server-only, not node-claimable), destructive-replace precedent shared with orientation edit, settings/env configuration, Doctor `ai.pictureEnhancer` check, the least-privilege `GET /api/features` endpoint and `/admin/settings/enhancer` admin page; documents the v1.1 scope actually shipped vs. the original draft (file-level EXIF carryover + AI marker shipped via `ExifCarryoverService`/`exiftool-vendored`; `enhanceImage` rate-limit classification fixed; two-tier analyze-first and a first-class `aiEnhancedAt` column remain open)
...
- [Workflow Automation](docs/specs/workflows.md) — the Subject · Trigger · If · Then rule model and its per-Subject registry/forward-compat contract, the versioned + Subject-tagged definition schema, the Media Item condition catalog and compiler (incl. read-time-refinement gaps), the full action library (item-level + review-queue + the `move_to_circle` cross-circle cascade), the run-lifecycle state machine, the scale strategy for a 150k-item library, trigger semantics (manual/scheduled/on-media-enriched rolling micro-runs + loop protection), the safety model (preview/approval, hard-delete guardrails), the node compute/persist split, data model, API, RBAC, and settings reference
- [Review Runs](docs/specs/review-runs.md) — issue #190's shared async run model consolidating bursts/duplicates/location-suggestions bulk review actions onto one `review_runs`/`review_run_items` exclusive-arc data model (absorbing the former `location_suggestion_runs`), the full CHECK/FK constraint table and why each exists, the data-model alternatives considered and rejected, the persisted `duplicate_groups.confidence` column this unlocked, the four new server-only job types, the per-subject strategy split wrapping existing `BurstService`/`DuplicateService` primitives, the run lifecycle/cancellation/retention model, the shared `RunProgressPanel` frontend, API, and RBAC
- [DataTable](docs/specs/datatable.md) — epic #238's shared table foundation (`apps/web/src/components/datatable/`): the renderer-agnostic `DataTableColumn`/`DataTableProps` contract every future table is declared against, the `priority` pivot (`primary`/`secondary`/`detail`) that drives desktop column visibility AND the mobile card layout from one declaration, the `render`-vs-`value` split (visual cell vs. the scalar sorting/filtering/export operate on), the controlled server-side pagination/sorting contract (zero-based `page` vs. the one-based API), page-scoped selection semantics including the MUI X v9 `{type:'include'|'exclude'}` model adaptation, row/bulk action descriptors with the single shared confirm dialog, the non-negotiable horizontal-containment rule, the DataGrid-backed desktop renderer + `columnAdapter`, the renderer-switch seam for the mobile card list, and which contract fields are reserved for the not-yet-built issues (#253 mobile, #254 filtering, #255 visibility/views, #256 virtualization/export)

See also: [Worker Node Setup & Troubleshooting](docs/worker-node-setup.md) — a practical setup/troubleshooting companion (not an architecture spec) covering the one-command `node install-deps` automated installer, per-OS ffmpeg install, native dependency (sharp/onnxruntime/tesseract) installed-vs-operational troubleshooting, model manifest download, and common doctor error remediation.

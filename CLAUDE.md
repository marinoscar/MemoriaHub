...
- `GET /api/media/duplicates?circleId=&status=&kind=&page=&pageSize=&sortBy=&sortOrder=` - List duplicate groups (review queue), ordered chronologically by `capturedAt` by default; items `{ id, status, kind, mediaCount, suggestedBestItemId, capturedAt, confidence, coverThumbnailUrls[] }`; `confidence` is the tightest-pair CLIP cosine similarity (0–1) and is now **persisted** as of issue #190 (`duplicate_groups.confidence`, single writer `DuplicateDetectionService.recomputeGroupMeta`, read-time self-heal backstop for pre-existing rows — see the [Review Runs spec §4](docs/specs/review-runs.md#4-duplicate-group-confidence-becomes-persisted)); `sortBy` `capturedAt`(default)|`confidence`|`mediaCount`, `sortOrder` `asc`(default)|`desc`; the `confidence` sort (added by issue #189) is still applied **in memory** over the fully-materialized, kind-filtered set before pagination — a group with no computable similarity sorts last regardless of direction. Now that the column exists, that sort could become a DB `orderBy` like the other two; it was left in memory when #190 merged so the read path's behaviour stayed byte-identical, and converting it is a follow-up. `capturedAt`/`mediaCount` are a DB `orderBy` with a trailing `id` tiebreaker; response `{ items, meta:{total,page,pageSize} }` (media:read + viewer)

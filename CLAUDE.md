...
Notification Center (epic #240, issue #245) — the in-app bell. **No new RBAC permission**, deliberately: every route is already scoped to the JWT's `userId` and a notification is inherently personal, so there is nothing an Admin-gated permission would protect — the same least-privilege, authentication-only rationale as `GET /api/features`. Rows come in two flavors written through the SINGLE service write path `NotificationsService` (`apps/api/src/notifications/`): **EVENT** types (`upload_completed`, `enrichment_failed`, `workflow_run_completed`, `share_expiring`) appended once per occurrence via `emit()`, and **STATE** types (the four `review_queue_*`) upserted in place via `upsertState()` keyed on `(userId, circleId, type)` among LIVE (`dismissed_at IS NULL`) rows, so a circle with 929 pending duplicate groups produces exactly ONE row per user rather than 929 (enforced in the DB by the partial unique index `notifications_review_queue_live_uniq_idx`). Both producer methods are **best-effort / fire-and-forget**: a failed notification write logs a warning and never throws into or blocks the triggering action — the same contract as the transactional email sends. `NotificationsModule` exports `NotificationsService` for the producers in #246/#247.

...
- `GET /api/nodes/models/manifest` (jobs:read) - List enrichment model files/versions a node should have locally to serve its `eligibleTypes`, with a real sha256/byte-size value for the 1 file (CLIP ONNX); face detection needs no manifest entry — a node runs it entirely against the external `compreface-core` sidecar (issue #113 removed the earlier Human face-model files from this manifest)
...
CompreFace is the sole face-recognition provider (`compreface`, keyless `compreface-core` sidecar, 128-d mobilenet, `requiresCredentials:false`) — issue #113 removed the earlier `human` (in-process WASM, 1024-d) and `rekognition` (delegated AWS) providers, both on the server and on worker nodes. The Face Settings UI has a "Test connection" button. Face recognition is enabled globally via `features.faceRecognition` system setting (default off); the per-circle `face_recognition_enabled` column was dropped in migration `20260621050000_drop_circle_feature_flags`.
...
- `POST /api/admin/face/backfill` body `{ from?, to?, force? }` - Bulk-enqueue face_detection jobs across all circles; `from`/`to` (optional ISO-8601) bound `capturedAt` to control how far back detection is recreated; 400 if `features.faceRecognition` is disabled (Admin + face_settings:write). **Post-upgrade note (issue #113):** after standardizing on CompreFace, operators should re-run this with `force: true` to regenerate the ~125 legacy faces previously detected by the removed `human`/`rekognition` providers — those old rows are in a different, non-comparable embedding space and will not match against new CompreFace-detected faces until reprocessed.
...
- `face_provider_credentials` - Face provider API keys/config (AES-256-GCM encrypted via same key as AI); one row per provider; `last4` exposed; plaintext never stored or returned. `compreface` is the only (keyless) provider — the credential row, if present, stores only a `baseUrl` override, no API key is set or required; `region` remains in the schema as an unused legacy column from the removed AWS Rekognition provider (issue #113).
...
- `MEMORIAHUB_COMPREFACE_URL` - Base URL of the node's local `compreface-core` sidecar, the only face-detection provider a node uses (issue #113 removed the `human`/`rekognition` alternatives and the `MEMORIAHUB_FACE_PROVIDER` selection flag/env var)

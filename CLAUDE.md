...
- **HEIC/HEIF decode fallback (issue #106):** sharp's prebuilt libvips ships without the HEVC/HEIC decoder (patent/licensing reasons), so `sharp(buffer)` throws on iPhone HEIC photos. `transcodeToDecodableJpeg` (`packages/enrichment-compute/src/image/index.ts`, re-exported from `apps/api/src/storage/processing/image-orientation.util.ts`) is wired as a FALLBACK — triggered only inside the sharp `catch` — in `prepareImageForProcessing`, `getOrientedDimensions`, and the server `ThumbnailProcessor.processImage` (which calls sharp directly and so needed its own wiring). It ffmpeg-transcodes the input to a JPEG that sharp can then process normally; bounded by `FFMPEG_TIMEOUT_MS` (default 60000ms) and SIGKILL-guarded, writes `memoriaHub-heic-*` temp files (swept by `TempFileJanitorTask` on leak), and THROWS on genuinely-corrupt input so the caller's existing catch still fails that one item cleanly rather than crashing the worker. **Cross-executor parity note:** the server decodes HEIC via ffmpeg while the containerized worker node (epic #108) decodes via native libheif — the re-encoded pixels can differ slightly, so a HEIC item's dedup/CLIP embedding computed server-side vs. node-side may not be bit-identical. Acceptable for now (success, not cross-executor bit-parity, is the bar); a documented follow-up (rebuild `apps/api`'s image against a libheif-enabled libvips and set `SHARP_FORCE_GLOBAL_LIBVIPS=1`) would let `sharp` decode HEIC natively server-side and close the gap.

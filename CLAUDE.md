...
Photo-only, single-item, human-reviewed AI enhancement via OpenAI `images.edit` (`gpt-image-1`) — see [AI Picture Enhancer spec](docs/specs/picture-enhancer.md). The AI output regenerates rather than retouches the image (potential fidelity risk on faces/text/detail) and is capped at `1024×1024`/`1024×1536`/`1536×1024`, so it is frequently lower-resolution than the original; nothing is ever auto-applied. The enhanced bytes are written to a staging object first — the user must explicitly review a side-by-side compare and choose **keep both** (creates a new `MediaItem`, "AI Enhanced" system tag + `metadata._aiEnhanced` breadcrumb, original untouched) or **replace** (destructively overwrites the original storage key, mirroring orientation edit: nulls `contentHash` to force recompute, `reprocessObjectNow`, best-effort re-enqueues `face_detection`), or **discard**. Only one live (`pending`/`processing`/`ready`) enhancement per item — a new request supersedes and discards any prior unresolved staging. Enabled globally via `features.pictureEnhancement` (default off); requires `ai.features.enhance` (provider+model) to be configured. **v1 EXIF limitation:** the enhanced file itself does NOT carry an AI-enhanced EXIF/XMP marker — only the in-app marker (`metadata._aiEnhanced` + system tag) is written; `pictureEnhancement.stampExif` defaults to `false` and currently has no effect (the file-level EXIF writer was deferred — see the spec's Implementation status note).
- `POST /api/media/:id/enhance` body (all optional — §4.1 of the spec: `intent`, `adjustments.*`, `strength`, `preserveFaces`, `instructions`, `model`) - Start an enhancement; photo-only guard (400 for video/non-image); creates a `media_enhancements` row (`pending`) and enqueues a `picture_enhancement` job at priority 0; supersedes any existing live enhancement for the item; returns `202` `{ data: { enhancementId, jobId, status: 'pending' } }`; 400 if feature disabled / not a photo / no `ai.features.enhance` configured (media:write + collaborator)
- `GET /api/media/:id/enhance/:enhancementId` - Poll status + compare payload: `{ data: { id, status, model, original: {url,width,height,size}, enhanced: {url,width,height,size}, downscaled, params } }` (byte fields as strings); `lastError` present on `failed` (media:read + viewer)
...
Enrichment: `picture_enhancement` (per-item, priority 0, reason `rerun`) downloads the original, sharp auto-orients it (deterministic pre-pass), calls the configured OpenAI model via `enhanceImage()` with `input_fidelity`, uploads the result to a staging object, and records dims/size → `ready`/`failed`. **Known limitation:** `OpenAiProvider.enhanceImage` rethrows a generic `Error` on failure (only 401/404 get distinct messages) rather than preserving the original HTTP status, so a real OpenAI 429/529 during enhancement is NOT classified as a rate limit — it routes through the job's normal-failure retry/backoff instead of the rate-limit-deferral path used by other enrichment handlers. Server-only in v1 — no `nodeResultSchema`/`persistNodeResult`, absent from the CLI `NODE_JOB_TYPES`, mirroring the `location_inference`/`face_auto_archive_sweep` precedent. An hourly cron (`PictureEnhancementPurgeTask`) enqueues a global `picture_enhancement_purge` job that deletes staging objects and marks rows `expired` for `ready`/`failed` enhancements older than `pictureEnhancement.retentionHours`, mirroring `TrashPurgeTask`.

> **UI:** An "AI Enhance" (`AutoFixHigh`) trigger appears in the gallery selection top bar (`BulkActionToolbar`, single-photo selection only) and in the full-screen viewer (`MediaLightbox`, next to Edit orientation), both feature-gated. `MediaEnhancementDrawer` walks params → progress (2s poll) → side-by-side compare (with a downscale warning) → keep both / replace / discard confirm dialogs. `MediaDetailDrawer` shows an "AI Enhanced" badge.
...
- `GET /api/admin/ai/enhance/status` - Get AI Picture Enhancer feature/provider status: `{ featureEnabled, provider, model, credentialConfigured, ready }`; backs the Doctor `ai.pictureEnhancer` check and the Admin AI settings UI (system_settings:read, Admin)
...
- `pictureEnhancement.stampExif` — boolean, default `false`; whether to embed a file-level EXIF/XMP "AI enhanced" marker. **v1 limitation:** no file-level EXIF writer is wired yet (deferred — see the spec's Implementation status note), so this setting currently has no effect regardless of value; only the in-app marker (`metadata._aiEnhanced` + "AI Enhanced" system tag) is written
...
- [AI Picture Enhancer](docs/specs/picture-enhancer.md) — photo-only, single-item, human-reviewed OpenAI `gpt-image-1` enhancement via a staged preview + keep-both/replace/discard review gate; `media_enhancements` data model, async `picture_enhancement`/`picture_enhancement_purge` enrichment job types (server-only, not node-claimable), destructive-replace precedent shared with orientation edit, settings/env configuration, Doctor `ai.pictureEnhancer` check; documents the v1 scope actually shipped vs. the original draft (EXIF/XMP file marker deferred — in-app marker only; `enhanceImage` failures use normal retry, not rate-limit deferral)

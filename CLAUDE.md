...
- `POST /api/media/bursts/bulk/resolve-by-threshold` body `{ circleId, threshold (int 0–100), action: 'archive'|'trash' }` - Resolve every PENDING burst group whose persisted `confidence >= threshold/100` in one call, capped at 500 groups (`MAX_THRESHOLD_RESOLVE`); null-confidence legacy groups are excluded — never auto-resolved by this path; manual trigger only, no cron; same keep-`suggestedBestItemId`-archive/trash-the-rest semantics as the bulk resolve above; returns `{ data: { resolvedGroups, keptCount, removedCount, action, skipped, errors, remaining } }` — `remaining` is the exact count of still-pending eligible groups after this batch (burst confidence is a persisted column, so an exact count is cheap via SQL); the frontend auto-loops the call until `remaining === 0` to fully drain the queue (media:write + collaborator; trash requires media:delete)
- `POST /api/media/bursts/bulk/dismiss-by-threshold` body `{ circleId, threshold (int 0–100) }` - Dismiss every PENDING burst group whose persisted `confidence < threshold/100` in one call, capped at 500 groups (`MAX_THRESHOLD_RESOLVE`); null-confidence legacy groups are excluded — never auto-dismissed by this path; manual trigger only, no cron; mark "not a burst": ungroups members, status=dismissed (nothing archived or trashed); returns `{ data: { dismissedGroups, ungroupedCount, skipped, errors } }` (media:write + collaborator); together with `bulk/resolve-by-threshold` above, one threshold partitions the pending queue: resolve `>= N%`, dismiss `< N%`
...
- `GET /api/media/duplicates?circleId=&status=&kind=&page=&pageSize=&sortBy=&sortOrder=` - List duplicate groups (review queue), ordered chronologically by `capturedAt` by default; items `{ id, status, kind, mediaCount, suggestedBestItemId, capturedAt, confidence, coverThumbnailUrls[] }`; `confidence` is the tightest-pair CLIP cosine similarity (0–1), computed at read time (not persisted); `sortBy` `capturedAt`(default)|`confidence`|`mediaCount`, `sortOrder` `asc`(default)|`desc`; `confidence` has no column, so that sort is applied in memory over the fully-materialized, kind-filtered set before pagination — a group with no computable similarity sorts last regardless of direction; `capturedAt`/`mediaCount` stay a DB `orderBy` with a trailing `id` tiebreaker; response `{ items, meta:{total,page,pageSize} }` (media:read + viewer)
...
- `POST /api/media/duplicates/bulk/resolve-by-threshold` body `{ circleId, threshold (int 0–100), action: 'archive'|'trash' }` - Same shape as the burst threshold endpoint above, but duplicate confidence is computed at READ time (tightest-pair CLIP similarity via `computeGroupKind`), not persisted — loads pending groups (capped at 500) then filters per-group in application code; groups whose computed similarity is null or below `threshold/100` are counted in `skipped`; returns `{ data: { resolvedGroups, keptCount, removedCount, action, skipped, errors, hasMore } }` — `hasMore` is a boolean rather than an exact count (unlike the burst endpoint's `remaining`) since duplicate confidence isn't persisted and the candidate query is capped at `MAX_THRESHOLD_RESOLVE=500`; `hasMore` reports whether that cap was hit; the frontend auto-loops the call while `hasMore` is true to fully drain the queue (media:write + collaborator; trash requires media:delete)
- `POST /api/media/duplicates/bulk/dismiss-by-threshold` body `{ circleId, threshold (int 0–100) }` - Same shape as the burst dismiss-by-threshold endpoint above, but duplicate confidence is computed at READ time (tightest-pair CLIP similarity via `computeGroupKind`), not persisted — loads pending groups (capped at 500) then filters per-group in application code; dismisses every group whose computed similarity is below `threshold/100`; groups whose similarity is null (uncomputable) or at/above the threshold are skipped; mark "not duplicates": ungroups members, status=dismissed; returns `{ data: { dismissedGroups, ungroupedCount, skipped, errors } }` (media:write + collaborator); together with `bulk/resolve-by-threshold` above, one threshold partitions the pending queue: resolve `>= N%`, dismiss `< N%`
...
- `POST /api/media/location-suggestions/bulk-accept` body `{ circleId, threshold }` (threshold 0–100 int) - **Async** (issue #125, rebuilt on the same evaluate/execute-batch run pattern as Empty Trash at Scale): starts a run that accepts every pending suggestion in the circle at/above `threshold/100` confidence (always unmodified, `coordSource='inferred'`); returns immediately `{ runId, status, matchedCount }` (`matchedCount` is 0 at creation, the real total once evaluated); 409 if a run is already in progress for the circle (media:write + collaborator)
...
- `GET /api/location-suggestion-runs/:id` - Run detail: counters (`matchedCount`/`processedCount`/`succeededCount`/`failedCount`/`skippedCount`), `action` (`accept`|`reject`), `threshold`, status, timestamps, `lastError`, plus `itemStatusCounts` (live tally by item status) (media:read + viewer)
- `GET /api/location-suggestion-runs/:id/items?status=&page=&pageSize=` - Paginated run items with batched signed thumbnails; `status` filters `matched|processing|applied|failed|skipped` (media:read + viewer)
- `POST /api/location-suggestion-runs/:id/cancel` - Cancel a non-terminal run (cooperative — an in-flight batch finishes before the next unclaimed chunk stops; already-applied/rejected suggestions stay resolved) (media:write + collaborator)
...
- `location_suggestion_runs` - One row per bulk accept/reject run for the Location Suggestions review queue (issue #125), mirroring `trash_empty_runs`; `action` (`LocationSuggestionRunAction` enum: `accept` | `reject`), `threshold` (Int, 0–100 snapshot of the confidence floor used at evaluation time), `status` (`LocationSuggestionRunStatus` enum: `evaluating` | `running` | `completed` | `completed_with_errors` | `failed` | `cancelled`), the five counters (`matched_count`/`processed_count`/`succeeded_count`/`failed_count`/`skipped_count`), `started_by_id` (FK → `users`, SetNull), `last_error`, timestamps; indexes `(circle_id, status)` (per-circle 409 concurrency guard — only one active run per circle) and `(status, updated_at)`; migration `20260725000000_location_suggestion_runs`
- `location_suggestion_run_items` - One row per matched `LocationSuggestion` within a `LocationSuggestionRun`; `status` (`LocationSuggestionRunItemStatus` enum: `matched` | `processing` | `applied` | `failed` | `skipped`); `error`; `@@unique([runId, suggestionId])` idempotency anchor for batch retries (same pattern as `trash_empty_run_items`/`workflow_run_items`); `suggestion_id` FK cascades on delete; index `(run_id, status)`. Unlike `trash_empty_run_items` (where a successful purge cascade-deletes the `MediaItem` row, so `deleted` doubles as claim-marker and terminal state), accept/reject never deletes the suggestion row, so `processing` is the transient crash-safe claim marker between `matched` and a terminal status; migration `20260725000000_location_suggestion_runs`

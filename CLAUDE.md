...
Social media detection is enabled globally via the `features.socialMediaDetection` system setting (default off). It applies code-defined protected **system tags** (`Social Media` and per-platform tags) to videos identified as clips saved from TikTok, Instagram, Facebook, or WhatsApp. Detection uses video container metadata (ffprobe), filename patterns, and — when metadata is inconclusive — keyless on-server OCR of sampled frames via `tesseract.js`. Status is tracked in `media_social_status`. See the [Social Media Detection spec](docs/specs/social-media-detection.md).

**Phase 2 — Enrichment gate:** When `features.socialMediaDetection` is enabled, the video upload pipeline enqueues `social_media_detection` FIRST, before any other enrichment. If the item IS detected as social media, no further enrichment jobs are enqueued for it — specifically, `video_face_detection` (the only other video-applicable enrichment) is suppressed. If the item is NOT detected, the pipeline chains `video_face_detection` (when `features.faceRecognition` is also eligible). This prevents the app from spending compute resources enriching social-media spam. As a defense-in-depth measure, the `video_face_detection` handler also marks items `no_faces` immediately if they already carry the `Social Media` system tag — so manual face re-runs and face backfills respect the gate even if the tag was applied after the initial upload. The face backfill endpoint likewise excludes social-tagged videos.

**Manual override (not-sticky):** Users CAN manually remove the `Social Media` or platform system tag from an individual item (via the single-item or bulk tag-remove endpoints). Removing the tag opens the gate; the user must then explicitly trigger the desired job (e.g. `POST /api/media/:id/faces/rerun`) to process the video. The override is NOT sticky — a later social re-run or backfill may re-detect and re-apply the tag. Users still cannot fabricate or apply social system tag names themselves (the add-side reserved-name guard remains in place).
- `POST /api/media/:id/social/rerun` - Re-enqueue social media detection at priority 0; upserts `media_social_status` to `pending`; returns `{ jobId, status }` (media:write + collaborator)
- `GET /api/media/:id/social/status` - Get per-item detection status: `{ status, detected, platform, processedAt, lastError }` (status `not_processed|pending|processing|processed|failed`); `detected` is boolean, `platform` is the matched platform key or null (media:read + viewer)
...
- `POST /api/admin/social/backfill` body `{ from?, to?, force? }` - Bulk-enqueue social_media_detection jobs across all circles; videos only (`mediaType = video`); `from`/`to` (optional ISO-8601) bound `capturedAt`; `force=false` skips items already `processed`; 400 if `features.socialMediaDetection` is disabled (Admin + system_settings:write)
- `GET /api/admin/social/detectors` - Return the read-only platform registry: `{ mainTag: 'Social Media', platforms: [{ key, tagName }] }`; useful for auditing which platforms are supported and what system tag names they apply (Admin + system_settings:read)
...
- `/admin/settings/social` — global social media detection toggle (`features.socialMediaDetection`), OCR settings, read-only platform registry, global backfill
...
- `media_social_status` - Per-media-item social media detection status (one row per item); statuses: `not_processed`, `pending`, `processing`, `processed`, `failed`; also stores `detected` (Boolean?, true when a platform was positively identified), `platform` (String?, the matched platform key e.g. `tiktok`), and `score` (Float?, aggregate confidence score); tracks `processed_at` and `last_error`; unique on `media_item_id`; cascade delete on both `media_items` and `circles`

**Note:** `media_items` has `description` (nullable, max 8 192 chars) written by the auto-tagging handler on each successful vision call. There is no `title` column. `media_items`, `albums`, and `tags` use `added_by_id` (not `owner_id`) to track the uploading user. Dedup uniqueness for `media_items` is `(circle_id, content_hash)`. Tag names are unique per `(circle_id, name)`. The `tags` table has an `is_system` (Boolean, default `false`) column: system tags are code-defined (by the detector registry in `apps/api/src/social/social-detectors.ts`), not user-manageable — they cannot be renamed, deleted, or created (fabricated) via the normal tag-label UI. The `media_tags` join table has a `source` column (`manual` | `ai` | `system`, default `manual`) that tracks how a tag was applied: `source='ai'` rows are owned by the auto-tagging handler (re-runs overwrite them); `source='system'` rows are owned by enrichment detectors such as `social_media_detection` — tag-label cascade deletes and AI re-runs never touch `source='system'` rows; however, users CAN manually remove a `source='system'` tag from an individual item (via single-item or bulk tag-remove) as a not-sticky override of the social-media enrichment gate (removing the tag opens the gate; a later re-run may re-apply it). AI re-runs are authoritative over `source='ai'` rows only and never touch `source='manual'` or `source='system'` rows. `media_items` also carries burst detection columns: `perceptual_hash` (**TEXT?**, unsigned 64-bit dHash stored as a decimal string — see storage rationale below), `sharpness_score` (Float?, variance-of-Laplacian sharpness measure), `burst_uuid` (String?, Apple BurstUUID from EXIF MakerNote — null for non-Apple cameras), `burst_score` (Float?, composite quality score within the group — null when not in a group), and `burst_group_id` (FK → `burst_groups`, SetNull on delete). The `perceptual_hash` column is omitted from default API responses via a Prisma global `omit` because it is an internal computation value; the burst matcher parses it with `BigInt(string)` only when computing Hamming distance. **Why TEXT and not `bigint`:** Postgres `bigint` is a signed 64-bit integer (max 2^63-1); a dHash is an unsigned 64-bit value and hashes with the high bit set overflow, producing a "value out of range for type bigint" error. Additionally, Prisma maps `BigInt` to JavaScript's `BigInt` primitive, which throws "Do not know how to serialize a BigInt" on `JSON.stringify`, crashing any endpoint that returns the column. Storing the value as a decimal string avoids both problems. `media_items` also carries the archive column: `archived_at` (DateTime?, nullable; null = active/not-archived; non-null = archived). Trash reuses the existing `deleted_at` column (soft-delete). The two states are independent: `deleted_at` for Trash, `archived_at` for Archive.
...
**Upload enrichment trigger:** `MediaService.createMedia` calls `MediaEnrichmentService.enqueueUploadEnrichment(...)` synchronously before returning 201, so job rows exist in the database before any client receives a response — eliminating the timing gap that previously caused CLI uploads to miss enrichment jobs. For photos, `face_detection`, `auto_tagging`, and `burst_detection` are enqueued when their respective feature flags are enabled. For videos, the pipeline follows a gate ordering: when `features.socialMediaDetection` is enabled, `social_media_detection` is enqueued first and `video_face_detection` is held back — it is only enqueued (by the social detection handler, after the job completes) when the item is NOT detected as social media and `features.faceRecognition` is eligible. When `features.socialMediaDetection` is disabled, `video_face_detection` is enqueued directly at upload time. A single `MediaEnrichmentEnqueueListener` on `OBJECT_PROCESSED_EVENT` acts as an idempotent backstop for any path where processing completes after item registration. Each job type is gated by its global feature flag (`features.faceRecognition`, `features.autoTagging`, `features.burstDetection`, `features.socialMediaDetection`, all default off) plus its env kill-switch; clients never enqueue enrichment directly. Metadata columns (`capturedAt`, dimensions, geo) are synced separately via `MediaMetadataSyncService` and are not part of this trigger. Feature flags are read through a 5-second TTL cache on `SystemSettingsService.getSettings()` to avoid per-upload DB hits during bulk imports.
...
- `SOCIAL_MEDIA_DETECTION_ENABLED` - Environment kill-switch for auto-enqueue on upload; set to `false` to disable social media detection regardless of system settings. The runtime toggle is `features.socialMediaDetection` in system settings; this env var is a hard override for CI/test environments (default: `true`)
...
- `features.socialMediaDetection` — boolean, default false; global on/off for social media clip detection and system tag application; env `SOCIAL_MEDIA_DETECTION_ENABLED=false` overrides this

**Social Media Detection Settings (System Settings):**

Controlled via `social.ocr.*` in system settings, editable in Admin Settings → Social (`/admin/settings/social`). All settings require `features.socialMediaDetection=true` to have any effect.
- `social.ocr.enabled` — boolean, default true; when false, the OCR tier is skipped entirely — detection relies only on video container metadata and filename patterns; useful when CPU overhead of `tesseract.js` WASM is a concern
- `social.ocr.frameCount` — integer, 1–10, default 3; number of video frames sampled for OCR (last frame always included; remaining frames sampled evenly from the second half of the video where watermarks appear most reliably)
...
- [Social Media Detection](docs/specs/social-media-detection.md) — detector registry model, tiered signal pipeline (container metadata → filename → OCR), tesseract.js keyless WASM OCR, enrichment gate (social first → suppresses face on detection), system-tag protection matrix (fabrication blocked; per-item removal allowed as not-sticky override), `media_social_status` lifecycle, upload-enqueue/rerun/backfill, adding new platforms

...
All three are **best-effort / fire-and-forget**: a failed notification write logs a warning and never throws into or blocks the triggering action — the same contract as the transactional email sends. `NotificationsModule` exports `NotificationsService` plus the two injectable #247 producers (`UploadNotificationService`, `WorkflowRunNotificationService`).

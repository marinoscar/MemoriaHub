...
- `GET /api/media/duplicates?circleId=&status=&kind=&page=&pageSize=` - List duplicate groups (review queue), ordered chronologically by `capturedAt`; items `{ id, status, kind, mediaCount, suggestedBestItemId, capturedAt, confidence, coverThumbnailUrls[] }`; `confidence` is the tightest-pair CLIP cosine similarity (0–1); **persisted** as of issue #190 (`duplicate_groups.confidence`, single writer `DuplicateDetectionService.recomputeGroupMeta`, read-time self-heal backstop for pre-existing rows — see the [Review Runs spec §4](docs/specs/review-runs.md#4-duplicate-group-confidence-becomes-persisted)); response `{ items, meta:{total,page,pageSize} }` (media:read + viewer)

...
6. [Running the CompreFace Sidecar — Advanced / Legacy](#5-running-the-compreface-sidecar)
...
The fastest, most consistent way to add compute to the enrichment fleet is the pre-built worker container image (`ghcr.io/marinoscar/memoriahub-worker`) plus the root-level `docker-compose.worker.yml` bundle — not the native `node install-deps` path described in §2 onward, which remains available for a machine that can't or shouldn't run Docker (see that section's own note below). This is the recommended path for anything beyond a single ad-hoc laptop: it needs no native dependency installation on the host at all, since ffmpeg, the npm native compute libraries, tesseract's OCR language data, and the CLIP model file are all baked into the image at build time.
...
| `--skip-compreface` | Skip the Docker + `compreface-core` install/verify steps entirely — useful if you're running CompreFace separately, or don't need this node to be face-capable |
...
**What this command does NOT do:** it doesn't install Node.js or the CLI itself — those are prerequisites, since you're already running `memoriahub`. There is no separate provider choice to make: a node always uses CompreFace for face detection (issue #113 removed the `human`/`rekognition` alternatives), so setting up the sidecar here is what makes the node face-capable — whether `face_detection`/`video_face_detection` actually appear in the node's `eligibleTypes` depends only on `node doctor` finding the sidecar reachable and healthy (§5.5), not on any flag.
...
These install automatically as part of a normal `memoriahub` install (curl installer or local-clone build) — there is no separate step to run before `node start` on a supported platform. Because they're `optionalDependencies`, a platform where one fails to build/download does not break the rest of the CLI; that job type is simply excluded from `eligibleTypes` (or runs degraded, for the dHash-only entry above) until the dependency resolves.

Note that `face_detection`/`video_face_detection` are not powered by an npm dependency in this table at all — a node's face detection always runs against the external `compreface-core` sidecar (issue #113 removed the in-process `human` provider these two job types previously depended on). See §5 for the sidecar's setup and capability-gating requirements.
...
## 5. Running the CompreFace Sidecar

> **Advanced / legacy path.** This section covers configuring CompreFace by hand for a native (non-container) node install. The container bundle's `docker-compose.worker.yml` already runs a health-gated, multi-process `compreface-core` sidecar and the worker uses it automatically — see [Quick Start: Container Bundle (Recommended)](#quick-start-container-bundle-recommended) above. Read on only if you're setting up a native install or want to understand what the bundle automates.

CompreFace is the only face-recognition provider MemoriaHub ships (issue #113 removed the earlier `human` and `rekognition` alternatives, both on the server and on nodes). A worker node therefore always uses CompreFace for `face_detection`/`video_face_detection` — there is no provider to select. What determines whether a node is face-capable is simply whether its local `compreface-core` sidecar is reachable and healthy: if it isn't, `node doctor` reports the `compreface` capability unavailable and the node's existing capability gating (§5.5, §5.4) means it simply stops advertising those two job types as eligible, rather than falling back to anything else.

This section covers running that sidecar for a native (non-container) node install.

### 5.1 Install Docker

Face detection via CompreFace is the only capability in this whole guide that requires a container runtime — every other native dependency (`sharp`, `onnxruntime-node`, `tesseract.js`) is a plain npm package installed alongside the CLI (see §5.6 below for how CompreFace's container-based model differs from those). To make a node face-capable, install Docker on that node machine first so it can run the `compreface-core` sidecar container.
...
One flag on `node register` and `node start`:
...
The setting is stored in local node config only (`comprefaceUrl`) — never sent to the server, which learns only the resulting `eligibleTypes` the node ends up advertising. Omitting the flag uses the default (`http://localhost:3000`). There is no provider-selection flag — a node always uses CompreFace (issue #113 removed the `human`/`rekognition` alternatives).

The equivalent TUI path is Tools ▸ Worker Node ▸ Register node / Node config — both screens expose the CompreFace URL field alongside the existing concurrency and job-type fields.

### 5.4 Hard-fail behavior — no fallback to another provider

If a node's local CompreFace sidecar is unreachable, `node doctor` reports the `compreface` capability unavailable, and `face_detection`/`video_face_detection` are **NOT READY** on that node — it simply stops advertising those job types as eligible. There is no other provider to fall back to (issue #113 removed both alternatives), so an unreachable sidecar means the node is simply not face-capable until it's fixed. See §9 (Troubleshooting) below for the exact symptom and fix.
...
If CompreFace does not become healthy within the ~40s budget, `node start` **fails outright** — there is no fallback provider to silently degrade to. The operator must fix the sidecar (see §9 troubleshooting) before the node can start with face detection eligible.

The face provider (always `compreface`) is logged at start, and surfaced in `memoriahub node status` output as a "Face provider" line — present in both the live/IPC status view (a running daemon) and the offline/config status view (reading local config when no daemon is running).
...
- **Operational** (step 3, `runOperationalSelfTests()`) means a real minimal operation was attempted and succeeded — `sharp` actually decoded and re-encoded a pixel buffer, the CLIP model actually produced a 512-dimensional embedding, tesseract actually initialized and tore down a real OCR worker. A capability can be installed but not operational — most commonly because its model file/language data hasn't been fetched yet (§4), or, more rarely, because the installed native binary is broken for this platform. (CompreFace's check is different in kind — a live HTTP reachability probe against the sidecar rather than a self-test; see §5.5.)
...
| `compreface` capability shows unavailable / `face_detection` not ready | local `compreface-core` container isn't running or isn't reachable at the configured URL | start/check the container (`docker ps`, `curl http://localhost:3000/status`), or fix `comprefaceUrl` if it's using a non-default port |
...
| `face_detection`/`video_face_detection` never show as eligible, or the node never claims a face job | The local CompreFace sidecar isn't reachable or isn't healthy | Confirm `node start` waited for CompreFace to report healthy before claiming jobs (see §5.4); verify the sidecar directly with `curl http://localhost:3000/status` (or the configured `comprefaceUrl`); check the `compreface` row in `node doctor` (§5.5). |

...
> **Issue #113 — CompreFace-only.** This document now describes a CompreFace-only worker node. The Human WASM face provider was removed; `MEMORIAHUB_FACE_PROVIDER`/`--face-provider` default to and only accept `compreface`. §5 (formerly "Matching the Server's Face-Detection Provider") is rewritten accordingly — there is no longer a provider to match, only a CompreFace sidecar to set up.
...
6. [Setting Up CompreFace for a Native Install — Advanced / Legacy](#5-setting-up-compreface-for-a-native-install)
...
**Two setup paths exist.** The containerized worker bundle (this document's Quick Start, below) is the recommended path for anything beyond a single ad-hoc machine: it needs no native dependency installation on the host at all, since ffmpeg, the npm native compute libraries, tesseract's OCR language data, and the CLIP model file are all baked into the published image. §2 onward documents the native, non-container install — kept as the advanced/legacy path for a machine that can't or shouldn't run Docker, or for understanding/customizing exactly what the container bundle automates.
...
The fastest, most consistent way to add compute to the enrichment fleet is the pre-built worker container image (`ghcr.io/marinoscar/memoriahub-worker`) plus the root-level `docker-compose.worker.yml` bundle — not the native `node install-deps` path described in §2 onward, which remains available for a machine that can't or shouldn't run Docker (see that section's own note below). This is the recommended path for anything beyond a single ad-hoc laptop: it needs no native dependency installation on the host at all, since ffmpeg, the npm native compute libraries, tesseract's OCR language data, and the CLIP model file are all baked into the image at build time, and the bundle also runs a `compreface-core` sidecar for face detection (see below).
...
| `MEMORIAHUB_FACE_PROVIDER` | `compreface` — the only accepted value (default); the bundle sets it explicitly and points it at the bundled sidecar |
...
The worker image is published multi-arch (amd64 + arm64) and tagged with the exact CLI version that built it (`cli-<version>`) — pulling a specific tag guarantees the same compute stack (and therefore the same embeddings) as a server running that CLI version, per the parity guarantees in [distributed-nodes.md §7.3](specs/distributed-nodes.md#73-four-mechanisms-to-guarantee-parity--as-built). The CLIP model file and the tesseract OCR data are baked into the image at `/app/models` at build time, so a freshly-started container is air-gap friendly — nothing needs to be downloaded from Hugging Face/GitHub at startup. Face detection is served by the bundled `compreface-core` sidecar, not a baked-in model file — see [The bundled CompreFace sidecar](#the-bundled-compreface-sidecar) above.
...
| `--skip-compreface` | Skip the Docker + `compreface-core` install/verify steps entirely — useful on a machine that can't or shouldn't run Docker, or when you only want the rest of the dependency sweep. **Note:** since CompreFace is the only face-detection provider (issue #113), skipping it means this node will not be face-job-eligible until CompreFace is set up some other way — there is no alternative in-process provider to fall back to |
...
The sweep runs in order: reinstall any missing/broken npm native compute dependencies (§4), install `ffmpeg`/`ffprobe` via `apt` if missing (Debian/Ubuntu only — other distros get an unsupported-for-automation note for this one step, and the rest of the sweep still runs), initialize the OCR engine once with network access so tesseract's language data downloads as a side effect (§4), fetch the CLIP model file via the existing model-manifest flow (skipped with a clear message if the CLI isn't logged in yet — run `memoriahub login` first, then re-run), and — unless `--skip-compreface` — install Docker via `apt` if missing, pull/start the `compreface-core` container (§5.1), and verify it actually responds at `/status` with a short retry loop. The exit code is non-zero if any step genuinely failed; skipped, unsupported, or already-fine steps never fail the run.

**What this command does NOT do:** it doesn't install Node.js or the CLI itself — those are prerequisites, since you're already running `memoriahub`. It also doesn't register or start the node — even after successfully setting up CompreFace, you still run `node register`/`node start` (§5.3) yourself; this command only makes CompreFace *available*.
...
`face_detection`/`video_face_detection` need `sharp` for pixel prep, but face detection itself runs against the CompreFace sidecar over HTTP (§5) — there is no npm face-model dependency on the node.
...
## 5. Setting Up CompreFace for a Native Install
...
CompreFace is the sole face-recognition provider, server-side and node-side (issue #113 removed the previously-available Human WASM and AWS Rekognition alternatives). `MEMORIAHUB_FACE_PROVIDER`/`--face-provider` default to `compreface` and accept no other value — there is no in-process fallback provider a node can run instead, so a node that wants `face_detection`/`video_face_detection` eligibility must have a reachable `compreface-core` sidecar. This section walks through setting one up by hand for a native (non-container) install.

### 5.1 Install Docker

Face detection via CompreFace is the only capability in this whole guide that requires a container runtime — every other native dependency (`sharp`, `onnxruntime-node`, `tesseract.js`) is a plain npm package installed alongside the CLI (see §5.6 below for how CompreFace's container-based model differs from those). To run a local sidecar, install Docker on that node machine first so it can run the `compreface-core` container.
...
`node register` and `node start` accept:

| Flag | Values | Default | Description |
|------|--------|---------|-------------|
| `--face-provider <compreface>` | `compreface` (the only accepted value) | `compreface` | Which face-detection provider this node uses for `face_detection`/`video_face_detection` jobs |
| `--compreface-url <url>` | any URL | `http://localhost:3000` | Base URL of the node's local `compreface-core` sidecar |

```bash
memoriahub node register --compreface-url http://localhost:3000
...
If a node's local CompreFace sidecar is unreachable, `node doctor` reports the `compreface` capability unavailable, and `face_detection`/`video_face_detection` are **NOT READY** on that node — it simply stops advertising those job types as eligible. Since CompreFace is the only face-recognition provider, there is nothing to silently fall back to; the node degrades to "not face-job-eligible" rather than running detection some other way. See §9 (Troubleshooting) below for the exact symptom and fix.

This same hard-fail principle also applies to the warm-up window at start time, not just to an already-known-unreachable sidecar. When a node's eligible types include a face job (`face_detection`/`video_face_detection`), `node start` **blocks** on a bounded wait (~40 seconds) polling CompreFace's `/status` endpoint until it reports healthy, before the node claims any jobs. This exists because CompreFace takes roughly 15–30 seconds to load its model on (re)create; starting immediately would let the node claim face jobs before the sidecar is actually ready.

If CompreFace does not become healthy within the ~40s budget, `node start` **fails outright** rather than proceeding without face-detection capability. The operator must fix the sidecar (see §9 troubleshooting) before the node will claim face jobs.

The actively-selected face provider is logged at start, and surfaced in `memoriahub node status` output as a "Face provider" line — present in both the live/IPC status view (a running daemon) and the offline/config status view (reading local config when no daemon is running).

### 5.5 `node doctor` coverage

`node doctor` (CLI and TUI) has a `compreface` capability row, reported using the exact same installed-vs-operational model described in §8 for every other capability: **installed** means the configured `comprefaceUrl` responds at all to a `GET /status` call; **operational** means that response actually reports `status: 'OK'`. Like every other capability, a clean, fully-healthy node collapses this row into the one-line summary count; only a real problem expands it with detail. (The `node start` warm-up wait described in §5.4 uses this same `/status` check.)

### 5.6 How this differs from the CLIP model-file pattern

Unlike the CLIP ONNX model documented in §6 below, CompreFace isn't a downloadable model file `ensureModels()` fetches into `~/.memoriahub/models/`. It's a separate, long-running local service — a Docker container — that the operator starts and keeps running themselves. There is nothing for `node start`/`node doctor` to download or sha256-verify here; the capability check is a live HTTP reachability probe against whatever `comprefaceUrl` is configured, not a file-presence check.

---

## 6. Model Manifest & Downloads

`GET /api/nodes/models/manifest` (`jobs:read` permission) returns a bare array of model entries the node should have locally to serve its `eligibleTypes` — each with a real, computed `sha256` and `bytes` value (`apps/api/src/nodes/nodes.service.ts`'s `getModelManifest()`). As of this writing the manifest lists a single file: the CLIP ONNX vision model (`clip-vit-b32-vision-quantized.onnx`). Face detection is served by the CompreFace sidecar over HTTP (§5) rather than a downloadable model file, so it has no manifest entry.

`ensureModels()` (`apps/cli/src/node/models.ts`) downloads and sha256-verifies each manifest entry into `~/.memoriahub/models/` (overridable via the `MODELS_DIR` environment variable), skipping any file already present and valid. It is called automatically by `node start`, `node doctor`, and the equivalent TUI flows — there is no separate "download models" command. On success it points `process.env.MODELS_DIR` at the local directory for the rest of the process's lifetime.

**Verified path note:** the CLIP manifest entry (`targetSubdir: ''`, landing directly at `~/.memoriahub/models/clip-vit-b32-vision-quantized.onnx`) is consistent end-to-end with where the consuming code looks for it — `self-test.ts`'s `testClip()` and `apps/cli/src/node/compute/duplicate-detection.ts`. No manual workaround is needed. (This previously required a manual workaround — an earlier `targetSubdir: 'models'` value caused a double-nested download path, `~/.memoriahub/models/models/clip-vit-b32-vision-quantized.onnx` — fixed in the API's manifest; see `apps/api/src/nodes/nodes.service.ts`'s `getModelManifest()`, CLIP entry's `targetSubdir: ''`.)

**Air-gapped / offline installs:** place the CLIP model directly at `~/.memoriahub/models/clip-vit-b32-vision-quantized.onnx` (the path the compute/self-test code reads) — `ensureModels()`'s own existing-file check (`isValid()`) will skip re-downloading a file already present there with a matching size/sha256. This mirrors the equivalent server-side escape hatch documented for the API's own copy of this model in [duplicate-detection.md](specs/duplicate-detection.md#41-model-loading-and-lifecycle): "place the model file manually at `MODELS_DIR/clip-vit-b32-vision-quantized.onnx` before starting."
...
- **Operational** (step 3, `runOperationalSelfTests()`) means a real minimal operation was attempted and succeeded — `sharp` actually decoded and re-encoded a pixel buffer, the CLIP model actually produced a 512-dimensional embedding, tesseract actually initialized and tore down a real OCR worker, and CompreFace's `/status` endpoint actually reported `status: 'OK'` (§5.5). A capability can be installed but not operational — most commonly because its model file/language data hasn't been fetched yet (§4), the CompreFace sidecar isn't reachable yet, or, more rarely, because the installed native binary is broken for this platform.
...
| Worker crashes with `Ineffective mark-compacts near heap limit — JavaScript heap out of memory` during a long-running import | The node auto-hardens its heap ceiling on startup assuming a 16 GB RAM / 8-core host (`apps/cli/src/node/runtime-tuning.ts`); a raised ceiling only *delays* a genuine (e.g. native-dependency) leak, and on a smaller/memory-limited host the auto-computed ceiling may not fit | Under a supervisor (container / `node service` systemd unit) the worker now **drains and restarts itself before OOM** (`MEMORIAHUB_HEAP_RESTART_FRACTION`, default 0.90 → exit 75) so imports self-recover instead of crash-looping, with no work lost. Watch the climb via the memory-watchdog line in `memoriahub node logs` (which pool grows: `heapUsed` = JS objects, `external`/`arrayBuffers` = native buffers). On a tight/limited container set `MEMORIAHUB_MAX_OLD_SPACE_MB` explicitly below the cgroup limit (or `0` to disable re-tuning). **Do not run sustained imports in the interactive TUI's in-process engine** — the interactive process cannot raise its own heap ceiling (a re-exec breaks raw-mode input, PR #159) and OOM'd exactly this way on issue #156; the Worker Node dashboard's `[s]` key now launches the tuned daemon and attaches to it, leaving `[e]` as an explicit untuned in-process fallback. See [Worker Container Configuration Reference §3.5](worker-container-config.md#35-memory--oom-hardening). |
| Need to pin down exactly what's leaking (not just that memory is climbing) | The watchdog log says which pool grows but not the retaining object graph | Run **`memoriahub node heap-snapshot`** against the running daemon once it has been under import load for a while (check `heapGrowthMbPerHour` in `memoriahub node logs` first — if it is not climbing, there is nothing to catch). The live daemon writes `~/.memoriahub/logs/heap-manual-<pid>-<timestamp>.heapsnapshot` and prints the path; capturing in place matters, because restarting the worker to attach a flag discards the accumulated state that names the retainer. The pre-OOM valve also writes a `heap-critical-*` snapshot to the same directory before it drains and exits. Open either in Chrome DevTools → Memory → Load, sort by **Retained Size**, and the top constructor names the leaking retainer. Share it (or the watchdog trend) on [issue #156](https://github.com/marinoscar/MemoriaHub/issues/156). |
| `node heap-snapshot` reports "not enough free disk" or "No worker node daemon is running" | A snapshot is roughly heap-sized, so it is refused unless free disk is ≥ heap × 1.5; and there is nothing to snapshot unless a **daemon** is hosting the engine | Free space on the volume holding `~/.memoriahub/logs` (older snapshots are auto-pruned to the newest 2, but a multi-GB heap needs real headroom), or point `MEMORIAHUB_STATE_DIR` at a larger volume. If no daemon is running, start one with `memoriahub node start --daemon` (or the container/systemd service) — the interactive TUI's `[e]` in-process engine is not reachable over IPC. |
| Is the leak the CLIP/onnx path? (issue #156 bisection) | `duplicate_detection` is the only onnxruntime user in the app, so excluding it isolates that path | Run one import batch with it excluded — `memoriahub node start --daemon --types face_detection,auto_tagging,metadata_extraction,geocode,thumbnail_regen,social_media_detection,video_face_detection` (or the same list in `MEMORIAHUB_ELIGIBLE_TYPES`) — and compare `heapGrowthMbPerHour` in `memoriahub node logs` against a run that includes it. A slope that flattens implicates the CLIP path; one that keeps climbing rules it out and points elsewhere. |

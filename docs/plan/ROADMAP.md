...
| FR | **Face Recognition & People** (cross-cutting — realizes Phase 09 §4.1) | Done — `Person`/`Face`/`MediaFaceStatus` models, a single provider (CompreFace, 128-d — issue #113 removed the earlier human WASM and Rekognition providers), face detection handler in enrichment queue, clustering/merge, `/api/people` + `/api/media/:id/faces` endpoints, PeoplePage + UnknownFacesReview web UI, gated by the global `features.faceRecognition` setting. See [docs/specs/face-recognition.md](../specs/face-recognition.md). |
...
|-------------|---------------|--------|-------|
| §4.1 Face Recognition | [docs/specs/face-recognition.md](../specs/face-recognition.md) | Done | Single provider (CompreFace; issue #113 removed the earlier human WASM and Rekognition providers); globally toggled via `features.faceRecognition`; people clustering/merge; full web UI. Implemented beyond the original sketch — a real provider integration, not face-api.js. |

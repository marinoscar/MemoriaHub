...
| FR | **Face Recognition & People** (cross-cutting — realizes Phase 09 §4.1) | Done — `Person`/`Face`/`MediaFaceStatus` models, face detection handler in enrichment queue, clustering/merge, `/api/people` + `/api/media/:id/faces` endpoints, PeoplePage + UnknownFacesReview web UI, per-circle opt-in. Originally shipped with three providers (human WASM 1024-d, CompreFace 128-d, Rekognition); **issue #113 (July 2026) consolidated onto CompreFace as the sole provider**, removing Human and Rekognition. See [docs/specs/face-recognition.md](../specs/face-recognition.md). |
...
| 08 | Android Sync | MVP Done — `apps/android/` ships a native Kotlin/Compose client (AGP 8.11.1, minSdk 26) with RFC 8628 device-flow auth (cookie-replay refresh, zero backend changes), a durable Room-backed sync state machine (`sync_files` / `sync_runs`), the full resumable multipart upload pipeline (hash-with-cache → dedup pre-check → init → presigned part PUTs → complete → `POST /api/media {source:"android"}`), three WorkManager triggers (15-min periodic, content-URI observer, manual), crash/resume recovery, and a Photos / Backup / Settings UI with per-tile sync badges. 11 JVM unit tests pass (`./gradlew :app:testDebugUnitTest`). Remaining gaps: camera-only bucket filter, Wi-Fi-only policy UI, server-library browsing, and instrumented tests. See [docs/specs/android-sync.md](../specs/android-sync.md). |
...
|-------------|---------------|--------|-------|
| §4.1 Face Recognition | [docs/specs/face-recognition.md](../specs/face-recognition.md) | Done | Circle-scoped; people clustering/merge; full web UI. Implemented beyond the original sketch — circle-scoped, real provider integrations (not face-api.js). Originally three providers (human WASM, CompreFace, Rekognition); **consolidated onto CompreFace alone by issue #113** (July 2026). |
...
| §4.6 Video Thumbnail Generation | — | Done | `ThumbnailProcessor` extracts poster frame via ffmpeg for video MIME types; `video-probe.processor.ts` reads ffprobe metadata. |

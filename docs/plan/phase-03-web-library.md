...
- `MediaLibraryPage` (`apps/web/src/pages/MediaLibrary/`) — responsive MUI `ImageList` grid; timeline grouping by `capturedAt` year/month; filter controls (type, date range, classification, album); infinite scroll or page controls
- `MediaDetailDrawer` (`apps/web/src/components/media/`) — slide-in drawer showing full-resolution download link, all typed metadata fields from `MediaItem`, and the ability to edit `capturedAt` and `classification`
...
| 6 | Implement `MediaDetailDrawer`: MUI `Drawer` with image preview (via `thumbnailUrl` or full download URL), all `MediaItem` fields, inline edit for `capturedAt` and `classification` (calls `PATCH /api/media/:id`) | `frontend-dev` |
| 7 | Implement `MediaLibraryPage`: MUI `ImageList` (Masonry variant on desktop, single-column on mobile), group items by `capturedAt` month/year, filter sidebar or toolbar, open `MediaDetailDrawer` on click, upload FAB that opens `MediaUploadDialog` | `frontend-dev` |
...
- `MediaDetailDrawer` shows all typed metadata fields from `MediaItem` (type, capturedAt, dimensions, duration, GPS, camera, source, classification).
- Inline edit of `capturedAt` and `classification` in the drawer calls `PATCH /api/media/:id` and reflects the update without a full page reload.

...
> **Issue #113 — CompreFace-only.** This spec now describes a CompreFace-only system. The Human WASM (in-process, keyless, 1024-d) and AWS Rekognition (delegated, cloud) providers were removed entirely — CompreFace is the sole, default face-recognition provider everywhere: the only registry entry, the only thing the Face Settings admin UI shows, and the only embedding dimensionality (128-d) any `Face` row can have. A migration dropped `Face.externalFaceId` (existed only for Rekognition's delegated matching, zero production rows) and hard-deleted the legacy `providerKey='human'` 1024-d rows, which were permanently incompatible with the compreface-only 128-d pgvector world. Use `POST /api/admin/face/backfill` to regenerate face detection for any media affected by that cleanup.
...
|----------|------|-------------|
| `key` | `string` | Registry identifier — `'compreface'`, the only registered provider |
...
CompreFace (`exadel/compreface-core`, RetinaFace detector + ArcFace MobileFaceNet calculator) is the sole face-recognition provider (issue #113 removed the Human WASM and AWS Rekognition alternatives that previously existed alongside it).
...
| `detect` | Yes |
| `embed` | Yes |
| Infrastructure | `compreface-core` sidecar (stateless, no API key, no admin UI) |
| Data leaves server | No |
| Cost | Free |

It requires no API key, runs as a stateless sidecar on the same Docker network, produces 128-d ArcFace MobileFaceNet embeddings stored in the application's own database, and keeps all photo data and biometric vectors on-premise.
...
CompreFace has `requiresCredentials: false`. No API key is needed. A credential row for it, if present, stores only an optional `baseUrl` override — useful when the CompreFace sidecar runs at a non-default address. No key is set or required.
...
**Step 8.** Normalize bounding boxes. If any of `x`, `y`, `w`, or `h` in the returned bounding box is greater than 1.0, the box is in absolute pixel coordinates — divide by `uprightWidth` and `uprightHeight` to get fractions in 0–1. If all values are already ≤ 1.0, use them as-is. The CompreFace provider always returns absolute pixel coordinates from the sidecar, so this branch is always taken in practice; the guard is kept defensive rather than assumed. Normalization is always applied in the service, not the provider.
...
CompreFace is the only provider, so embedding dimensions are fixed:

| Provider | Dimensions | Algorithm |
|----------|-----------|-----------|
| `compreface` | 128 | ArcFace MobileFaceNet |

Never compare embeddings from different model versions. Switching the CompreFace image tag/model build (e.g. `1.2.0-mobilenet` → a different build with a different embedding dimension) requires re-processing all photos from the original S3 blobs.
...
`embedding_vec` is `NULL` for empty-array rows (e.g. manual `providerKey='manual'` associations, which carry no embedding by design). A one-shot backfill (`UPDATE faces SET embedding_vec = face_embedding_to_vector(embedding) WHERE array_length(embedding,1)=128`) populated existing rows before the indexes were built — index-after-backfill is deliberate, giving one clean index build instead of many incremental ones. The issue #113 migration also hard-deleted the legacy `providerKey='human'` 1024-d rows that previously would have left `embedding_vec` NULL for a non-empty-embedding reason.
...
**Dimension guard.** The pgvector path is taken only when the probe embedding is exactly 128-d (`PGVECTOR_EMBEDDING_DIM = 128`, the `compreface` mobilenet provider's dimensionality). Since CompreFace is now the only provider (issue #113 removed the Human WASM provider that previously produced 1024-d embeddings), essentially every non-empty embedding satisfies this guard; it remains in place as a defensive check that still correctly routes an empty embedding (e.g. a manual `providerKey='manual'` association) to the in-app cosine path — a `vector(128)` cast would otherwise raise a dimensionality error.
...
**The tab strip renders only when at least one person is hidden** (issue #369). Because `GET /api/people` hard-filters `hiddenAt: null` and no other surface in the web app queries `hidden=true`, this tab is the sole escape hatch from a hide — so it must appear the moment anything is hidden, but in the (normal) empty case a permanently-visible tab is dead UI that is easily mistaken for the separate **Archived faces** page at `/people/archived`, which operates on `Face.hiddenAt` rather than `Person.hiddenAt`. Two consequences of making it conditional:

- The count that decides visibility comes from a `pageSize: 1` count-only probe (`usePeople(…, { hidden: true, pageSize: 1 })`) reading `meta.totalItems`; the `HiddenPeopleView` component does its own full fetch when the tab is actually open, so the probe never pays for a page of person records and their signed cover thumbnails.
- Unhiding or purging the last hidden person makes the tab disappear while the user is standing on it, so the page falls back to `activeTab = 0` rather than rendering a body for a tab that no longer exists.
...
**Memory bound:** holding the full candidate set in memory costs `candidates × embeddingDim × 8 bytes` (float64 JS numbers) — at the default cap of 5000 and CompreFace's fixed 128-d embedding, that's `5000 × 128 × 8 ≈ 5 MB` per job, comfortably inside a worker's heap even on a constrained VPS. Lower `FACE_ARCHIVE_MAX_CANDIDATES` on memory-constrained deployments with very large archived pools.
...
| `embedding` | Float[] | L2-normalized embedding vector; 128-d for CompreFace (the only provider), empty for manual (`providerKey='manual'`) associations |
...
Indices: `circleId`, `mediaItemId`, `personId`, `(circleId, hiddenAt)` (added alongside `people(circleId, hiddenAt)` in migration `20260704000000_add_face_hidden_at`). The `externalFaceId` column and its index existed only for AWS Rekognition's delegated matching (zero production rows) and were dropped by the issue #113 migration.
...
|--------|------|-------------|
| `id` | UUID | Primary key |
| `provider` | String (unique) | Provider key — `compreface`, the only value used today |
| `encryptedKey` | String | AES-256-GCM encrypted API key (empty for the keyless `compreface` provider) |
| `baseUrl` | String? | URL override for the CompreFace sidecar |
| `last4` | String | Last 4 chars of plaintext key (display only; not applicable for the keyless provider) |
...
**Model-specific embeddings are not cross-comparable.** CompreFace is the only provider, so this is now purely a model-version concern: switching the CompreFace container's model build (e.g. away from the pinned `1.2.0-mobilenet` tag) changes the embedding space, and stored embeddings from the prior build become incomparable. Switching model builds requires erasing existing face data (`DELETE /api/face/biometrics`) and re-running detection.
...
| 3.8 | July 2026 | AI Assistant | Issue #113: standardized on CompreFace as the sole face-recognition provider — removed the Human WASM and AWS Rekognition providers, the delegated-recognition matching path, and the `Face.externalFaceId` column; simplified the provider comparison, embedding-dimension, and credentials sections to a single 128-d CompreFace model; removed the Human-provider and Rekognition operational subsections in §17 |
| 3.9 | August 2026 | AI Assistant | Issue #369: the People page Hidden tab is now conditional — the tab strip renders only when at least one person is hidden, its visibility is decided by a `pageSize: 1` count-only probe instead of a full `GET /api/people?hidden=true` per page load, and unhiding/purging the last hidden person falls the page back to the People tab. Frontend-only: no API, schema, or RBAC change |

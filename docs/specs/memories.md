...
| Field | Value |
|-------|-------|
| **Version** | 1.1 (complete and closed out: data model + generation plumbing + curation engine; all seven curators; AI titles; read/act API + per-user preferences; web Home carousel, hub & story player; notifications & email digest; admin settings page & sharded library backfill; epic-closing documentation sweep) |
| **Last Updated** | August 2026 |
| **Status** | Implemented and epic complete — Data Model (#301), Generation plumbing & Settings (#302), Curation engine / On This Day / template titles (#303), Trips curator (#304), People / Theme / Seasonal / Year-in-Review curators (#305), AI titles / subtitles / narratives (#306), API / per-user state / delete / save-as-album / preferences (#307), web Home carousel & `/memories` hub (#309), Notification Center integration & HTML email digest (#311), story player / save-as-album / share / preferences UI (#313), admin settings page & sharded library backfill (#315), full spec / CLAUDE.md / README documentation sweep (#317, this revision) — [Epic #300](https://github.com/marinoscar/MemoriaHub/issues/300) is complete |
...
|---|---|---|---|
| 1.1 | August 2026 | AI Assistant | Issue #317 (the epic's final, documentation-only issue): read every merged file under `apps/api/src/memories/` and `apps/web/src/{components,pages,services}/memories*` against this document section by section and found it already accurate and internally consistent — no contradictions with the shipped code, including the five deviations most likely to have drifted (`memory_generation`/`memory_digest` use `reason: JobReason.backfill`, not a `'scheduled'` value the `JobReason` enum does not have; `person_highlights` never writes `periodKey='all'`; the Trips overlap-match denominator is the SHORTER span, not the union; the digest watermark is the succeeded job row's own `finishedAt`; `memory_digest`'s handler succeeds on a partial send). This revision's actual changes are the version/status header, this history entry, and the epic-closing cross-references it drove elsewhere: the "### Memories" endpoint section, the `memories`/`memory_items`/`memory_user_state` Database Tables entries, the `memory_generation`/`memory_digest` job-type prose, `features.memories` + the full `memories.*` settings block + `MEMORIES_ENABLED`, `ai.features.memories`, `memories_ready`, `user_settings.memories`, `/admin/settings/memories`, and this spec's own link added to CLAUDE.md; a headline "✨ Memories" feature section added to README.md; a `## Memories` endpoint reference added to `docs/API.md`; a spec-index row added to `docs/ARCHITECTURE.md`; and `VISION.MD`'s pre-epic MVP-era memories-dashboard bullet (~line 196) updated to point at the shipped feature instead of describing a design that predates it. |

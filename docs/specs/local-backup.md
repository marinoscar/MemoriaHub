...
- **No changes to the server-side replication feature.** `apps/api/src/jobs/backup/` (`POST /api/admin/backup`, `GET /api/admin/backup/runs`, `/status`, `/runs/:runId`) was a separate, older feature — an Admin-triggered server-to-local-disk S3 mirror that ran *on the server itself* against `BACKUP_LOCAL_PATH`. This epic left it in place, removing only its superseded eager-listing endpoint (see [Gotchas §15](#15-gotchas)), which existed to presign every ready object in the library in one response and is redundant with the paginated, presigned change feed this epic introduces (§3). **Since superseded:** issue #367 retired that v0 feature outright — its module, endpoints, admin page, and `backup:run`/`backup:read` permissions are gone, and `/admin/settings/backup` now redirects to the Worker Nodes page from which this epic's per-node backup UI is reached. This epic's fencing-off is therefore historical; there is no longer a second media-backup surface to keep distinct from this one.

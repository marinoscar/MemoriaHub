...
**Closing the residual TOCTOU race — write-time re-check.** Eviction above is reactive: it only fires *after* `BurstDetectionService.processMediaItem` has finished grouping an item, and it is wrapped in a best-effort try/catch that never fails the burst job. This left a narrow but real window: `DuplicateDetectionService.processMediaItem` reads `burstGroupId` once at job start (Rule 1 above), but doesn't write `duplicateGroupId` until much later in the job (after both matching tiers and candidate resolution have run). An item could pass the start-of-job guard as ungrouped, then become a burst-group member *during* that same dedup job's execution, and still get written into a `DuplicateGroup` before eviction had a chance to run — a classic time-of-check-to-time-of-use gap that "burst wins" eviction alone could not close, only clean up after the fact. To close it, the duplicate-group membership write in `DuplicateDetectionService.processMediaItem` now runs inside a `$transaction` that, immediately before writing `duplicateGroupId`, executes `SELECT ... FOR UPDATE` on the subject media item and re-reads its `burstGroupId` plus that burst group's `status`. If the item is now in a `pending` burst group, the duplicate-group write is skipped entirely. Because this re-check takes a row lock on the same `media_items` row that burst detection's own `burstGroupId` write must also lock, the two operations serialize against each other: either dedup's re-check observes the burst assignment and skips, or dedup's transaction commits first and burst's reactive eviction then removes the item afterward. There is no interleaving in which an item ends up in both a burst group and a duplicate group at the same time.
...
Net effect: burst membership always wins, backed by two independent guarantees rather than one. The write-time re-check is the actual correctness guarantee — it is synchronous, transactional, and lock-serialized, so an item provably cannot be written into a duplicate group at the moment it is (or has just become) a pending burst-group member. Reactive eviction is the remediation/cleanup path for the complementary case — a dedup write that committed *before* the item's burst grouping happened — and for healing pre-existing overlap from before this fix existed. An item that lands in a burst group — whether it got there before or after duplicate detection ran — is guaranteed not to remain in a duplicate group at the same time.
...
### 3.5.1 `sortBy=confidence` is a DB `orderBy` (issue #194)

`sortBy=confidence` on `GET /api/media/duplicates` (§9.1) was added by issue #189 while `confidence` was still read-time-only, so it started life as an in-memory `Array#sort` over the fully-materialized result set. Issue #190 persisted the column but deliberately left the sort in memory so that PR's read-path behavior stayed byte-identical to what #189 shipped and tested. **Issue #194 finishes that conversion:** `DuplicateService.listDuplicateGroups` now pushes `sortBy=confidence` into the Prisma `orderBy` — `{ confidence: { sort: dir, nulls: 'last' } }` as the primary key, with `capturedAt` (`asc`, nulls last) then `id` (`asc`) as tiebreakers — mirroring `BurstService.listBurstGroups`'s existing `confidence` ordering exactly. The old `Array#sort` block and the `rawConfidence` field that fed it are gone. `sortBy=capturedAt`/`mediaCount` were already real-column DB orderings and are unchanged.

The observable guarantees carried over unchanged: a group with no computable similarity (`confidence IS NULL`) still sorts last in both directions — now via Postgres's `NULLS LAST`/`NULLS LAST` on both `asc` and `desc` rather than an explicit null-check comparator — and because the DB does the full ordering before the in-JS pagination slice, page N is still a true global rank rather than a per-page reshuffle.

**What did and didn't get cheaper:** this change removes exactly one cost — the `Array#sort` pass over the full matched set when `sortBy=confidence` is requested. It does **not** touch the more expensive step: `computeGroupKind` still runs its per-group tightest-pair similarity computation for every group in the matched set on every call, regardless of `sortBy`, because it feeds the `kind` classification field (and the optional `kind` query filter) — and `kind` is not a persisted column. The overall pagination model is also unchanged: the full matched set is still materialized, `kind`-filtered in application code if requested, and then sliced in JS for the requested page. Only the *order* of that materialized set is now determined by SQL for `sortBy=confidence`; the load-everything-then-slice shape of the endpoint is the same as before.
...
`duplicate_detection`'s own upload-time priority is unchanged at 10. As of the write-time re-check above, `burst_detection`'s upload-time enqueue priority was separately lowered to 5 (from 10) in `MediaEnrichmentService.enqueueUploadEnrichment` (`apps/api/src/media/enrichment/media-enrichment.service.ts`), so the enrichment worker (which claims jobs in `priority ASC, created_at ASC` order) claims and runs burst detection before duplicate detection in the common case. This is a scheduling optimization only, shrinking the race window before it's even hit — it is not what guarantees no double-membership; that guarantee comes from the write-time re-check described above.
...
- **Response `200`:**
  ...
  ```
  Ordered by `capturedAt ASC, createdAt ASC, id ASC` by default (chronological — see §3.4's discussion of immich Discussion #25831); `sortBy=confidence`/`mediaCount` and `sortOrder=desc` override this — as of issue #194, `sortBy=confidence` is a real DB `orderBy` on the persisted, indexed `confidence` column (§3.5.1), and a group with no computable similarity always sorts last regardless of direction (`NULLS LAST` on both `asc`/`desc`). Every ordering carries a trailing `id` tiebreaker so offset pagination cannot repeat or drop a group across pages. `coverThumbnailUrls` contains up to 4 signed thumbnail URLs for the first 4 active members. `kind` filtering is applied in application code after read-time classification (§3.4), so `meta.total` reflects the post-filter count. `confidence` is the read-time tightest-pair CLIP cosine similarity (`maxSim`, §3.5), or `null` if the group was linked hash-only.
...
| 1.6 | July 2026 | AI Assistant | Issue #194: `sortBy=confidence` on `GET /api/media/duplicates` is now a real DB `orderBy` over the persisted, indexed `confidence` column (§3.5.1, §9.1), mirroring `burst_groups.confidence`'s existing ordering — nulls-last in both directions, `id` tiebreaker. Rewrote §3.5.1, which previously (as of #190) documented this as a deliberately-deferred in-memory `Array#sort` follow-up; that follow-up is now done. Clarified that `computeGroupKind`'s per-group similarity computation over the full matched set — needed for the `kind` field/filter, which is not a persisted column — is unaffected and remains the dominant per-request cost; the overall materialize-then-slice pagination model is unchanged. |

...
> **Issue #113 — CompreFace-only.** This spec now describes a CompreFace-only face-detection compute path, server and node alike. The Human WASM in-process pipeline (the shared package's `/face` export, `@vladmandic/human`/`@tensorflow/tfjs*` dependencies) and AWS Rekognition were removed; `./face-compreface` is the sole face-detection subpath, embeddings are always 128-d, the node model manifest no longer lists Human model files, and `MEMORIAHUB_FACE_PROVIDER`/`--face-provider` accept only `compreface`.
...
| `POST /api/nodes/:id/jobs/:jobId/credentials` | Get a transient, per-job provider credential for `auto_tagging`, `video_auto_tagging`, or `geocode` (§2.7); response shape depends on job type — see §6 |
...
|----------|----------------------|-------|
| `face_detection` / `video_face_detection` | `{ modelVersion, providerKey, imageWidth, imageHeight, faces: [{ boundingBox: {x,y,width,height}, confidence?, embedding: number[], landmarks? }] }` | `boundingBox` is a **pixel** box relative to `imageWidth`/`imageHeight` (not the 0–1 normalized convention the `faces` table stores) — normalization happens server-side in the persist half. CompreFace is the sole face-recognition provider (issue #113), so `embedding` is always 128-d; the schema does not hard-pin the length, and the persist half still validates against the active provider's expected dimensionality. `landmarks` is opaque passthrough for CompreFace's facial-landmark data, persisted verbatim and never interpreted by the compute/persist split |
...
| `video_auto_tagging` | `{ rawText, frameCount, sampledTimestampsMs, transcript? }` submitted via the SAME `/result` endpoint | Epic #452. Same rationale as `auto_tagging`: parsing needs the DB-loaded `TagLabel` vocabulary, so it stays server-side and the node returns raw text plus frame/transcript provenance. The credentials response hands over `labelNames`/`peopleNames` rather than a finished prompt, because the user prompt embeds frame timestamps only the node knows |
...
- **Faces:** CompreFace (`exadel/compreface-core` sidecar, ArcFace MobileFaceNet), 128-dimensional embeddings — the sole face-recognition provider since issue #113 removed the Human WASM and AWS Rekognition alternatives.
...
- **Exact-pinned native dependencies, enforced at the root.** The package's own `dependencies`/`optionalDependencies` pin exact versions — `sharp@0.35.1`, `onnxruntime-node@1.27.0`, `tesseract.js@7.0.0` — and the **root** `package.json`'s `overrides` block repeats the exact same pins, so npm's workspace resolution cannot let `apps/api` or `apps/cli` end up with a different patch version of any of these than the shared package itself uses (a version-skew bug the parity guarantee in §7.1 depends on preventing). `tesseract.js` is declared as an `optionalDependency` so a lean CLI install doesn't force-download it — see §8 for the per-job-type degraded-mode behavior when it's absent. Issue #113 removed the Human WASM compute pipeline (`@tensorflow/tfjs`, `@tensorflow/tfjs-backend-wasm`, `@vladmandic/human`) from this package's dependencies and version-pin overrides entirely — face detection is served by the `./face-compreface` HTTP client instead of an in-process model.
- **Subpath exports, one per compute domain.** `package.json`'s `exports` map publishes fourteen entry points (root plus thirteen subpaths), each independently buildable/importable: `.` (root), `/image` (`prepareImageForProcessing` + orientation), `/clip` (CLIP ONNX session + embedding), `/dhash` (perceptual hash), `/face-compreface` (CompreFace sidecar HTTP client — the sole face-detection compute path, server and node alike, since issue #113 removed the `/face` Human WASM export), `/face-video` (shared face-video clustering helpers), `/ocr` (tesseract wrapper), `/metadata` (EXIF/ffprobe extraction), `/social` (social-media detection rule engine), `/video` (ffmpeg frame extraction), `/ai` (Anthropic/OpenAI vision calls), `/geo` (Nominatim/Google reverse-geocode calls), `/dto` (the zod result schemas — §6), `/rate-limit` (`ProviderRateLimitError`). A consumer imports only the subpath(s) it needs (`@memoriahub/enrichment-compute/clip`, etc.) rather than the whole package.
...
interface ModelManifestEntry {
  ...
  targetSubdir: string; // '' — where under the node's model dir to place it
}
```

The manifest currently lists a single file with real `sha256`/`bytes` values (§7.2 / §8): the CLIP ONNX vision model. Face detection has no manifest entry — it is served by the CompreFace sidecar over HTTP (§8.1), not a downloadable model file. (Issue #113 removed the four Human face-recognition model-file entries — `blazeface-back.json`/`.bin`, `faceres.json`/`.bin` — that were previously listed here.) A node fetches this manifest and compares it against the sha256 of its own local CLIP model file before advertising `duplicate_detection` in its `eligibleTypes` (§3.1). A byte-identical model file is the concrete, checkable proxy for "this node will produce embeddings comparable to the server's." (The CLIP entry's hash is documented as a point-in-time snapshot, not an eternal guarantee — Hugging Face may rebuild/re-quantize that file over time, so an unexpected hash-verification failure on that entry should prompt a re-download-and-re-hash, not an assumption that the local file is corrupt.)
...
|----------|--------|-------|
| `face_detection` | ✅ Implemented | Real compute via `@memoriahub/enrichment-compute/face-compreface`, the sole face-detection compute path (issue #113 removed the Human WASM `/face` export) — requires a local `compreface-core` sidecar the node itself runs; see [worker-node-setup.md §5](../worker-node-setup.md#5-setting-up-compreface-for-a-native-install) for setup |
...
| `video_face_detection` | ✅ Implemented | `apps/cli/src/node/compute/video-face-detection.ts` is now fully implemented, matching the server's `VideoFaceDetectionService`: probe → frame extraction (the shared `/video` package's `extractFramesAt`, the same function the social-media-detection compute module uses) → per-frame detection (CompreFace — the same sole provider as `face_detection` above) → cross-frame embedding-similarity clustering → face-crop thumbnail build and upload via the same `upload-url` presigned-PUT flow `thumbnail_regen` uses. The only remaining throw is the ordinary missing-sidecar capability guard, shared with the photo path — not a stub. Node-eligible like every other row in this tier, with the same hard-fail-no-silent-fallback CompreFace behavior as `face_detection` (see [worker-node-setup.md §5.4](../worker-node-setup.md#54-hard-fail-behavior--no-silent-fallback)) |
...
| `video_auto_tagging` | ✅ Implemented, via transient credentials | Epic #452. Requires `['sharp','ffmpeg','ffprobe']` — a node without ffmpeg never claims it, which is one of the three reasons it is a separate job type from `auto_tagging` (see [Video AI Auto-Tagging §3](video-auto-tagging.md#3-why-a-separate-job-type)). The ONLY type in `URL_INPUT_TYPES`: the engine passes the presigned URL straight to compute rather than pre-downloading, since ffmpeg range-seeks it and needs only the bytes around each seek point |
...
| `workflow_execute_batch` | ✅ Implemented, but **not CPU/GPU-bound like the rest of this table** | Media Workflow Automation's (epic #138) per-batch action-execution job. `JOB_TYPE_REQUIREMENTS['workflow_execute_batch'] = []` — zero native deps, zero model files, `inputUrl` always `null` (a global job, no media bytes involved). Node eligibility here is about **posture completeness**, not compute offload: the node's compute module (`apps/cli/src/node/compute/workflow-execute-batch.ts`) is a pure, dependency-free pass over the frozen action-type list in the claim `params` that declares intended per-item outcomes and never touches a database; `WorkflowExecuteBatchHandler.persistNodeResult` re-runs the FULL authoritative pipeline server-side from the trusted `job.payload`, ignoring the submitted result as a source of truth (§6.1). The type is ALSO kept in the API's `ENRICHMENT_WORKER_MODE=system` claim set alongside `thumbnail_repair`, so a fleet-only or `system`-mode deployment still executes workflows even with zero nodes registered. See the [Workflow Automation spec §11](workflows.md#11-node-execution-computepersist-split) |

These remain the primary target of this feature — CPU/GPU-bound, per-item, secret-free work that scales cleanly across however many nodes a household has online. (`workflow_execute_batch` above is the one exception admitted to this table for posture-completeness reasons rather than being CPU/GPU-bound.)
...
| `workflow_evaluate` | Global, one-per-run, pure-SQL materialization sweep (keyset-paginated `MediaItem` matching into `workflow_run_items`) — no per-item unit of work a node could claim, same reasoning as `location_inference` |
| `workflow_evaluate_item` | A per-item single-row indexed condition check against a live workflow definition, dispatched server-side directly off enrichment-settlement event listeners — cheap enough, and tightly coupled to server-local event plumbing, that distributing it would add coordination overhead without benefit |
| `workflow_history_purge` | Direct batch DB deletes (terminal `workflow_runs`/`workflow_run_items` past retention) plus a stale-approval expiry sweep — same reasoning as `job_history_purge`/`trash_purge` |
| `review_run_evaluate` | Keyset-paginated, pure-SQL materialization sweep against `review_run_items` (the shared run engine behind bursts/duplicates/location suggestions, issue #190) — same reasoning as `workflow_evaluate`; see [review-runs.md §5](review-runs.md#5-job-types) |
| `review_run_execute_batch` | Applies a review run's action (archive/trash/dismiss/accept/reject) by delegating into existing server-side domain services (`BurstService`, `DuplicateService`, the location-suggestion accept/reject transaction) — no media bytes a node would need to touch; see [review-runs.md §6](review-runs.md#6-subject-strategy-split) |
| `review_run_history_purge` | Direct batch DB deletes (terminal `review_runs`/`review_run_items` **and** `trash_empty_runs` past retention) plus a stale-run expiry sweep — same reasoning as `job_history_purge`/`workflow_history_purge`; see [review-runs.md §10](review-runs.md#10-retention-and-stale-run-sweep) |
| `duplicate_confidence_backfill` | Keyset-paginated, pure-SQL backfill of the persisted `duplicate_groups.confidence` column — a DB read/write sweep, not a per-item media-compute task; see [review-runs.md §4](review-runs.md#4-duplicate-group-confidence-becomes-persisted) |
| `location_group_rebuild` | Set-based, keyset-paginated SQL sweep that re-derives `media_items.geo_canonical_*` from the `location_groups`/`location_group_aliases` tables after an admin edit (issue #373) — the whole point is that its cost scales with the number of GROUPS, not media items, so there is no per-item unit of work to hand a node, and it needs direct DB access a node structurally does not have; same reasoning as `duplicate_confidence_backfill` |

`workflow_execute_batch` (epic #138, §8.1 above) is the deliberate exception to this table: unlike every row here, it genuinely IS node-eligible, but is ALSO kept server-claimable under `ENRICHMENT_WORKER_MODE=system` for posture-completeness (§14.1) rather than because it needs server-only resources.
...
| `node heap-snapshot` | Ask the running daemon to write a V8 heap snapshot into the log directory, for memory-leak diagnosis (issue #156). Requires a daemon: the value of the snapshot is that it comes from a process that is *already* leaking, so restarting the worker to attach a diagnostic launch flag defeats the purpose. Guarded by a free-disk pre-flight (≥ heap × 1.5) and keep-newest-2 retention; suppressed entirely by `MEMORIAHUB_HEAP_SNAPSHOT=0` |
...
**IPC protocol (`apps/cli/src/node/daemon.ts`, `apps/cli/src/node/ndjson.ts`).** The socket speaks one JSON object per line (NDJSON). Server → client frames: `{ kind: 'snapshot', ...EngineSnapshot }` (sent once on connect), `{ kind: 'log-tail', lines: string[] }` (recent log lines on connect), `{ kind: 'event', ev, payload, ts }` (every engine event, live), `{ kind: 'status', ...EngineSnapshot }` (reply to a status query), `{ kind: 'ack', cmd, ... }`, `{ kind: 'error', message }`. Client → server commands: `{ cmd: 'status' }`, `{ cmd: 'set-concurrency', value }`, `{ cmd: 'drain' }`, `{ cmd: 'stop' }`, `{ cmd: 'heap-snapshot' }`. This is how `node stop`, `node status`, `node set-concurrency`, `node heap-snapshot`, and the TUI's attach mode (below) all talk to an already-running daemon without needing a second server-side round trip for every query.

**Client backpressure.** A client that stops reading does not make the daemon's `socket.write()` fail — Node queues the frame in the socket's writable buffer instead. Since the daemon emits several frames per job, an unnoticed stalled reader (a suspended terminal, a `SIGSTOP`'d TUI, an undetected half-open connection) would accumulate frames on the worker's heap in proportion to jobs processed. The host therefore drops any client whose write backlog exceeds 1 MiB, logging it once: this is an observability channel, and a detached dashboard is worth strictly less than the worker's memory (issue #156).
...
| `system` | The in-process worker runs ONLY the server-only job types — `burst_detection`, `duplicate_detection_batch`, `job_history_purge`, `face_auto_archive_sweep`, `storage_insights`, `location_inference`, `trash_purge`, `storage_migration`, `workflow_evaluate`, `workflow_evaluate_item`, `workflow_history_purge`, `review_run_evaluate`, `review_run_execute_batch`, `review_run_history_purge`, `duplicate_confidence_backfill`, `location_group_rebuild` (light, DB-bound global jobs — see §8.3 plus the two global-sweep exceptions in §8.1), PLUS `thumbnail_repair` and `workflow_execute_batch` explicitly pinned server-side for posture-completeness even though both carry a `nodeResultSchema` (§8.1, §14.1's note below) — while every genuinely media-compute job type (`face_detection`, `video_face_detection`, `auto_tagging`, `geocode`, `duplicate_detection`, `metadata_extraction`, `social_media_detection`, `thumbnail_regen`) is left for the worker-container fleet (§15) to claim |
...
`apps/cli/Dockerfile` (target `worker`) builds a glibc `node:26-slim`-based image with `ffmpeg`/`perl` and the native compute dependencies (§7.3) installed, plus the CLIP model and tesseract OCR language data baked in at `/app/models` at build time — so a freshly-started container needs no first-run download to become operational (contrast with a native install's lazy model fetch, [worker-node-setup.md §6](../worker-node-setup.md#6-model-manifest--downloads)). Face detection is served by the bundled `compreface-core` sidecar, not a baked-in model file — see §15.2. It runs as an unprivileged user with a `/data` volume for node state, and its entrypoint execs `node .../index.js node start --headless` directly (exec form, so `SIGTERM` reaches the Node process rather than a shell wrapper) with `STOPSIGNAL SIGTERM` set in the image. The image is published multi-arch (amd64 + arm64) to `ghcr.io/marinoscar/memoriahub-worker`, tagged with the exact CLI version that built it (`cli-<version>`) — pulling a specific tag pins a known-good compute stack, the same parity guarantee §7.3 makes for any other install path.
...
| 2.2 | July 2026 | AI Assistant | Issue #113: standardized on CompreFace as the sole face-detection compute path — removed the Human WASM `/face` export and its dependency pins from the shared package (§7.2, §7.3), removed the four Human model-file entries from the node model manifest (§7.3), simplified the node-result payload's `embedding`/`landmarks`/`externalFaceId` documentation to the CompreFace-only shape (§6), updated the `face_detection`/`video_face_detection` node-eligibility rows (§8.1), removed the `testHuman` doctor self-test reference (§10.1), and updated the worker image description (§15.1) |
| 2.3 | July 2026 | AI Assistant | Issue #190: added the four new server-only job types backing the shared review-run engine to the §8.3 server-only table and the `ENRICHMENT_WORKER_MODE=system` claim list (§14.1) — `review_run_evaluate`, `review_run_execute_batch`, `review_run_history_purge`, `duplicate_confidence_backfill`; none is added to the CLI's `NODE_JOB_TYPES`. See [review-runs.md](review-runs.md) for the full feature. |

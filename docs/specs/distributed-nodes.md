...
|----------|----------------------|-------|
| `face_detection` / `video_face_detection` | `{ modelVersion, providerKey, imageWidth, imageHeight, faces: [{ boundingBox: {x,y,width,height}, confidence?, embedding: number[], landmarks? }] }` | `boundingBox` is a **pixel** box relative to `imageWidth`/`imageHeight` (not the 0–1 normalized convention the `faces` table stores) — normalization happens server-side in the persist half. `embedding` has no hard-pinned length in the schema, but in practice is always 128-d — CompreFace is the node's only face provider (issue #113 removed the `human`/`rekognition` alternatives, and with them the `externalFaceId` field this schema used to carry for delegated-recognition passthrough); the persist half validates against the active provider's expected dimensionality. `landmarks` remains an opaque optional passthrough, always absent for a node result today since CompreFace does not return delegated-recognition landmarks — see [worker-node-setup.md §5](../worker-node-setup.md#5-running-the-compreface-sidecar) |
...
export const faceDetectionResultSchema = z.object({
  ...
  // Pinned to the one surviving provider (issue #113), so a stale node
  // advertising a removed provider is rejected here rather than deep in the
...
- **Faces:** CompreFace (`exadel/compreface-core:1.2.0-mobilenet`, ArcFace MobileFaceNet), 128-dimensional embeddings, served by a local sidecar container over HTTP. Issue #113 removed the earlier in-process Human (`@vladmandic/human`, tfjs-wasm, 1024-d) provider; because both the server and every node now call the same sidecar image, face parity is a matter of pinning that image tag rather than pinning npm model libraries.
...
- **Exact-pinned native dependencies, enforced at the root.** The package's own `dependencies`/`optionalDependencies` pin exact versions — `sharp@0.35.1`, `onnxruntime-node@1.27.0`, `tesseract.js@7.0.0` — and the **root** `package.json`'s `overrides` block repeats the exact same pins, so npm's workspace resolution cannot let `apps/api` or `apps/cli` end up with a different patch version of any of these than the shared package itself uses (a version-skew bug the parity guarantee in §7.1 depends on preventing). The heavy model libraries (`onnxruntime-node`, `tesseract.js`) are declared as `optionalDependencies` so a lean CLI install doesn't force-download them — see §8 for the per-job-type degraded-mode behavior when they're absent. (Issue #113 removed the `@tensorflow/tfjs*` and `@vladmandic/human` pins along with the in-process Human face provider; face parity is now pinned by the `compreface-core` image tag instead.)
- **Subpath exports, one per compute domain.** `package.json`'s `exports` map publishes fourteen entry points, each independently buildable/importable: `.` (root), `/image` (`prepareImageForProcessing` + orientation), `/clip` (CLIP ONNX session + embedding), `/dhash` (perceptual hash), `/face-compreface` (CompreFace sidecar HTTP client), `/face-video` (cross-frame face clustering), `/ocr` (tesseract wrapper), `/metadata` (EXIF/ffprobe extraction), `/social` (social-media detection rule engine), `/video` (ffmpeg frame extraction), `/ai` (Anthropic/OpenAI vision calls), `/geo` (Nominatim/Google reverse-geocode calls), `/dto` (the zod result schemas — §6), `/rate-limit` (`ProviderRateLimitError`). The former `/face` subpath (the in-process Human detector) was removed with that provider in issue #113. A consumer imports only the subpath(s) it needs (`@memoriahub/enrichment-compute/clip`, etc.) rather than the whole package.
...
interface ModelManifestEntry {
  ...
  targetSubdir: string; // where under the node's model dir to place it ('' = the models root)
}
```

The manifest currently lists **one** file with real `sha256`/`bytes` values (§7.2 / §8): the CLIP ONNX vision model. Face detection contributes no manifest entry — CompreFace ships its weights inside the `compreface-core` sidecar image, so a node's face readiness is gated by sidecar reachability rather than by a downloaded model file. (Until issue #113 the manifest also carried four Human face-model files — `blazeface-back.json`/`.bin` and `faceres.json`/`.bin` — which were removed with that provider.) A node fetches this manifest and compares it against the sha256 of its own local model files before advertising the corresponding job type in its `eligibleTypes` (§3.1). Byte-identical weights are the concrete, checkable proxy for "this node will produce embeddings comparable to the server's." (The CLIP entry's hash is documented as a point-in-time snapshot, not an eternal guarantee — Hugging Face may rebuild/re-quantize that file over time, so an unexpected hash-verification failure on that one entry should prompt a re-download-and-re-hash, not an assumption that the local file is corrupt.)
...
|----------|--------|-------|
| `face_detection` | ✅ Implemented | Real compute via `@memoriahub/enrichment-compute/face-compreface`, which calls a local `compreface-core` sidecar the node itself runs. Since issue #113 there is no provider choice — CompreFace is the only face provider on both the server and the node, so the embedding spaces match by construction and the old cross-provider mismatch warning is gone. A node whose sidecar is unreachable stops advertising this job type rather than falling back to anything; see [worker-node-setup.md §5](../worker-node-setup.md#5-running-the-compreface-sidecar) for setup |
...
| `video_face_detection` | ✅ Implemented | `apps/cli/src/node/compute/video-face-detection.ts` is now fully implemented, matching the server's `VideoFaceDetectionService`: probe → frame extraction (the shared `/video` package's `extractFramesAt`, the same function the social-media-detection compute module uses) → per-frame detection (CompreFace, the same sole provider as `face_detection` above) → cross-frame embedding-similarity clustering → face-crop thumbnail build and upload via the same `upload-url` presigned-PUT flow `thumbnail_regen` uses. The only remaining throw is the ordinary sidecar-unreachable capability guard, shared with the photo path — not a stub. Node-eligible like every other row in this tier, with the same hard-fail-not-silent-fallback CompreFace behavior as `face_detection` (see [worker-node-setup.md §5.4](../worker-node-setup.md#54-hard-fail-behavior--no-fallback-to-another-provider)) |
...
`apps/cli/Dockerfile` (target `worker`) builds a glibc `node:26-slim`-based image with `ffmpeg`/`perl` and the native compute dependencies (§7.3) installed, plus the CLIP and tesseract model files baked in at `/app/models` at build time — so a freshly-started container needs no first-run download to become operational (face detection needs no baked model at all: the compose bundle runs a `compreface-core` sidecar alongside the worker, §15.2) (contrast with a native install's lazy model fetch, [worker-node-setup.md §6](../worker-node-setup.md#6-model-manifest--downloads)). It runs as an unprivileged user with a `/data` volume for node state, and its entrypoint execs `node .../index.js node start --headless` directly (exec form, so `SIGTERM` reaches the Node process rather than a shell wrapper) with `STOPSIGNAL SIGTERM` set in the image. The image is published multi-arch (amd64 + arm64) to `ghcr.io/marinoscar/memoriahub-worker`, tagged with the exact CLI version that built it (`cli-<version>`) — pulling a specific tag pins a known-good compute stack, the same parity guarantee §7.3 makes for any other install path.
...
- **`memoriahub-worker`**, `restart: unless-stopped`, with a `stop_grace_period` so an in-flight job gets a real chance to drain on shutdown (§14.2's reaper is the backstop if it doesn't), configured by default with `MEMORIAHUB_COMPREFACE_URL=http://compreface-core:3000` so the bundle's own sidecar is used out of the box. (The companion `MEMORIAHUB_FACE_PROVIDER=compreface` setting was dropped in issue #113 — with one provider there is nothing to select.)

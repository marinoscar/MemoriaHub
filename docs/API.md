...
**Provider abstraction:** The face domain supports pluggable providers via a `FaceProvider` interface. MemoriaHub standardizes on a single provider (issue #113 removed the earlier `human` and `rekognition` alternatives):

| Provider key | Type | Embeddings | Notes |
|---|---|---|---|
| `compreface` | Keyless core sidecar (only provider) | 128-d ArcFace mobilefacenet, owned by app | `compreface-core` container (no DB, no API key); API calls directly to `http://compreface-core:3000`. |
...
**Path Parameter:** `provider` — `compreface`
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `apiKey` | string | No | Provider API key or secret. Not applicable — `compreface` is keyless. |

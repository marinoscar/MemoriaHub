...
- **Tools > Worker Node TUI dashboard (`[e]` in-process fallback only):** never kills the interactive session (and restarting only the in-process engine wouldn't free native singletons like the CLIP session), so it drains and stops the embedded engine and logs a message telling you to restart via `[s]` (the tuned daemon) for sustained loads. Because the interactive process usually runs at the untuned ~2 GB default ceiling, the in-process valve is tightened to 15 s sampling and an 0.80 critical fraction (an explicit `MEMORIAHUB_HEAP_RESTART_FRACTION` still wins) — at a 2 GB ceiling the standard 60 s/0.90 settings leave only ~200 MB of margin, which the 1.2.14 crash on issue #156 blew straight through. The dashboard's primary `[s]` path avoids all of this by never running compute in the interactive process.

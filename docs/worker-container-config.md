...
| `NODE_OFFLINE_STALE_MULTIPLIER` | `10` | multiple of `NODE_HEARTBEAT_STALE_SECONDS` (min 1) | How many consecutive stale heartbeat windows a node must miss before `NodeStaleOfflineTask` (every 10 min) flips it from `online`/`draining` to `status='offline'`. Default 10 x 60s = 10 minutes of total silence. Deliberately a MULTIPLE of the same freshness window the admin fleet view's "Stale" pill uses, so the two can never drift into unrelated notions of liveness. `disabled` nodes are never auto-transitioned (explicit admin intent); a node that never heartbeated is aged by `registeredAt`. | Raise on a flaky network where nodes legitimately go quiet for minutes at a time; lower for a faster-reacting fleet view. Note this sweep is also what makes a crashed node eligible for `NODE_OFFLINE_RETENTION_DAYS` pruning at all — that pruner selects only `status='offline'`. |
| `NODE_STALE_OFFLINE_ENABLED` | `true` | `true` / `false` | Kill switch for the `NodeStaleOfflineTask` staleness sweep above. | Set `false` only if you want `status` to reflect explicit `node deregister` calls exclusively — accepting that crashed nodes then show a green "online" chip forever and are never pruned. |
...
| `MEMORIAHUB_HEAP_SNAPSHOT` | `1` (enabled) | Master switch for every heap snapshot the worker can produce: the `--heapsnapshot-near-heap-limit=1` launch flag (written to the process's working directory on a genuine near-OOM), the snapshot the pre-OOM safety valve writes at its own threshold, and the on-demand `memoriahub node heap-snapshot` command (both of the latter land in the worker log directory); `1` \| `0`. | Set `0` to suppress — a snapshot of an 8 GB+ heap can be multi-GB on disk. Retention keeps only the newest 2 valve/on-demand snapshots, and each is refused unless free disk ≥ heap × 1.5. |
...
| `MEMORIAHUB_MEMWATCH` | `1` (enabled) | The memory watchdog samples `process.memoryUsage()` on an interval into the worker log (rss / heapUsed / heapTotal / external / arrayBuffers / heapLimit), plus `heapGrowthMbPerHour` once the readings span 30 min, escalating to `warn` past 85% of the ceiling so a slow climb is visible *and quantified* (see §3.5); `1` \| `0`. | Set `0` to silence the periodic sample line. |
...
On startup the worker re-execs itself once with `--max-old-space-size` raised to ~50% of physical RAM (clamped 4–12 GB; 8 GB on the 16 GB baseline), giving the same slow climb many more hours of headroom, plus `--heapsnapshot-near-heap-limit=1` so a genuine recurrence writes a single heap snapshot — **landing in the process's current working directory** — pinpointing the retainer. This applies to `node start` (foreground, `--daemon`, the systemd unit, and the container image entrypoint). The interactive Worker Node dashboard is the one place it CANNOT apply: a re-exec shim breaks Ink's raw-mode keyboard input (PR #159), so the interactive process keeps Node's ~2 GB default ceiling — which is why the dashboard's `[s]` key now **spawns the tuned detached daemon and attaches to it** instead of hosting compute in-process (issue #156; the daemon keeps running after the dashboard detaches). The in-process engine remains only as the explicit `[e]` fallback, which warns about the untuned ceiling and tightens the memory watchdog (15 s sampling, drain at 80%). Alongside the heap ceiling, sharp's per-process operation cache is disabled and per-pipeline libvips concurrency is capped so peak native memory doesn't scale with cores × in-flight jobs, and `MEMORIAHUB_CONCURRENCY`'s default is now core/RAM-aware (2–4) instead of a flat `1` (§3.1).
...
Each line also carries **`heapGrowthMbPerHour`** — a least-squares slope of `heapUsed` over a rolling one-hour window, with `trendWindowMinutes` alongside it — so "is it still climbing?" is a number you can read off one line instead of a judgement call across hours of them. It is deliberately withheld until the retained readings span **30 minutes**: a bounded ±40 MB GC sawtooth regresses to ~100 MB/h over a 12-minute window but only ~4 MB/h over a full hour, so a short-window slope would report a phantom leak on a perfectly healthy worker. This is the number to compare across a bisection run (e.g. with `duplicate_detection` excluded from `MEMORIAHUB_ELIGIBLE_TYPES`) — a sustained positive slope is a leak, a slope oscillating around zero is not.
...
- **Worker Node TUI dashboard (`[e]` in-process fallback only):** never kills the interactive session (and restarting only the in-process engine wouldn't free native singletons like the CLIP session), so it drains and stops the embedded engine and logs a message telling you to restart via `[s]` (the tuned daemon) for sustained loads. Because the interactive process usually runs at the untuned ~2 GB default ceiling, the in-process valve is tightened to 15 s sampling and an 0.80 critical fraction (an explicit `MEMORIAHUB_HEAP_RESTART_FRACTION` still wins) — at a 2 GB ceiling the standard 60 s/0.90 settings leave only ~200 MB of margin, which the 1.2.14 crash on issue #156 blew straight through. The dashboard's primary `[s]` path avoids all of this by never running compute in the interactive process.

Set `MEMORIAHUB_HEAP_RESTART_FRACTION=0` to disable the valve.

**CLIP-path mitigation.** The shared onnxruntime CLIP path (`duplicate_detection` — the *only* onnxruntime user in the app) now: disables the CPU mem arena / mem-pattern pools, disposes input/output tensors after each inference, and **recycles the `InferenceSession` every N embeds** (release + recreate) so any native memory the session accumulates across `run()` calls is periodically reclaimed rather than pinned for the process lifetime. All parity-safe (the embedding is unchanged). The recycle cadence is `MEMORIAHUB_CLIP_RECYCLE_AFTER` on the worker and `CLIP_SESSION_RECYCLE_AFTER` on the API (both default `1000`; `0` disables). Recreating a session costs ~200 ms, negligible amortized over 1000 jobs. This lowers the likelihood/rate of the leak; it is not a guaranteed cure, which is why the watchdog and safety valve exist — and since `duplicate_detection` is the sole onnxruntime path, excluding that job type from a worker (or turning off `features.duplicateDetection`) sidesteps it entirely while the root cause is confirmed.

**Capturing a heap snapshot to pin a leak.** There are three ways, and the first is the one to reach for:

1. **On demand, from the running daemon — `memoriahub node heap-snapshot`.** Let the worker run under import load until the watchdog shows a clear positive `heapGrowthMbPerHour`, then run the command; the *live* daemon serializes its heap to `~/.memoriahub/logs/heap-manual-<pid>-<timestamp>.heapsnapshot` and prints the path. This is the preferred route because restarting a leaking worker to attach a diagnostic flag throws away exactly the accumulated state that names the retainer. The write stops the world for several seconds on a multi-GB heap (harmless — in-flight leases are renewed far inside the server's lease window), and the CLI waits up to 3 minutes for it.
2. **Automatically, at the pre-OOM valve.** When `heapUsed` crosses `MEMORIAHUB_HEAP_RESTART_FRACTION`, the worker writes `heap-critical-<pid>-<timestamp>.heapsnapshot` into the same directory *before* draining and exiting, and logs the path. This exists because the valve fires **below** where V8 would ever invoke `--heapsnapshot-near-heap-limit`: without it, a hardened worker recycles cleanly forever and never produces the snapshot the flag promised.
3. **The launch flag,** `--heapsnapshot-near-heap-limit=1` (applied automatically unless `MEMORIAHUB_HEAP_SNAPSHOT=0`), which writes `Heap.*.heapsnapshot` to the process's **working directory** at genuine near-OOM. In practice (2) pre-empts it whenever the valve is enabled.

Both (1) and (2) refuse to write unless free disk is at least **1.5×** the current heap, and keep only the **newest 2** snapshots in the log directory, so a repeatedly-recycling worker cannot fill the volume. Open the file in Chrome DevTools → Memory → **Load**, sort by **Retained Size**, and the top constructor names the retainer.
...
- **`memoriahub node doctor`** (CLI, or Worker Node ▸ Node doctor in the TUI) — reports API access, installed-vs-operational capability status per dependency, per-job-type readiness, model manifest verification, and daemon liveness. See [Worker Node Setup & Troubleshooting §8](worker-node-setup.md#8-reading-node-doctor-output) for how to read installed vs. operational.
...
| 1.4 | August 2026 | AI Assistant | Issue #156: documented the two heap-snapshot paths that actually fire — the pre-OOM valve's own snapshot (the valve fires *below* where V8's `--heapsnapshot-near-heap-limit` would ever run, so without it a hardened worker never produces one) and the on-demand `memoriahub node heap-snapshot` command — plus their disk pre-flight/retention guards and the watchdog's new `heapGrowthMbPerHour` slope (§3.3, §3.5). |

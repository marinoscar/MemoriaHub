...
describe('listDuplicateGroups — sortBy/sortOrder (issue #189)', () => {
  it('omits sortBy and sortOrder from the URL when neither is provided (byte-identical to before)', async () => {
    ...
    await listDuplicateGroups({ circleId: 'circle-1', status: 'pending', page: 1, pageSize: 20 });
  ...
  it('includes sortBy and sortOrder in the URL when provided', async () => {
    ...
    await listDuplicateGroups({ circleId: 'circle-1', sortBy: 'confidence', sortOrder: 'desc' });
...
describe('fetchAllPendingDuplicateGroupIds — no sort params', () => {
  it('never sends sortBy/sortOrder on any page request', async () => {
    ...
    await fetchAllPendingDuplicateGroupIds({ circleId: 'circle-1' });

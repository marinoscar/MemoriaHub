...
/** Shape of the slice of `GET /api/user-settings` this hook cares about. */
interface UserSettingsDataTables {
...
export function useDataTableLayoutPrefs<Row>({
  ...
  const write = useCallback(
    (next: DataTableStoredLayout | null) => {
      ...
      void api
        .patch('/user-settings', { dataTables: { [tableId]: next } })
        .catch((error: unknown) => {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        const settings = await api.get<UserSettingsDataTables>('/user-settings');

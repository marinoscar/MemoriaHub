...
const computeGeocode: ComputeFn = async (_inputPath, _params, ctx): Promise<GeocodeComputeResult> => {
  ...
  try {
    ...
    // GeoProviderRateLimitError signals a 429/5xx/quota-exhaustion throttle —
    // rethrown as a plain Error (same note as auto-tagging.ts: the node
    // engine's failure-report path does not yet forward a rateLimited flag).
    if (err instanceof GeoProviderRateLimitError) {
      throw new Error(`${err.provider} rate limit / throttle — will retry: ${err.message}`);
    }
    throw err;
  }
};

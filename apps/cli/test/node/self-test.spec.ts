 ...
 *   - `testClip` / `testTesseract` mock the corresponding
 *     `@memoriahub/enrichment-compute/*` subpath exports via
 *     jest.unstable_mockModule so the tests never depend on the heavy
 *     optionalDependencies (onnxruntime-node, tesseract.js) actually being
 *     installed in CI — only the "is a model file present on disk" branch
 *     and the "does the compute call succeed/fail" branch are under test
 *     here.
 ...
 * `@memoriahub/enrichment-compute/face` no longer exists as a package export,
 * and `testHuman()`/`humanModelBasePath()`/`HUMAN_TIMEOUT_MS` were deleted
 * from self-test.ts. There is no Human self-test coverage anymore; only
 * `testCompreface` covers face-provider self-testing.
 */

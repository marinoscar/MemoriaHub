 ...
 *  - backfillBurstDetection: RBAC (collaborator), 400 when burstDetectionEnabled=false,
 *    force=false skips already-grouped items, force=true enqueues all
 */
...
import { BurstGroupStatus, CircleRole, JobReason, JobStatus, MediaType } from '@prisma/client';
...
function makeBackfillDto(overrides: Partial<BurstBackfillDto> = {}): BurstBackfillDto {
  return { circleId: CIRCLE_ID, force: false, ...overrides } as BurstBackfillDto;
}
...
describe('BurstService', () => {
  ...
  // -------------------------------------------------------------------------
  // backfillBurstDetection
  // -------------------------------------------------------------------------

  describe('backfillBurstDetection', () => {
    it('calls assertCircleAccess with collaborator role', async () => {
      ...
      await service.backfillBurstDetection(makeBackfillDto(), USER_ID, PERMS_MEDIA_WRITE);
    ...
    it('throws NotFoundException when circle not found', async () => {
      ...
      await expect(
        service.backfillBurstDetection(makeBackfillDto(), USER_ID, PERMS_MEDIA_WRITE),
      ).rejects.toThrow(NotFoundException);
    });

    it('throws BadRequestException when burstDetectionEnabled=false', async () => {
    ...
    it('enqueues with backfill reason and priority 100', async () => {
      (mockPrisma.circle.findUnique as jest.Mock).mockResolvedValue({ burstDetectionEnabled: true });
      (mockPrisma.enrichmentJob.findMany as jest.Mock).mockResolvedValue([]);
      (mockPrisma.mediaItem.findMany as jest.Mock).mockResolvedValue([
        { id: 'media-1' },
      ...
      await service.backfillBurstDetection(makeBackfillDto({ force: false }), USER_ID, PERMS_MEDIA_WRITE);
    ...
    it('force=false skips items that already have a burstGroupId or succeeded job', async () => {
      (mockPrisma.circle.findUnique as jest.Mock).mockResolvedValue({ burstDetectionEnabled: true });
      // Already-succeeded job for media-1
      (mockPrisma.enrichmentJob.findMany as jest.Mock).mockResolvedValue([
        { mediaItemId: 'media-1' },
      ]);
      // Remaining ungrouped items without succeeded jobs
      (mockPrisma.mediaItem.findMany as jest.Mock).mockResolvedValue([{ id: 'media-2' }]);
      mockEnrichmentJobService.enqueue.mockResolvedValue({ id: 'job-new', status: JobStatus.pending });

      const result = await service.backfillBurstDetection(
        makeBackfillDto({ force: false }),
        USER_ID,
        PERMS_MEDIA_WRITE,
      );

      // Only media-2 should be enqueued
    ...
    it('force=true enqueues all non-deleted photos regardless of existing jobs', async () => {
      ...
      mockEnrichmentJobService.enqueue.mockResolvedValue({ id: 'job-x', status: JobStatus.pending });
      ...
      // All 3 items enqueued; no job filtering query
      expect(mockEnrichmentJobService.enqueue).toHaveBeenCalledTimes(3);
      expect(result.data.enqueued).toBe(3);
    });

    it('force=true queries only type=photo non-deleted items', async () => {
      (mockPrisma.circle.findUnique as jest.Mock).mockResolvedValue({ burstDetectionEnabled: true });
      (mockPrisma.mediaItem.findMany as jest.Mock).mockResolvedValue([]);

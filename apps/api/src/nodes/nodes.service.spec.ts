 ...
 *     has no mediaItemId or linked StorageObject; happy path derives a
 *     VERSIONED thumbnails/<storageObjectId>-<version>.jpg key (issue #434 —
 *     the key must differ per run so regenerated bytes never land at a URL a
 *     browser/CDN has cached as immutable) and returns the resolver's signed
 *     PUT URL
...
describe('NodesService — result/failure ingestion', () => {
  ...
  describe('getJobUploadUrl', () => {
    ...
    it('derives a VERSIONED thumbnails/<storageObjectId>-<version>.jpg key and returns the resolver-signed PUT URL (thumbnail_regen)', async () => {
      (mockPrisma.enrichmentJob.findUnique as jest.Mock).mockResolvedValue(
        makeHeldJob({ type: 'thumbnail_regen' }),
      );
      (mockPrisma.mediaItem.findUnique as jest.Mock).mockResolvedValue({
      ...
      expect(res.storageKey).toMatch(
        /^thumbnails\/original-object-id-[0-9a-f]{16}\.jpg$/,
      );
    ...
    it('derives the same versioned key shape for thumbnail_repair', async () => {
      (mockPrisma.enrichmentJob.findUnique as jest.Mock).mockResolvedValue(
    ...
    // Issue #434: the whole point of the version is that the key MOVES. Two
    // consecutive upload-url calls for the same object must not collide, or a
    // node-regenerated thumbnail would again overwrite bytes that browsers and
    // CDNs were told to cache as immutable for a year.
    it('issues a DIFFERENT key on each call for the same storage object', async () => {
      (mockPrisma.enrichmentJob.findUnique as jest.Mock).mockResolvedValue(
        makeHeldJob({ type: 'thumbnail_regen' }),
      );
      (mockPrisma.mediaItem.findUnique as jest.Mock).mockResolvedValue({

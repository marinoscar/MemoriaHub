...
describe('NodesService — result/failure ingestion', () => {
  ...
  describe('getModelManifest', () => {
    ...
    it('returns exactly 1 entry (the human/rekognition model files were removed with issue #113)', () => {
      const manifest = service.getModelManifest();

      expect(manifest).toHaveLength(1);
    });

    it('has no entry targeting the human subdir (Human provider model files were removed)', () => {
      const manifest = service.getModelManifest();

      expect(manifest.some((m) => m.targetSubdir === 'human')).toBe(false);
    });
    ...
    it('has the exact committed sha256/bytes for the CLIP ONNX entry', () => {
      const manifest = service.getModelManifest();
